...
- **UI buttons and social functions: INCOMPLETE** — next missing behavior:
  ...
  friend request `0xB9E9` and remove friend `0x98A1` — had no row anywhere in
  this matrix, so nothing recorded that they exist or how far they got. Each answer is the player's own
  ...
  reaching a client. `observed_frames = 0` for all five, which is why every row
