...
class SceneScopedCombatWiringTests(unittest.TestCase):
    ...
    def _start_game(self, state, character):
        ...
        # is about scene-scoped combat, not pose, so class_id is cleared
        # here, after StartGame binds `foundation.selected` (it is None
        # before this call), shared by both `_state` and `_state_at_scene2`.
