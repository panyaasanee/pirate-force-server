"""LANE-B: scene 7 (Bg0007) is a REGISTERED combat scene.
...
crash-bg0009-two-ambiguous-rows.md).  ``field_mob_tables_bg0007`` is
registered in
``field_mobs._SCENE_TABLE_MODULES`` in the same commit that adds it, the same
shape bg0003/bg0004/bg0005/bg0008/bg0006 already ship: no staged gate.

NINE placements over SEVEN distinct templates, no player-model body among
them and no withhold.

``test_the_ai_register_opens_for_every_shipped_row`` is the trap reproduced
on this scene before the AI union widened: ``MobAiControlError:
ai_row_missing: placement 28 points at AI_WANDER 10, which is not in the
mined rows`` -- the first of this round's four scenes to actually crash on
its own, since bg0006's two rows happened to already be covered.
"""
...
MODULE_PATH = SRC / "pirateforce_foundation" / "field_mob_tables_bg0007.py"
GAMEDATA = ROOT.parent / "pf_bridge" / "gamedata"

EXPECTED_SCENE = "Bg0007"
EXPECTED_SCENE_ID = 7
EXPECTED_HOSTILE_COUNT = 9
EXPECTED_TEMPLATE_COUNT = 7
EXPECTED_UNAMBIGUOUS = 38
EXPECTED_CENSUS_RANK = 9
EXPECTED_CENSUS_AI_COMBAT = 9
EXPECTED_CENSUS_RANK_AND_AI_COMBAT = 9
EXPECTED_CENSUS_DROPS_NORMAL = 9
...
EXPECTED_SHIPPED_ROWS = (
    (28, 388, 0x201D, "Ominous Bird"),
    (29, 388, 0x201E, "Ominous Bird"),
    (35, 390, 0x2024, "Dark roar"),
    (40, 393, 0x2029, "Avarice Lerch"),
    (43, 395, 0x202C, "Remain Alert Weapon"),
    (44, 395, 0x202D, "Remain Alert Weapon"),
    (48, 397, 0x2031, "Green Eye Minced"),
    (55, 536, 0x2038, "Jet cat thieves No.7"),
    (56, 526, 0x2039, "Jet cat thieves No.8"),
)
...
class Bg0007ShapeTests(unittest.TestCase):
    ...
    def test_registering_scene_seven_left_the_other_scenes_alone(self) -> None:
    ...
    def test_the_shipped_templates_have_a_death_ruling_and_a_stray_row_still_refuses(
        ...
        self.assertEqual(
            ...
            frozenset(
                row[1] for row in field_mob_tables_bg0007.HOSTILE_PLACEMENTS),
        )
        self.assertEqual(
            mob_death.WIDENING_RULING_SCENES[RULING_NAME],
            field_mob_tables_bg0007.SCENE,
        )
        rows = field_mobs.roster_for_scene_id(EXPECTED_SCENE_ID)
        self.assertEqual(len(rows), EXPECTED_HOSTILE_COUNT)
        seen_templates = set()
...
class Bg0007RegenerateTests(unittest.TestCase):
    ...
    def test_regenerating_reproduces_the_committed_module_byte_for_byte(
        ...
        self.assertEqual(
            ...
            "src/pirateforce_foundation/field_mob_tables_bg0007.py is stale - "
            "regenerate with tools/pf_mine_scene_mob_roster.py --gamedata "
            "<bridge>/gamedata --scene Bg0007 --identity-rule cline --out "
            "<this file>",
        )
