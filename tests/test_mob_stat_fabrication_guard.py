...
LANE_B_MODULES = (
    ...
    # ROUND 4m2kx7: scene 8 (Bg0008, Silver Harbour), generated by the same
    # miner as every other roster module here and swept by the same rules.
    # No player stat of any kind.  It is the first entry on this tuple whose
    # scene is NOT in field_mobs._SCENE_TABLE_MODULES -- no death ruling
    # covers its templates yet, so it is declared in
    # field_mobs.MINED_NOT_SHIPPED_TABLE_MODULES instead.  That changes
    # nothing this guard cares about: the file exists on disk, so it is
    # listed here, which is what this tuple is for.
