...
LANE_B_MODULES = (
    ...
    "field_mob_tables_bg0003.py",
    # ROUND r6isy5: scene 4 (bg0004, Slave Market Island), generated by the
    # same tool as its siblings and swept by the same rules -- the first
    # scene this lane ships whose four hostility predicates disagree, and
    # the first to carry an n_DROPS_SPECIALLY set.
    "field_mob_tables_bg0004.py",
    # ROUND jqeo2m: scene 5 (bg0005, Evil Port), generated by the same
    # miner as every other roster module here.  No player stat of any kind:
    # every column is copied from a committed MOBS/STANDARD_MOB row, and
    # ``max_hp`` is the one derived column, derived the same way for all
    # four scenes.  Listed in the same commit as the module.
    "field_mob_tables_bg0005.py",
    # THIS ROUND: scene 8 (Bg0008, Silver Harbour), generated by the same
    # miner as every other roster module here.  No player stat of any kind:
    # every column is copied from a committed MOBS/STANDARD_MOB row, and
    # ``max_hp`` is the one derived column, derived the same way for every
    # other scene.  Listed in the same commit as the module.
    "field_mob_tables_bg0008.py",
    # THIS ROUND: scenes 6, 7, 9 and 11, generated by the same miner as
    # every other roster module here (COO-DECISION 2026-09-06T07:48+07:00).
    # No player stat of any kind: every column is copied from a committed
    # MOBS/STANDARD_MOB row, and ``max_hp`` is the one derived column,
    # derived the same way for every other scene.  Listed in the same
    # commit as the four modules.
