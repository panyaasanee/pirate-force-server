"""LANE-A: the Pale Silver Sea's census, on the real frozen serializers.

The wire/DB half of the two-layer evidence rule for scene 305.  What this
...
Mob-Set number, the builder refuses every scene but 305, and the console
lines a headless boot would print say the true numbers - which for this
scene include NO drops at all, the claim that would rot most quietly if a
table row went missing.

What it cannot prove, and does not: that a client draws any of it - and on
this scene the gap is wider than on the islands, because 25 of the 59
shipped bodies are ``INVISIBLE`` weather markers and 5 more are
``MAP_ISLAND_01``, neither of which anyone has watched a client render as an
actor.  The
...
from pirateforce_foundation import world_population_bg3008 as census  # noqa: E402
...
# 001.json, n_id=305): CONSTDATA_TH__MARKER row 345, pinned by COO-DECISION
...
# nobody uses.
ANCHOR = tuple(
    world_scene_travel.spawn_position(world_scene_travel.destination(305)))


class Bg3008Census(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.legacy = load_legacy(LEGACY_PATH)

    def _build(self, **kwargs):
        return census.build_bg3008_population(
    ...
    def test_the_whole_roster_assembles(self) -> None:
        ...
        self.assertEqual(generation.actor_count, 59)
        self.assertEqual(len(generation.placement_indices), 59)
        self.assertEqual(len(generation.n_ids), 59)

    def test_header_count_equals_the_bodies_in_the_frame(self) -> None:
        for count in (1, 7, 21, 59):
            with self.subTest(count=count):
                generation = census.build_bg3008_population(
    ...
    def test_the_four_two_set_placements_ship_the_first_legs_body(
        self,
    ) -> None:
        """Leg 57 is MOBS 8167; leg 58 is MOBS 8171 - the same two bodies
        the sibling scene's ``53|54`` pair resolves to, reached through this
        scene's own CLINE rows.  The rule is only worth writing down if the
        bytes obey it."""
        by_index = {
    ...
    def test_every_placement_reaches_the_wire_and_the_siblings_do_not(
        self,
    ) -> None:
        """This scene drops nothing, so the sibling's "the dropped ones are
        absent" test is replaced by its opposite: all 59 placement indices
        are in the frame.  The four bodyless leaders scene 304 loses
        (8176-8179) are checked as ABSENT here too - not because this scene
        drops them, but because their presence would mean this census was
        built from the wrong scene's crosswalk, which every count in this
        file would otherwise still call correct."""
        generation = self._build()
        self.assertEqual(
            sorted(generation.placement_indices), list(range(59)))
    ...
    def test_a_truncated_census_carries_the_nearest_actors(self) -> None:
        few = census.build_bg3008_population(
    ...
    def test_it_refuses_every_scene_but_305(self) -> None:
        for scene_id in (1, 2, 3, 11, 14, 17, 126, 130, 278, 304, "305", None):
            with self.subTest(scene_id=scene_id):
                with self.assertRaises(census.Bg3008CensusError):
                    census.build_bg3008_population(
        ...
        for anchor in ((1.0, 2.0), [1.0, 2.0, 3.0], (1.0, 2.0, float("nan"))):
            with self.subTest(anchor=anchor):
                with self.assertRaises(census.Bg3008CensusError):
                    census.build_bg3008_population(
                        self.legacy, anchor, scene_id=census.SCENE_N_ID)
        # 60 is one past the roster - a caller may not ask for more actors
        # than this scene has bodies for.
        for count in (0, -1, 60, True, 3.0):
            with self.subTest(count=count):
                with self.assertRaises(census.Bg3008CensusError):
                    census.build_bg3008_population(
    ...
    def test_the_console_line_states_the_true_shortfall(self) -> None:
        ...
        self.assertIn("assembled=59/59", line)
        self.assertIn("shippable=59", line)
        self.assertIn("wire=59", line)
        self.assertIn("unresolved=0", line)
        # NOT ``identity_unresolved``: this scene loses nothing, so the
        # honest shortfall is none, and a line that claimed one would be
    ...
    def test_the_headless_lines_name_every_actor_and_every_drop(self) -> None:
        ...
        self.assertEqual(len(actor_lines), 59)
        self.assertEqual(census.unresolved_lines(), ())
        joined = "\n".join(actor_lines)
        for name in ("Viking Princess", "Santa Maria", "Skull Phantom",
                     "Utopia", "Merchant Ship", "Merchant marine Trade Ship",
                     "Pirate Ship", "Tornado", "Ice Island", "Turtle Island",
    ...
    def test_there_are_no_drop_lines_because_there_are_no_drops(
        self,
    ) -> None:
        """The sibling prints sixteen ``BG3007_UNSHIPPED`` lines.  This
        scene prints none, and the token itself must not appear anywhere in
        the console output - a stray one would send a tester looking for an
        actor that was never missing."""
        self.assertEqual(census.unresolved_lines(), ())
        lines = census.census_console_lines(self.legacy, ANCHOR)
        self.assertEqual(len(lines), 1 + 59)
        for line in lines:
            with self.subTest(line=line[:40]):
                self.assertNotIn("BG3008_UNSHIPPED", line)
    ...
    def test_only_the_population_seam_imports_this_module(self) -> None:
        ...
        for path in (ROOT / "src").rglob("*.py"):
            if path.name == "world_population_bg3008.py":
                continue
            ...
            for node in ast.walk(tree):
                ...
                if "world_population_bg3008" in names:
