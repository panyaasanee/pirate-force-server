"""LANE-A: Bg0007's census, on the real frozen serializers.
...
Mob-Set number, the builder refuses every scene but 7, and the console lines
...
from pirateforce_foundation import world_population_bg0007 as census  # noqa: E402
from pirateforce_foundation.legacy_bridge import load_legacy  # noqa: E402


LEGACY_PATH = ROOT / "current" / "pf_login_game_server_v141.py"
# The scene's own registry spawn (scenarios/world_scene_registry_001.json,
# n_id=7): SCENE_NAME[7].n_MARKER = 7 -> MARKER[7].
ANCHOR = (-23266.0, 7709.0, 5220.0)


class Bg0007Census(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.legacy = load_legacy(LEGACY_PATH)

    def _build(self, **kwargs):
        return census.build_bg0007_population(
    ...
    def test_the_whole_roster_assembles(self) -> None:
        ...
        self.assertEqual(generation.actor_count, 56)
        self.assertEqual(len(generation.placement_indices), 56)
        self.assertEqual(len(generation.n_ids), 56)

    def test_header_count_equals_the_bodies_in_the_frame(self) -> None:
        for count in (1, 7, 45, 56):
            with self.subTest(count=count):
                generation = census.build_bg0007_population(
    ...
    def test_nearest_first_order_puts_the_anchor_placement_first(self) -> None:
        # Placement 1 (Mob-Set 2, "Columbus") is the closest SHIPPABLE
        # placement to the registry's own spawn point -- placement 0
        # (Mob-Set 1) is nearer in raw distance but unresolved (no MOBS row
        # for its leader), the same "nearest is unshippable" shape bg0008's
        # own test recorded.  Measured directly, not assumed.
        generation = self._build()
        self.assertEqual(generation.placement_indices[0], 1)
        self.assertEqual(generation.display_names[0], "Columbus")

    def test_it_refuses_every_scene_but_seven(self) -> None:
        for scene_id in (1, 2, 3, 4, 5, 6, 8, 10, 11, 14, 278, "7", None):
            with self.subTest(scene_id=scene_id):
                with self.assertRaises(census.Bg0007CensusError):
                    census.build_bg0007_population(
    ...
    def test_the_console_line_states_the_true_shortfall(self) -> None:
        ...
        self.assertIn("assembled=56/68", line)
        self.assertIn("shippable=56", line)
        self.assertIn("wire=56", line)
        self.assertIn("bodies=ok", line)
        self.assertIn("unresolved=12", line)
        self.assertIn("shortfall=identity_unresolved=12", line)
        # The 68 target is never quietly rewritten to 56 (CHARTER-02).
        self.assertNotIn("assembled=56/56", line)
    ...
    def test_the_headless_lines_name_every_actor_and_every_drop(self) -> None:
        ...
        self.assertEqual(len(lines), 1 + 56 + 12)
        self.assertTrue(all(line.isascii() for line in lines))
        self.assertTrue(lines[0].startswith("WORLD_CENSUS_BG0007 "))
        self.assertEqual(
            sum(1 for line in lines if line.startswith("BG0007_UNSHIPPED ")),
            12)
    ...
    def test_only_the_population_seam_imports_this_module(self) -> None:
        ...
        # `p4wire`'s/`p7wm17`'s precedent for scenes 5, 6, 8 and 3), so this
        # module never carries the "nothing imports this yet" tripwire its
        # earlier siblings each had to rename in a later round -- this file
        # starts with the widened, final shape directly.
        ...
        # 7 gained a combat roster (field_mob_tables_bg0007,
        # COO-DECISION 2026-09-06T07:48+07:00) and a recompose composer
        # (mob_scene_recompose.COMPOSER_BG0007) that calls THIS builder for
        ...
        for path in (ROOT / "src").rglob("*.py"):
            if path.name == "world_population_bg0007.py":
                continue
            ...
            for node in ast.walk(tree):
                ...
                if any("world_population_bg0007" in name for name in names):
