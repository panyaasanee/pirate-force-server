...
class TheReviewedShapesArePinnedTests(unittest.TestCase):
    ...
    EXPECTED = {
        ...
        # exactly 20 bytes, checked with `!=` against BOTH the answerer's
        # own constant and this row (round m54yxh D7's shape).  64 is the
        # same deliberately-inexact frame bound the row above uses; the
        # frame measured 52 on the commit that added it, pinned one file
        # over in tests/test_lane_ui_friend_remove_answer.py.
