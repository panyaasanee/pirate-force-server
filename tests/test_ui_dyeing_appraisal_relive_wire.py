...
+ fail-closed (truncated / wrong-tag / trailing-bytes) coverage for the six
fully-tagged classes across the ``Appraisal``/``Dyeing``/``Relive`` catalog
groups. Not wiring tests -- see ``ui_social_wire.py``'s module docstring.
The excluded classes (``DyeingVitalReq``, ``UserSetting_
UpdateServerSettingVital``, ``ReliveMarkerVital``, ``ItemLockVital``,
``DyeingModule_Client``, ``AppraisalModule_Client``) have no module and no
tests here -- see ``ui_dyeing_appraisal_relive_wire.py``'s module docstring
for why.
"""
...
class VitalIdTests(unittest.TestCase):
    ...
    def test_all_six_ids_are_distinct(self):
        ids = {
