...
class GroundLootDispatchTests(unittest.TestCase):
    ...
    def test_the_pair_is_appended_after_the_inherited_actions(self):
        ...
        # corpse_override -> full_roster_override).  ``ungated`` now goes
        ...
        # frozen fallback ``gated`` takes is untouched by that override.
        # Measured, not assumed, the same way ``added_bytes`` above is
        # measured: the delta between an overridden and a plain rung-3
        # build.
        ...
        roster_override = mob_death.full_roster_override(
            ...
            ungated.mob_death_register, ledger=ungated.mob_combat_ledger,
        )
        overridden_rung3 = (
