...
class Bg0011TableShape(unittest.TestCase):
    ...
    def test_the_lane_b_hostile_roster_module_agrees_with_this_table(
        ...
        tests.  ``field_mob_tables_bg0011.py`` exists now and is registered
        in ``field_mobs._SCENE_TABLE_MODULES`` (COO-DECISION widen-death-
        scope-bg0011-five-templates 2026-09-06T07:48+07:00).
        ...
        self.assertTrue(
            (ROOT / "src" / "pirateforce_foundation"
             / "field_mob_tables_bg0011.py").is_file())
        self.assertIn(
            field_mob_tables_bg0011.SCENE, field_mobs._SCENE_TABLE_MODULES)
        sets = field_mob_tables_bg0011.SET_NUMBER_FOR_PLACEMENT
        for row in field_mob_tables_bg0011.HOSTILE_PLACEMENTS:
