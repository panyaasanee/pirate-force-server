"""LANE-A: Bg0005's crosswalk, and the controls that make it believable.

Same discipline as ``tests/test_world_bg0004_identity.py`` and
...
class Bg0005TableShape(unittest.TestCase):
    def test_the_scene_is_the_one_the_module_says_it_is(self) -> None:
        self.assertEqual(identity.SCENE_N_ID, 5)
        self.assertEqual(identity.SCENE_MODEL_ID, "BG0005")
        # A direct SCENE_NAME selector (one of RE-128's 19), not one of its
        # 240 instance scenes.
        self.assertEqual(identity.SCENE_CLINE_TYPE, 5)
        self.assertEqual(identity.SCENE_DECLARED_LEVEL, 60)

    def test_counts_are_the_ones_the_docstring_states(self) -> None:
        self.assertEqual(len(identity._RESOLVED_ROWS), 59)
        self.assertEqual(len(identity.UNRESOLVED), 5)
        self.assertEqual(identity.PLACEMENT_COUNT, 92)
        self.assertEqual(len(identity.shippable_placements()), 87)
        self.assertEqual(len(identity.unshippable_placements()), 5)

    def test_control_1_scene_sets_and_the_table_keys_are_the_same_64(
        self,
    ) -> None:
        scene_sets = {row[1] for row in identity._PLACEMENT_ROWS}
        table_sets = set(identity.IDENTITIES) | set(identity.UNRESOLVED)
        self.assertEqual(len(table_sets), 64)
        self.assertEqual(scene_sets, table_sets)
        # This scene uses CLINE type 5's entire key range - unlike bg0004
        # (61 of 62 keys) and bg0010 (40 of 41 keys), there is no key CLINE
        # type 5 carries that this scene's placements never touch.
        self.assertTrue({1, 59, 101, 105} <= table_sets)
    ...
    def test_the_five_dropped_placements_split_into_two_reasons(self) -> None:
        ...
        # Set 1: CLINE carries a real leader (104) but MOBS has no row for
        # it at all -- a different failure mode from the other four, which
        # DO have a MOBS row, just no outfit.
        self.assertIn("no row", by_template[1]["reason"])
        self.assertEqual(by_template[1]["leader_n_id"], 104)
        for template_id in (101, 102, 103, 104):
    ...
    def test_no_extraction_unresolved_sentinel_in_this_scene(self) -> None:
        # Unlike bg0010's placement index 50, no row's template_ids column
        # is the literal UNRESOLVED, and no placement carries a sentinel -1
        # template id.
    ...
    def test_identity_for_never_substitutes(self) -> None:
        ...
        self.assertEqual(identity.identity_for(2).mobs_n_id, 105)
        ...
        # this module claims.
        self.assertEqual(identity.identity_for(2).cline_row_id, 1801)
        for row in identity.IDENTITIES.values():
    ...
    def test_multi_variant_outfits_ship_their_first_variant(self) -> None:
        ...
        # Measured, not estimated: 38 of 87 shippable placements (set 44
        # alone repeats 9 times), well over a third of the roster - a
        # different distribution from bg0010's, but the same "not a corner
        # case" shape that module's own docstring records.
    ...
    def test_no_name_column_disagreement_in_this_scene(self) -> None:
        ...
        # name (MOBSET_NN) matches its machine-parsed template_ids column
        # here, checked for all 92 rows.
    ...
    def test_self_check_refuses_a_drifted_table(self) -> None:
        ...
        try:
            ...
            with self.assertRaises(identity.Bg0005IdentityError):
