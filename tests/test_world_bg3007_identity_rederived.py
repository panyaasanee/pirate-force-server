"""LANE-A: the Dark Fog Sea crosswalk, re-derived from the committed TSVs.

WHY THIS FILE EXISTS.  ``world_bg3007_identity`` ships 37 hand-checked table
rows and 66 placement rows, and a wrong digit in any of them puts a wrong
...
scene 304: it re-derives the crosswalk from the bridge's own files and
compares it to the shipped literal FIELD BY FIELD.
...
gate-runnable half of the claim is ``tests/test_world_bg3007_identity.py``,
...
_REQUIRED_SOURCES = (
    ...
    Path("scene") / "Bg3007" / "Bg3007.placements.tsv",
)
...
class TheShippedTableMatchesTheSource(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.placements = _rows(
            cls.root / "scene" / "Bg3007" / "Bg3007.placements.tsv")
        cls.keys = {
    ...
    def test_every_dropped_set_is_dropped_for_the_reason_given(self) -> None:
        """This scene's ONE drop shape, re-derived rather than trusted.

        All four dropped sets name a leader the CLINE row really carries and
        that ``CONSTDATA_TH__MOBS`` really has no row for - the shape the
        module docstring calls new for this lane.  A future regeneration
        that turned one of them into a leader-0 drop, or into a body that
        exists after all, has to change this table and this test together.
        """
        ...
        for template_id, (cline_row_id, leader, reason) in sorted(
            ...
            with self.subTest(set=template_id):
                ...
                self.assertNotEqual(leader, 0)
                self.assertNotIn(str(leader), self.mobs)
                self.assertTrue(reason.isascii())
                self.assertIn("no CONSTDATA MOBS row", reason)
                # The tip table DOES name all four, and the reason quotes
                # that name: a reader who greps the console for "Yamato"
                # must find both the shipped set-4 ship and this drop, and
                # be able to tell them apart.
                self.assertIn(str(leader), self.tip)
                self.assertIn(self.tip[str(leader)]["s_NAME"], reason)
    ...
    def test_the_docstrings_measured_numbers_are_still_true(self) -> None:
        ...
        self.assertEqual(len(self.placements), 66)
    ...
    def test_no_crew_column_is_set_on_this_cline_block(self) -> None:
        ...
        self.assertEqual(backups, ["61410"])
