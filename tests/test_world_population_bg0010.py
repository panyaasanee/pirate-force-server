"""LANE-A: Bg0010's census, on the real frozen serializers.
...
Mob-Set number, the builder refuses every scene but 10, and the console lines
...
# n_id=10): SCENE_NAME[10].n_MARKER = 10 -> MARKER[10].
ANCHOR = (15740.0, 25461.0, 465.0)


class Bg0010Census(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.legacy = load_legacy(LEGACY_PATH)

    def _build(self, **kwargs):
        return census.build_bg0010_population(
    ...
    def test_the_whole_roster_assembles(self) -> None:
        ...
        self.assertEqual(generation.actor_count, 94)
        self.assertEqual(len(generation.placement_indices), 94)
        self.assertEqual(len(generation.n_ids), 94)

    def test_header_count_equals_the_bodies_in_the_frame(self) -> None:
        for count in (1, 7, 55, 94):
            with self.subTest(count=count):
                generation = census.build_bg0010_population(
    ...
    def test_nearest_first_order_puts_the_anchor_placement_first(self) -> None:
        generation = self._build()
        self.assertEqual(generation.placement_indices[0], 64)
        self.assertEqual(generation.display_names[0], "Columbus")

    def test_it_refuses_every_scene_but_ten(self) -> None:
        for scene_id in (1, 2, 4, 11, 14, 278, "10", None):
            with self.subTest(scene_id=scene_id):
                with self.assertRaises(census.Bg0010CensusError):
                    census.build_bg0010_population(
    ...
    def test_the_console_line_states_the_true_shortfall(self) -> None:
        ...
        self.assertIn("assembled=94/100", line)
        self.assertIn("shippable=94", line)
        self.assertIn("wire=94", line)
        self.assertIn("bodies=ok", line)
        self.assertIn("unresolved=6", line)
        self.assertIn("shortfall=identity_unresolved=6", line)
        # The 100 target is never quietly rewritten to 94 (CHARTER-02).
        self.assertNotIn("assembled=94/94", line)
    ...
    def test_the_headless_lines_name_every_actor_and_every_drop(self) -> None:
        ...
        self.assertEqual(len(lines), 1 + 94 + 6)
        self.assertTrue(all(line.isascii() for line in lines))
        self.assertTrue(lines[0].startswith("WORLD_CENSUS_BG0010 "))
        self.assertEqual(
            sum(1 for line in lines if line.startswith("BG0010_UNSHIPPED ")),
            6)
    ...
    def test_only_the_population_seam_imports_this_module(self) -> None:
        ...
        for path in (ROOT / "src").rglob("*.py"):
            if path.name == "world_population_bg0010.py":
                continue
        ...
        #
        # [CROSS-LANE EDIT BY LANE-B, ROUND 9t75cr] mob_scene_recompose.py
        # becomes a third importer, same addition round 4tnhzw made on
        # world_population_bg0011's own copy of this test: scene 10 gained
        # a combat roster (field_mob_tables_bg0010, COO-DECISION
        # widen-death-scope-bg0010-six-templates 2026-09-06T14:53+07:00)
        # and a recompose composer (mob_scene_recompose.COMPOSER_BG0010)
        # that calls THIS builder for this scene's MID-SESSION recompose
        # census. The arrival census is lane A's (via the seam below); the
        # recompose of that same census after a hit or a kill is lane B's,
        # and lane B composes it by CALLING this builder rather than
        # writing a second one. runtime.py still does not import either.
        self.assertEqual(
            sorted(importers),
