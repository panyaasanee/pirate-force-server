...
def _raw_vitals(path, character_id):
    """The row's own columns, read outside the store's accessors entirely.

    Round `uhfve8` follow-up: the Windows gate (`pirate-force-server#819`)
    failed at `TemporaryDirectory` teardown with `PermissionError: [WinError
    32] ... being used by another process` on this exact test's database --
    an ad-hoc `sqlite3.connect(...).execute(...)` with no `.close()` leaves
    the connection alive (and, in WAL mode, its `-shm`/`-wal` handles) for
    however long CPython's collector takes to get to it, which is not
    "before the test's `addCleanup(self.tmp.cleanup)` runs" on that
    platform, even though it always was on Linux, which is why this passed
    in every local/sandbox run before it ever reached that gate.  This
    helper is the fix: `finally: db.close()`, the same shape every
    `sqlite3.connect` call already committed in this repository uses
    (`tests/test_persistence_ground_drops_010.py`'s own `_raw_rows`
    included) and the one shape this file's two ad-hoc connects had not
    copied.
    """
...
class ListRosterWritesNothingButTheAccountTests(_Workspace):
    def test_listing_an_existing_account_changes_nothing(self):
        ...
        fixture.seed_rows(self.path, MIGRATIONS, [(character_id, 9, 250)])
        before = _raw_vitals(self.path, character_id)
        roster = fixture.list_roster(
            self.path, MIGRATIONS, "gt221-fixture-tests"
        )
        after = _raw_vitals(self.path, character_id)
