...
class TheGroupingMatchesLiveCodeTests(unittest.TestCase):
    ...
    def _files_naming(self, needle):
        ...
        for path in SRC.rglob("*.py"):
            # .as_posix(), not str(): str() on a WindowsPath returns
            # "gm\\attr_wire.py" -- the gate runs on windows-latest, so a
            # hand-typed "/"-separated allow-list would go permanently
            # unmatched there while still matching on a Linux clone,
            # letting this test pass locally and fail red only on the
            # gate (observed: PR pirate-force-server#949, gate run
            # 34038141149, `extra = ['gm\\attr_wire.py']`).
