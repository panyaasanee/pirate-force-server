"""LANE-A: Bg0003's census, on the real frozen serializers.
...
Mob-Set number, the builder refuses every scene but 3, and the console lines
...
from pirateforce_foundation import world_population_bg0003 as census  # noqa: E402
from pirateforce_foundation.legacy_bridge import load_legacy  # noqa: E402


LEGACY_PATH = ROOT / "current" / "pf_login_game_server_v141.py"
# The scene's own registry spawn (scenarios/world_scene_registry_001.json,
# n_id=3): SCENE_NAME[3].n_MARKER = 3 -> MARKER[3].
ANCHOR = (-21215.0, 16907.0, -830.0)


class Bg0003Census(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.legacy = load_legacy(LEGACY_PATH)

    def _build(self, **kwargs):
        return census.build_bg0003_population(
    ...
    def test_the_whole_roster_assembles(self) -> None:
        ...
        self.assertEqual(generation.actor_count, 62)
        self.assertEqual(len(generation.placement_indices), 62)
        self.assertEqual(len(generation.n_ids), 62)

    def test_header_count_equals_the_bodies_in_the_frame(self) -> None:
        for count in (1, 7, 45, 62):
            with self.subTest(count=count):
                generation = census.build_bg0003_population(
    ...
    def test_nearest_first_order_puts_the_anchor_placement_first(self) -> None:
        # Placement 0 (Mob-Set 1, "Columbus") is both the closest to the
        # registry's own spawn point AND shippable directly - unlike
        # bg0008's own nearest placement, which was unresolved.  Measured
    ...
    def test_it_refuses_every_scene_but_three(self) -> None:
        for scene_id in (1, 2, 4, 5, 6, 8, 10, 11, 14, 278, "3", None):
            with self.subTest(scene_id=scene_id):
                with self.assertRaises(census.Bg0003CensusError):
                    census.build_bg0003_population(
    ...
    def test_the_console_line_states_the_true_shortfall(self) -> None:
        ...
        self.assertIn("assembled=62/72", line)
        self.assertIn("shippable=62", line)
        self.assertIn("wire=62", line)
        self.assertIn("bodies=ok", line)
        self.assertIn("unresolved=10", line)
        self.assertIn("shortfall=identity_unresolved=10", line)
        # The 72 target is never quietly rewritten to 62 (CHARTER-02).
        self.assertNotIn("assembled=62/62", line)
    ...
    def test_the_headless_lines_name_every_actor_and_every_drop(self) -> None:
        ...
        self.assertEqual(len(lines), 1 + 62 + 10)
        self.assertTrue(all(line.isascii() for line in lines))
        self.assertTrue(lines[0].startswith("WORLD_CENSUS_BG0003 "))
        self.assertEqual(
            sum(1 for line in lines if line.startswith("BG0003_UNSHIPPED ")),
            10)
    ...
    def test_only_the_population_seam_imports_this_module(self) -> None:
        ...
        # `p4wire`'s precedent for scenes 5, 6 and 8), so this module never
        # carries the "nothing imports this yet" tripwire its earlier
        # siblings each had to rename in a later round -- this file starts
        # with the widened, final shape directly.
        import ast

        importers = []
        for path in (ROOT / "src").rglob("*.py"):
            if path.name == "world_population_bg0003.py":
                continue
