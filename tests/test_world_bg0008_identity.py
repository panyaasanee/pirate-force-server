"""LANE-A: Bg0008's crosswalk, and the controls that make it believable.
...
class Bg0008TableShape(unittest.TestCase):
    def test_the_scene_is_the_one_the_module_says_it_is(self) -> None:
        self.assertEqual(identity.SCENE_N_ID, 8)
        self.assertEqual(identity.SCENE_MODEL_ID, "Bg0008")
        # A direct SCENE_NAME selector (one of RE-128's 19), not one of its
        # 240 instance scenes.
        self.assertEqual(identity.SCENE_CLINE_TYPE, 8)
        self.assertEqual(identity.SCENE_DECLARED_LEVEL, 86)
    ...
    def test_counts_are_the_ones_the_docstring_states(self) -> None:
        ...
        self.assertEqual(identity.PLACEMENT_COUNT, 76)
        self.assertEqual(len(identity.shippable_placements()), 69)
        self.assertEqual(len(identity.unshippable_placements()), 7)

    def test_control_1_scene_sets_and_the_table_keys_are_the_same_48(
        self,
    ) -> None:
        scene_sets = {row[1] for row in identity._PLACEMENT_ROWS}
        table_sets = set(identity.IDENTITIES) | set(identity.UNRESOLVED)
        self.assertEqual(len(table_sets), 48)
        self.assertEqual(scene_sets, table_sets)
        # This scene uses CLINE type 8's entire key range, the same shape
        # scenes 5's and 6's own crosswalks carry (unlike bg0004's 61-of-62
        # and bg0010's 40-of-41).  See the module docstring for the
        # registry's own ``native_definition_count`` (49) disagreeing with
        # this measured 48 - recorded there, not silently reconciled here.
        self.assertTrue({1, 42, 101, 106} <= table_sets)
    ...
    def test_the_seven_dropped_placements_split_into_two_reasons(
        self,
    ) -> None:
        ...
        # all.  Set 106: CLINE's own leader column is 0, which resolves to
        # no leader at all -- a leader-less variant of the same "no row"
        # family, so this scene's own leader_n_id for it is 0 rather than
        # positive (checked explicitly, not folded into the same assertion
        # as set 1's).
        self.assertIn("no row", by_template[1]["reason"])
        self.assertGreater(by_template[1]["leader_n_id"], 0)
        self.assertIn("no row", by_template[106]["reason"])
        self.assertEqual(by_template[106]["leader_n_id"], 0)
    ...
    def test_identity_for_never_substitutes(self) -> None:
        ...
        self.assertEqual(identity.identity_for(2).mobs_n_id, 250)
        ...
        self.assertEqual(identity.identity_for(2).cline_row_id, 2401)
        for row in identity.IDENTITIES.values():
    ...
    def test_multi_variant_outfits_ship_their_first_variant(self) -> None:
        ...
        # Measured, not estimated: 36 of 69 shippable placements.
    ...
    def test_self_check_refuses_a_drifted_table(self) -> None:
        ...
        try:
            ...
            with self.assertRaises(identity.Bg0008IdentityError):
