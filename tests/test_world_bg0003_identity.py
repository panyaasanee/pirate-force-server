"""LANE-A: Bg0003's crosswalk, and the controls that make it believable.

Same discipline as ``tests/test_world_bg0004_identity.py``,
``tests/test_world_bg0005_identity.py``, ``tests/test_world_bg0006_identity.py``,
``tests/test_world_bg0008_identity.py`` and ``tests/test_world_bg0010_identity.py``,
...
from pirateforce_foundation import world_bg0003_identity as identity  # noqa: E402


class Bg0003TableShape(unittest.TestCase):
    def test_the_scene_is_the_one_the_module_says_it_is(self) -> None:
        self.assertEqual(identity.SCENE_N_ID, 3)
        self.assertEqual(identity.SCENE_MODEL_ID, "Bg0003")
        # A direct SCENE_NAME selector (one of RE-128's 19), not one of its
        # 240 instance scenes.
        self.assertEqual(identity.SCENE_CLINE_TYPE, 3)
        self.assertEqual(identity.SCENE_DECLARED_LEVEL, 25)
    ...
    def test_counts_are_the_ones_the_docstring_states(self) -> None:
        ...
        self.assertEqual(identity.PLACEMENT_COUNT, 72)
        self.assertEqual(len(identity.shippable_placements()), 62)
        self.assertEqual(len(identity.unshippable_placements()), 10)

    def test_control_1_scene_sets_and_the_table_keys_are_the_same_51(
        self,
    ) -> None:
        ...
        # This scene uses CLINE type 3's entire key range, the same shape
        # scenes 5's, 6's and 8's own crosswalks carry (unlike bg0004's
        # 61-of-62 and bg0010's 40-of-41).  See the module docstring for the
        # registry's own ``native_definition_count`` (52) disagreeing with
        # this measured 51 - recorded there, not silently reconciled here.
        self.assertTrue({1, 40, 101, 111} <= table_sets)
    ...
    def test_the_ten_dropped_placements_split_into_two_reasons(
        self,
    ) -> None:
        ...
        # Set 2: CLINE carries a real leader but MOBS has no row for it at
        # all.
        self.assertIn("no row", by_template[2]["reason"])
        self.assertGreater(by_template[2]["leader_n_id"], 0)
        ...
        # Unlike every sibling scene's own 101+ block, this scene's does NOT
        # cover its entire tail: sets 110 and 111 both resolve.
        self.assertIn(110, identity.IDENTITIES)
        self.assertIn(111, identity.IDENTITIES)
    ...
    def test_identity_for_never_substitutes(self) -> None:
        ...
        self.assertEqual(identity.identity_for(1).name, "Columbus")
        self.assertEqual(identity.identity_for(1).mobs_n_id, 36)
        ...
        self.assertEqual(identity.identity_for(1).cline_row_id, 1400)
        for row in identity.IDENTITIES.values():
    ...
    def test_multi_variant_outfits_ship_their_first_variant(self) -> None:
        ...
        # Measured, not estimated: 25 of 62 shippable placements.
        self.assertEqual(len(affected), 25)
        # UNLIKE every sibling scene so far, one of the nine lists NINE
        # variants rather than two - the widest fan-out this lane has
        # recorded.
        self.assertEqual(
            identity.MULTI_VARIANT_OUTFITS[908].count(";"), 8)
    ...
    def test_self_check_refuses_a_drifted_table(self) -> None:
        ...
        try:
            ...
            with self.assertRaises(identity.Bg0003IdentityError):
