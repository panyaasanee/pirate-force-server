"""LANE-A: Bg0009's crosswalk, and the controls that make it believable.
...
class Bg0009TableShape(unittest.TestCase):
    def test_the_scene_is_the_one_the_module_says_it_is(self) -> None:
        self.assertEqual(identity.SCENE_N_ID, 9)
        self.assertEqual(identity.SCENE_MODEL_ID, "Bg0009")
        # A direct SCENE_NAME selector (one of RE-128's 19), not one of its
        # 240 instance scenes.
        self.assertEqual(identity.SCENE_CLINE_TYPE, 9)
    ...
    def test_counts_are_the_ones_the_docstring_states(self) -> None:
        ...
        self.assertEqual(len(identity.UNRESOLVED), 6)
        self.assertEqual(identity.PLACEMENT_COUNT, 63)
        self.assertEqual(len(identity.shippable_placements()), 57)
        self.assertEqual(len(identity.unshippable_placements()), 6)

    def test_control_1_scene_sets_and_the_table_keys_are_the_same_44(
        self,
    ) -> None:
        scene_sets = {row[1] for row in identity._PLACEMENT_ROWS}
        table_sets = set(identity.IDENTITIES) | set(identity.UNRESOLVED)
        self.assertEqual(len(table_sets), 44)
        self.assertEqual(scene_sets, table_sets)
        # This scene uses a SUBSET of CLINE type 9's own 48-row key range
        # (44 of 48), the same "subset, not exact" shape bg0004's 55-of-61
        # and bg0010's 40-of-41 own crosswalks carry (unlike scenes 5's,
        # 6's, 7's and 8's own EXACT-match crosswalks).  See the module
        # docstring for the four CLINE keys (38, 39, 40, 106) this scene
        # never touches, three of which carry a real leader anyway.
        self.assertTrue({1, 37, 41, 44, 101, 105} <= table_sets)
        self.assertFalse({38, 39, 40, 106} & table_sets)
    ...
    def test_the_six_dropped_placements_split_into_two_reasons(
        self,
    ) -> None:
        ...
        # No "leader is literally zero" set this scene needed (unlike
        # bg0007's set 111): every dropped reason string is one of the two
        # families above.
        for row in rows:
            self.assertTrue(
    ...
    def test_identity_for_never_substitutes(self) -> None:
        ...
        self.assertEqual(identity.identity_for(2).mobs_n_id, 285)
        ...
        self.assertEqual(identity.identity_for(2).cline_row_id, 2601)
        for row in identity.IDENTITIES.values():
            self.assertGreater(row.cline_row_id, 0)

    def test_multi_variant_outfits_ship_their_first_variant(self) -> None:
        self.assertEqual(len(identity.MULTI_VARIANT_OUTFITS), 11)
        ...
        # Measured, not estimated: 30 of 57 shippable placements.
        self.assertEqual(len(affected), 30)
        # UNLIKE bg0003's nine-variant outlier, every one of this scene's
        # eleven multi-variant sets lists exactly two variants.
    ...
    def test_self_check_refuses_a_drifted_table(self) -> None:
        ...
        try:
            ...
            with self.assertRaises(identity.Bg0009IdentityError):
