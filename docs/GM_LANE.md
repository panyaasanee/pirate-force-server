...
## รอบ `y6j1mn` (2026-09-04 10:12 +07:00) -- x=9 กลายเป็นฉากปัจจุบัน และรั้วตัวเลือกสองตัว

บริโภค `COO-DECISION 20260904_0846` ทั้งใบ (สามข้อ)

**ข้อ 1 = ตัวเลือก (ข)**: x=9 (`category_5C` = ตัวเลือกคู่ HP ตาม `SELECTOR_NOTE_R301`)
ต้องเป็น **ฉากปัจจุบันของ session ตอนส่งเฟรม** ไม่ใช่ไบต์ที่ล็อกอินเขียนไว้
อ่านไม่ได้ = ปฏิเสธ ไม่มีเฟรมเกิดขึ้น และ **ไม่มี fallback ไปไบต์ล็อกอิน** --
fallback นั้นคือตัวเลือก (ก) ที่ `0846` ปัดตกด้วยตัวอักษร

จุดอ่านที่สาม `CURRENT_SCENE_READ_POINT = "current_session_scene_id"` --
**ยังไม่มีตัวตน** สะกดไว้ที่เดียวเหมือน `LIVE_VALUE_READ_POINT` และ
`LOGIN_BYTES_READ_POINT` · `CORE-REQUEST-GM-054` ขอ chief · `0846` อนุญาต
ให้ raise ที่ประตูก่อนแล้วระบุชื่อฮุกไว้ ซึ่งคือสิ่งที่รอบนี้ทำ

### ทำไมรั้วไม่ได้เขียนว่า `0x430E10(x9) == 8`

`SELECTOR_NOTE_R301` [PROVEN in-repo] บอกว่าไคลเอนต์ป้อน x=9 เข้าฟังก์ชัน
`0x430E10` แล้วเทียบ **ผลลัพธ์** กับ 8 -- **ไม่ใช่** เทียบ x=9 กับ 8
`0x430E10` ยังไม่ถูกถอด เซิร์ฟเวอร์จึงประเมินเงื่อนไขนี้ไม่ได้กับค่าใด ๆ เลย
รั้วที่เขียนตามเงื่อนไขจึงเป็นรั้วที่เขียนตามการเดา รอบนี้ปฏิเสธที่จะทำแบบนั้น

สิ่งที่พูดได้โดยไม่ต้องถอดอะไรเลย: **ไคลเอนต์รอดค่าที่ล็อกอินส่งมาแล้ว ทุกวัน**
(วัดไว้ `LANE-GM 20260904_0505`) ⇒ เฟรมที่ส่งค่าเดิมซ้ำพลิกอะไรไม่ได้ ไม่ว่า
`0x430E10` จะทำอะไรกับมัน · เฟรมที่ **เปลี่ยน** ค่าคือเฟรมที่ไม่มีใครรับประกันได้
นั่นคือรูปของ `_refuse_selector_change` และมันไม่ต้องใช้ image เลย

รั้วที่ COO ระบุด้วยมือ (`x9 == 8` ไม่มี x=52/x=53) ยังลงไว้ที่กำแพง
`make_update_attr_frame` แต่ **ติดป้ายในโค้ดว่ามันแคบและไม่ใช่เงื่อนไขพลิก**
และมีเทสที่แดงถ้าใครลบคำเตือนนั้นทิ้ง · สองรั้วไม่ทับกัน: ตัวที่กำแพงจับผู้เรียก
ที่ประกอบ dict เองแล้วไม่เคยผ่าน `live_full_block_values` (ตัวที่แหล่งมองไม่เห็น)
ตัวที่แหล่งจับค่าที่ตัวกำแพงเรียกชื่อไม่ได้

### ราคาที่รอบนี้สร้างขึ้น -- เขียนไว้ ไม่ซ่อน

`_refuse_selector_change` ปฏิเสธ **กรณีที่ถูกต้องด้วย**: ผู้เล่นที่เดินออกจากฉาก
ที่ล็อกอินไว้ มีฉากปัจจุบันต่างจากไบต์ล็อกอิน ⇒ ทุกประตูที่ประกอบบล็อกเต็ม
stand-down ให้คนคนนั้น (Door B และ `/speed` รวมอยู่ด้วย)
**บนบูตจริงวันนี้ไม่มีอะไรเปลี่ยน** -- ทั้งสองประตูปฏิเสธอยู่แล้วเพราะ
`LOGIN_BYTES_READ_POINT` ยังไม่มี ราคานี้เริ่มมีผลวันที่ chief ลงจุดอ่าน
ทางยกมีสามทาง เขียนไว้ในใบ `20260904_1024` ถึง COO ทางเดียวที่ **ไม่** อยู่ในนั้น
คือการเดาว่า `0x430E10` ทำอะไร

**เขต GM**: `gm/attr_wire.py`, `tests/test_gm_attr_wire.py`, `tests/test_gm_login_mask.py`
**นอกเขต ประกาศแล้ว** (ใบ `20260904_1023` ถึง LANE-B): `mob_hit_frame.py` ส่งผ่าน
read point ตัวที่สามเข้า shim รูปเดียวกับที่มันส่งผ่าน `LOGIN_BYTES_READ_POINT` อยู่แล้ว
และ `tests/test_lane_b_mob_ai_tick.py::_compose` ได้ `scene_hook` ที่ค่าเริ่มต้น
**derive จาก `login_hook` เอง ไม่พิมพ์เลข** -- ถ้าไม่แตะ Door B จะปฏิเสธด้วยเหตุผล
เรื่อง shim ไม่ใช่เรื่องโลกจริง และเทส LANE-B หกใบแดง

### ผู้เทสจะทำอะไรได้ที่เมื่อวานทำไม่ได้

**ไม่มีอะไรใหม่** -- และนั่นคือผลที่ตั้งใจ รอบนี้ทำให้ประตูปฏิเสธ **มากขึ้น** ไม่ใช่น้อยลง
สิ่งที่ต่างบนคอนโซลถ้าวันหนึ่งมีคนเรียกประตูจริง: บรรทัด `GM_ATTR_SELECTOR_STANDDOWN`
ที่บอกชื่อจุดอ่านที่ขาด หรือบอกว่าฉากจะเปลี่ยนจากค่าไหนเป็นค่าไหน แทนความเงียบ

### nonclaim

- ไม่ได้ใช้ความสามารถ GM เพื่อไปถึงผลใด ๆ ในรอบนี้ -- ไม่บูตเซิร์ฟเวอร์ ไม่บูตเกม
- ไม่มีไบต์ `0x309A` ออกจากประตูไหน (ยังไม่มีจุดเรียกใน `runtime.py` เลย)
- ไม่อ้างว่า `0x430E10` ถูกถอดแล้ว -- **ยังไม่ถูกถอด** และรั้วทั้งสองตัวถูกออกแบบ
  มาให้ทำงานได้โดยไม่ต้องรู้คำตอบนั้น
- ไม่อ้างว่า `GT-218` ถูกตอบ · ไม่อ้างว่า `/speed` (b'') Door B M2 M3 M4 P-2 P-3 ขยับ
- ไม่แตะ `runtime.py` / `app.py` / `pf_login_game_server_v141.py` / canonical DB /
  `scenarios/world_*.json` / `scenarios/combat_*.json`
- ประวัติเดิมขีดฆ่า ไม่ลบ

### 🔴 แก้รอบ `y6j1mn` เอง 10:5x -- pf-adversary หักล้างหัวเรื่องของรอบนี้

ผลคืนหลัง push แปดข้อ สองข้อวิกฤต ขีดฆ่าไว้ที่นี่ ไม่ลบส่วนบน

- **D1 [MEASURED]** ~~"ไบต์ล็อกอินไม่ขึ้นเฟรมอีกแล้ว"~~ **ผิด** ·
  `_refuse_selector_change` raise เว้นแต่ค่าเท่ากัน ⇒ `combined[9] = current_scene`
  เป็น **self-assignment** · ลบบรรทัดนั้นทิ้ง 296 เทสยังเขียว รวมมิวแทนต์ที่ COO สั่ง
  ⇒ สิ่งที่ลงคือ **ตัวเลือก (ก) + การปฏิเสธ** ไม่ใช่ (ข) · ข้อ 1 ของ `0846` **ยังไม่ได้ implement**
- **D2 [MEASURED]** `build_named_field_update` ประกอบจาก `RawBlockCache`
  **ไม่เรียก `live_full_block_values`** ⇒ ไม่มีรั้วบนประตูนั้นเลย · วัดปลายถึงปลาย:
  cache ที่ฉาก 3 + จุดอ่านตอบ 5 ⇒ ประกอบเฟรม 129 ไบต์ที่มีฉากเก่าบนตัวเลือก **ไม่มีบรรทัดคอนโซล**
  = มิวแทนต์ที่ `0846` สั่งให้แดง เขียวอยู่เพราะเทสไปยิงฟังก์ชันที่ประตูนี้ไม่เรียก
- **D3 แก้แล้ว** `overrides={9: ...}` ทะลุรั้วได้ (apply หลังรั้ว) ⇒ `build_login_shaped_frame` ปฏิเสธ x=9
- **D4/D5 ติดป้ายแล้ว** รั้ว `x9==8`: false positive ที่มีชื่อ (ฉาก 8 = `Silver Harbour` ฉากจริง
  ⇒ ปฏิเสธถาวร) · false negative = ทุกฉากที่ `0x430E10` แมพเป็น 8 · เงื่อนไข x=52/53
  ของรั้วทั้งสอง **ไม่เคยถูกรันเลย** (instrument แล้ว 0 ครั้ง)
- **D6 แก้แล้ว** `gm/warp_executor.execute_warp` **ไม่มีอยู่จริงทั้งรีโป** (พิมพ์จากความจำ) ·
  และ field ที่ `0846` ชี้ (`position.scene_id`) คือค่าที่ **เซิร์ฟเวอร์เดา** ตามที่ไฟล์ของสายนี้เอง
  บันทึกไว้ ⇒ `CORE-REQUEST-GM-054` แก้ให้ขอ `client_confirmed_scene` +
  `scene_label_is_server_guess` (ทั้งคู่อยู่บน `main` แล้วตั้งแต่ R328)
- **D7 แก้แล้ว** f-string ของบรรทัดคอนโซลอยู่นอก guard ⇒ identity ที่ไม่ใช่ int ทำให้เกิด
  `ValueError` เปล่าที่ `except AttrWireError` จับไม่ได้

**คำถามที่ adversary ถามและรอบนี้ตอบไม่ได้**: รั้วเปลี่ยนค่ายืนบน "ไคลเอนต์รอดค่าที่ล็อกอินส่ง"
แต่ล็อกอินส่งมันใน **บล็อกสร้าง actor** ส่วน `0x309A` คือ apply กับ actor ที่มีอยู่แล้ว --
ซึ่งคือคำถามเปิดของ `RE-222` เป๊ะ ๆ ถ้าเป็นสมบัติของการสร้าง หลักฐานของรั้วระเหยทั้งก้อน
ส่งถึง COO ในใบ `20260904_1055` ไม่เดา

**งานแรกของรอบ GM ถัดไป**: D1 + D2 -- x=9 เป็นกลุ่มแหล่งที่สาม + ย้ายรั้วไป `make_update_attr_frame`

## รอบ `zq18m1` -- 2026-09-04T11:5x+07:00 -- D2 ปิด (ด้วยการปฏิเสธ ไม่ใช่การเดินสาย) · ใบ RE `0x430E10` ร่างส่ง chief

`COO-DECISION 20260904_1046` ตอบใบ `20260904_1024`: รั้ว `x9==8` คงเป็น stand-down (ไม่ลดเป็น
เตือน) ยืนยัน · สั่งร่างใบ RE `0x430E10` รอบ 12:11 · ราคา "รีล็อกอินก่อนตี" ไปกับขั้นตอนเทส ไม่ใช่โค้ด
(chief ใส่ลง `GT-224` เอง) · ไม่มีจดหมายใหม่ถึง LANE-GM ใบอื่นที่ยังไม่บริโภค (เช็คทุกใบ
`ADDRESSEE: LANE-GM` -- มีแค่ `1046` ที่ยังไม่มี `.CONSUMED.txt`)

### D2 [MEASURED, ปิดแล้ว] -- `build_named_field_update` ปฏิเสธ `SELECTOR_ROW_X` โดยชื่อ

รอบ `y6j1mn` วัดว่า `build_named_field_update` ประกอบจาก `RawBlockCache` โดยไม่เคยเรียก
`live_full_block_values` เลย -- ตัวรั้วเปลี่ยนแปลงค่า (`_refuse_selector_change`) จึงไม่มีวันรัน
บนประตูนี้ ตัวรั้วแคบที่ `make_update_attr_frame` (`x9==8`) จับได้แค่ค่าที่ตรง 8 พอดี ค่าอื่น
ทุกค่าทะลุไปได้ วัดซ้ำก่อนแก้: cache ที่ล็อกอินฉาก 3, `build_named_field_update(..., x=9,
value=5)` ประกอบเฟรม 129 ไบต์จริงที่มีเลข 5 บนตัวเลือก ไม่มีบรรทัดคอนโซล

**ทางแก้จริง (D1)**: x=9 เป็นกลุ่มแหล่งของตัวเอง สร้างจาก `live_current_scene` -- ต้องใช้จุดอ่าน
`CORE-REQUEST-GM-054` ที่ chief ยังไม่ลง (กำหนด 12:51 ตาม `COO 1047`) **ยังเปิดอยู่**

**ทางปิดรอบนี้ (D2)**: แทนที่จะเดินสายประตูนี้ผ่าน `live_full_block_values` (ซึ่งต้องเดา
contract ของผู้เรียกในอนาคตที่ยังไม่มี ก่อน GM-054 จะลง) `build_named_field_update` ปฏิเสธ
`x == SELECTOR_ROW_X` **โดยชื่อ** ตั้งแต่ต้นฟังก์ชัน เหมือน `SENSITIVE_FIELDS` -- ประตูที่รันรั้ว
เปลี่ยนแปลงค่าไม่ได้ จะแตะแถวที่รั้วนั้นคุ้มครองไม่ได้เลย จนกว่า GM-054 จะลงและ D1 ปิดของจริง
แถวยังตั้งค่าได้ผ่านประตูเดียวที่มีรั้วครบ: `login_mask.build_login_shaped_frame`

ปักด้วย `test_d2_the_selector_row_is_refused_by_this_door_by_name` (สามค่า: ค่าจากล็อกอินเอง,
8, และค่าอื่น -- ทั้งสามถูกปฏิเสธ) และ `test_D2_the_named_field_door_no_longer_reaches_either_
value_at_all` (เปลี่ยนชื่อจาก `..._is_NOT_behind_either_fence` -- การ์ดใบเดิมพลิกความหมาย
จาก "หลุมที่ยังไม่ได้ปิด" เป็น "ปิดแล้วด้วยวิธีนี้") · `test_every_login_set_row_is_individually_
composable` แก้ให้ตัดแถว x=9 ออก (แบบเดียวกับที่ `mp_current` ถูกตัดออกไปก่อนหน้านี้)

### ใบ RE `0x430E10` -- ร่างส่งแล้ว รอ chief ตั้งเลขและมอบหมาย

`notes_to_chief/20260904_1154_LANE-GM-RE-0x430E10-TICKET-selector-category-to-alt-hp-pair-
mapping.md` (pf_bridge) -- ขอ decode ฟังก์ชัน `0x430E10` เต็มตัว (ชนิด lookup/คำนวณ + ถ้าเป็น
lookup ขอรายการค่า `category_5C` ที่คืนผล 8 ครบ) `[STATIC-ON-BRIDGE]` ต้องอิมเมจ ทำที่นี่ไม่ได้
ค้น `external/`, `gamedata/`, `VITAL_REGISTRY_FROM_CLIENT_BINARY_20260817.tsv` แล้ว -- ไม่เจอ

### เขต GM

`gm/attr_wire.py` (docstring + refusal ใหม่ใน `build_named_field_update`, ปรับคอมเมนต์ที่ถูก
STRUCK ใน `live_full_block_values` ให้ตรงกับสถานะปิด/เปิดจริง), `tests/test_gm_attr_wire.py`
(เทสใหม่ 1 ตัว + เทสเดิม 2 ตัวแก้ให้ตรงกับพฤติกรรมใหม่) ไม่แตะไฟล์อื่น

### ผู้เทสจะทำอะไรได้ที่เมื่อวานทำไม่ได้

**ไม่มีอะไรใหม่บนจอ** -- รอบนี้ปิดหลุมความปลอดภัยภายในของ `attr_wire.py` (ประตูที่เคยทะลุรั้ว
เงียบ ๆ ตอนนี้ปฏิเสธพร้อมบรรทัดคอนโซลเสมอ) ไม่มีจุดเรียกจาก `runtime.py` เข้าประตูนี้อยู่แล้ว
วันนี้ ⇒ ไม่มีพฤติกรรมที่ผู้เล่นหรือผู้เทสเห็นเปลี่ยนไป การแก้นี้ป้องกัน "รอบต่อไปที่เผลอเรียก
ประตูนี้กับ x=9" ไม่ให้ประกอบเฟรมอันตรายเงียบ ๆ

### nonclaim (เขียนตอน ~12:06 -- ก่อนพบ `1149`)

- ไม่ได้ใช้ความสามารถ GM เพื่อไปถึงผลใด ๆ ในรอบนี้ -- ไม่บูตเซิร์ฟเวอร์ ไม่บูตเกม
- ไม่มีไบต์ `0x309A` ออกจากประตูไหนที่ยังไม่เคยออก (ยังไม่มีจุดเรียกใน `runtime.py`)
- ~~D1 ยังไม่ปิด -- บล็อกอยู่ที่ `CORE-REQUEST-GM-054` ไม่ใช่งานของรอบนี้~~ -- **แก้ต่อในหัวข้อ
  ถัดไป**: `COO-DECISION 20260904_1149` (พบตอน `git fetch` รอบสอง ~12:10 หลัง push commit นี้ยัง
  ไม่เกิดขึ้นจริง) สั่งให้ D1+D2 ต้องทำพร้อมกันในรอบนี้ ก่อนใบ RE -- ใบ RE ถูกส่งไปแล้วตอน 11:54
  (ก่อนพบจดหมาย ลำดับคลาดกับที่ `1149` สั่งภายหลัง เนื้อหาใบไม่ผิด)
- ไม่อ้างว่า `0x430E10` ถูกถอดแล้ว -- ใบ RE เพิ่งร่างส่ง ยังไม่มีผล
- ไม่อ้างว่า `GT-218` ถูกตอบ · ไม่อ้างว่า `/speed` (b'') Door B M2 M3 M4 P-2 P-3 ขยับ
- ไม่แตะ `runtime.py` / `app.py` / `pf_login_game_server_v141.py` / canonical DB /
  `scenarios/world_*.json` / `scenarios/combat_*.json`
- ประวัติเดิมขีดฆ่า ไม่ลบ

## รอบ `zq18m1` ต่อ (~12:10-13:0x+07:00) -- พบ `COO-DECISION 20260904_1149` กลางรอบ: D2 ยกระดับเป็นรั้วจริงที่ `make_update_attr_frame`

`git fetch origin main` รอบสองก่อน push (ตามกติกา) เจอ `1149` (`ADDRESSEE: LANE-GM`, เขียน 11:49
-- ก่อนรอบนี้เริ่มแค่ 6 นาที แต่รอบนี้เพิ่งอ่านตอน 12:10) สั่งว่า: D1+D2 มาก่อนทุกอย่างในรอบ 12:11
(รอบนี้เอง) · ถ้าไม่จบในรอบเดียว ใบ RE เลื่อนไป 13:41 (ใบ RE ส่งไปแล้วตอน 11:54 -- ไม่ใช่ปัญหาเนื้อหา
แค่ลำดับคลาด บันทึกไว้ไม่ถอน) · D2 ต้องเป็น "รั้วเดียวที่ `make_update_attr_frame` ทุกผู้ประกอบต้อง
ผ่าน · มิวแทนต์ (a) ของ `0846` ต้องแดงจริงที่ประตู `build_named_field_update`" -- ไม่ใช่การปฏิเสธ
ทั้งแถวแบบที่ทำไปก่อนหน้าในรอบนี้เอง (ยัง**ไม่เคย push**)

### D2 ยกระดับ: รั้วที่สามที่ `make_update_attr_frame` (opt-in ผ่าน `character_id`)

`make_update_attr_frame` รับพารามิเตอร์ใหม่ `*, character_id=None, hooks=None` (keyword-only,
ดีฟอลต์ `None` = ปิด ไม่กระทบผู้เรียกเดิมสักตัว) เมื่อมี `character_id` และ `SELECTOR_ROW_X` อยู่ใน
`values` ฟังก์ชันเรียก `live_current_scene(character_id, hooks=hooks)` เอง แล้วเทียบกับ
`values[SELECTOR_ROW_X]` -- ค่าไหนก็ตามที่กำลังจะออกจากกำแพงนี้ ไม่ว่าจะมาจากไหน ต้องตรงกับความจริง
ของ session ตอนนี้ ไม่ตรง = ปฏิเสธ (`selector_would_change_at_the_wall`)

**ทำไมเป็น opt-in ไม่ใช่บังคับ**: `mob_hit_frame.py` (LANE-B's Door B) เรียกฟังก์ชันนี้ตรง และเป็น
ไฟล์นอกเขตเขียนของสายนี้ -- บังคับ `character_id` ที่กำแพงจะพังการเรียกของ Door B ทันทีโดยไม่มีสิทธิ์
แก้ไฟล์นั้นเอง (Door B ไม่ต้องพึ่งรั้วนี้อยู่แล้ว: มันเรียก `live_full_block_values` ก่อนเสมอ ซึ่งรัน
รั้วเปลี่ยนแปลงค่าจริงไปแล้วต้นทาง) `build_named_field_update` (ประตูของสายนี้เอง ไม่มีผู้เรียก
production วันนี้) เปลี่ยน `character_id` เป็น **required keyword-only** เสมอ -- ทุกการเรียกผ่าน
ประตูนั้นจึงเปิดรั้วที่สามโดยอัตโนมัติ ไม่มีทางเลี่ยง

`login_mask.build_login_shaped_frame` (ในเขตสายนี้) ก็ส่ง `character_id`/`hooks` ต่อให้กำแพงด้วย
เพื่อความสม่ำเสมอ (เป็น no-op บนเส้นทางนี้เพราะค่ามาจาก `live_full_block_values` ที่ถูกต้องอยู่แล้ว)

**มิวแทนต์ (a) ของ `0846` แดงจริงผ่านประตู `build_named_field_update` แล้ว** -- ปักด้วย
`test_D2_the_named_field_door_now_re_verifies_at_the_wall` (`SelectorRowIsTheCurrentSceneTests`)
และ `test_d2_a_stale_cached_selector_is_caught_at_the_wall_even_setting_an_unrelated_field`
(`BuildNamedFieldUpdateTests`) -- ตัวหลัง reproduce การวัดของ pf-adversary ตรงตัว: cache ที่ล็อกอิน
สนามหนึ่ง hook ปัจจุบันตอบอีกสนาม แล้วตั้งค่าฟิลด์ที่ไม่ใช่ selector เลย ก็ยังถูกปฏิเสธ

x=9 ที่เป็น**เป้าที่ผู้เรียกเลือกเอง**ผ่านประตูนี้ ยังถูกปฏิเสธเหมือนเดิม (เปลี่ยนแค่ข้อความ ไม่ใช่
พฤติกรรม) -- ค่า selector มาจาก session เสมอ ไม่มีประตูไหนให้ผู้เรียกเลือกเอง (สอดคล้องกับที่
`login_mask.build_login_shaped_frame` ปฏิเสธแถวเดียวกันใน `overrides` อยู่แล้ว)

~~**D1 ยังไม่ปิด** (ตามที่ `1149` ยอมรับว่าอาจไม่จบในรอบเดียว):~~ **ปิดแล้วรอบ `zjbjys`** -- ย่อหน้านี้เก็บไว้เป็นประวัติ อ่านหัวข้อรอบ `zjbjys` ท้ายไฟล์แทน: `split_sources`/`LOGIN_SOURCED_ROWS`
ยังไม่ถูกปรับให้ x=9 เป็นกลุ่มแหล่งที่สามแยกจาก `live_login_bytes` -- ผลลัพธ์ไบต์ที่ส่งจริงเหมือนเดิม
ทุกประการ (ยังคง self-assignment เมื่อ current==login เท่านั้น) การไม่ทำรอบนี้เป็นการตัดสินใจ ไม่ใช่
การลืม: เป็น refactor เพื่อความชัดเจนของโค้ด ไม่ใช่การแก้ safety (รั้วที่สามข้างบนปิดช่องโหว่ที่วัดได้
จริงแล้วโดยไม่ต้องแตะ `split_sources`) และมีความเสี่ยงต่อการรีเกรสในโค้ดที่เทสละเอียดมากอยู่แล้ว --
บันทึกเป็นงานแรกของรอบ GM ถัดไป (เหมือนที่ `y6j1mn` บันทึกไว้ก่อนหน้า)

### เขต GM (แก้เพิ่มจากหัวข้อบน)

เพิ่ม `gm/login_mask.py` (บรรทัดเดียว: ส่ง `character_id`/`hooks` ต่อให้ `make_update_attr_frame`)
· `tests/test_gm_attr_wire.py` แก้เพิ่ม (เทสเก่าทุกตัวที่เรียก `build_named_field_update` เติม
`character_id=`, เทสใหม่ 3 ตัว) · `tests/test_gm_speed_wire.py` แก้ 1 บรรทัด (เติม `character_id=`)

### จดหมายที่ยังไม่ได้ตอบกลับ (บันทึกไว้ ยังไม่ได้เขียนตอนจบรอบนี้)

`LANE-B-TO-LANE-GM`-style suggestion ยังไม่ได้ส่ง: เสนอให้ Door B ผ่าน `character_id`/`hooks` เข้า
`make_update_attr_frame` ของตัวเองด้วย เพื่อกำแพงป้องกันจริงแทน no-op -- ไม่บังคับ (Door B ปลอดภัย
อยู่แล้วผ่านเส้นทางของมันเอง) แต่เป็น defense-in-depth ที่สายนี้ทำให้เองไม่ได้ (ไฟล์นอกเขต)

### ผู้เทสจะทำอะไรได้ที่เมื่อวานทำไม่ได้ (แก้จากหัวข้อบน)

**ยังไม่มีอะไรใหม่บนจอ** เหมือนเดิม -- ข้อแตกต่างจากฉบับก่อนหน้าในรอบเดียวกัน: รั้วที่ปิดตอนนี้เป็น
รั้วที่ตรวจค่าจริง (ปฏิเสธเมื่อผิดจริง อนุญาตเมื่อถูกจริง) แทนการปฏิเสธ x=9 ทุกกรณีแบบเหมารวม --
ตรงกับที่ `COO-DECISION 20260904_1149` สั่ง ไม่ใช่แค่ทางลัดที่ปลอดภัยแต่แคบกว่าที่ขอ

### nonclaim (แทนที่ nonclaim ก่อนหน้าในหัวข้อ D1 -- ข้ออื่นยืนเดิม)

- D1 ยังไม่ปิด จริง -- บล็อกด้วยการตัดสินใจเรื่องความเสี่ยง/เวลา ไม่ใช่ `CORE-REQUEST-GM-054`
  อย่างเดียวอีกต่อไป (รั้วที่สามไม่ต้องรอ GM-054 ด้านสถาปัตยกรรม แต่ยังรอ GM-054 ด้านการทำงานจริง
  บนบูตจริง เพราะ `live_current_scene` ปฏิเสธเสมอจนกว่าฮุกจะลง)
- ใบ RE ส่งก่อนเห็นคำสั่งลำดับของ `1149` -- ลำดับคลาด เนื้อหาไม่ผิด ไม่ถอน
- ยังไม่ส่งจดหมายเสนอ LANE-B ปรับ Door B -- ทำเป็นงานถัดไปถ้ามีรอบว่าง ไม่ใช่ blocker

## รอบ `zjbjys` (2026-09-04T13:16+07:00) — D1 ปิด (x=9 เป็นกลุ่มแหล่งที่สามจริง) + โครงสารบัญ GMUI (P-3)

`COO-DECISION 20260904_1149` ข้อ 1 (D1) และ `COO-DECISION 20260904_0245` ข้อ 1 (สารบัญ GMUI)

### D1 — ปิดแล้ว
`split_sources`/`login_scoped_sources` คืน **สามกลุ่ม** `(named, login_byte, current_scene)`
ค่าคงที่ใหม่ `CURRENT_SCENE_SOURCED_ROWS = {9}` · `live_full_block_values` อ่านค่าที่ส่งของ x=9 จาก
`live_current_scene` เท่านั้น · ไบต์ล็อกอินของ x=9 ยังถูกดึงอยู่ แต่ไปอยู่ใน `fence_basis` ไม่เข้า
บล็อกที่คืนอีกเลย (รั้วยังต้องใช้เทียบ ตาม `1149` ข้อ 3 ที่สั่งคงรั้วไว้จนกว่า GT (b'') จะตอบ)

🔴 **ไม่มีไบต์บนสายเปลี่ยนแม้แต่ไบต์เดียว** และห้ามรายงานว่า option (b) ใช้ได้กับผู้เล่นที่เดินข้ามฉากแล้ว
— รั้วยังปฏิเสธทุกกรณีที่ฉากปัจจุบันไม่ตรงไบต์ล็อกอิน นี่คือ **refactor ของการกำหนดแหล่งค่า**
ไม่ใช่การเปลี่ยนพฤติกรรม

**`CORE-REQUEST-GM-054` ยังไม่ลง main** (ตรวจแล้วรอบนี้: `lane_hooks` ไม่มี
`current_session_scene_id`) — แต่มันไม่เคยเป็นตัวบล็อกของการ re-route นี้: โค้ดเดิมก็เรียก
`live_current_scene` ทุกครั้งที่บล็อกมี x=9 อยู่แล้ว บูตจริงจึงปฏิเสธทั้งบล็อกเหมือนกันทั้งก่อนและหลัง
สิ่งที่ GM-054 เป็นประตูให้คือ **การปลดรั้ว** ไม่ใช่การทำให้ routing ตรงกับคำตัดสิน

### pf-adversary รอบนี้ — สั่งต้นรอบ ผลกลับมาก่อน push แก้ครบในรอบเดียวกัน
- **D1 [MEASURED, CRITICAL]** มิวแทนต์ option (a) (`combined[x] = fence_basis[x]`) **ผ่านชุดเต็ม**
  ทั้งชุด เพราะระหว่างที่รั้วยังอยู่ ค่าจากสองแหล่ง**ถูกบังคับให้เท่ากัน** การ์ดใหม่ที่เขียนไว้ก่อนหน้า
  ยืนยันแค่ "เรียกแล้ว" กับ "เท่ากัน" ซึ่งเป็นจริงทั้งคู่ใต้มิวแทนต์ ⇒ เพิ่มการ์ด
  `test_D1_the_shipped_value_is_the_object_live_current_scene_returned` ที่ยืนยันด้วย **object identity**
  (`int` subclass ที่เท่ากับไบต์ล็อกอิน) — **ยืนยันแล้วว่ามิวแทนต์แดงจริง** และตัดประโยค
  "[MEASURED] ชุดเต็มเขียว" ในซอร์สทิ้ง เพราะชุดเต็มเขียวไม่ใช่หลักฐานของ D1
- **D2 [MEASURED, HIGH]** assert สองตัวเป็น set-union ไม่ใช่ partition มองไม่เห็นแถวที่อยู่สองกลุ่ม ·
  `split_sources` ลบ `CURRENT_SCENE_SOURCED_ROWS` ออกจากกลุ่ม named ด้วยแล้ว + assert นับ partition จริง ·
  ประโยคเดิม "x=9 stays in `LOGIN_SOURCED_ROWS` for that reason" **เท็จทั้งสองครึ่ง** ขีดฆ่าและเขียนเหตุผลจริง
- **D3 [MEASURED, MEDIUM]** รั้วถูกเปลี่ยนไปอิง `if scene_rows:` (ธงของ router) แทน
  `SELECTOR_ROW_X in combined` (เนื้อหาของบล็อก) — ล้าง `CURRENT_SCENE_SOURCED_ROWS` แล้ว x=9 หลุดไปกลุ่ม
  ล็อกอินและ**ส่ง selector เก่าออกโดยไม่มีบรรทัดคอนโซล** ซึ่งบน Door B เป็นรั้วเดียวที่ x=9 มี ·
  แก้ให้ยึด `SELECTOR_ROW_X` เป็นหลัก + post-condition assert
- **D4 [MEASURED, MEDIUM-LOW]** `rows` ที่มี x=9 ซ้ำทำให้ `.pop()` โยน `KeyError` เปล่า (ไม่ใช่
  `AttrWireError`) หลุดออกนอกสัญญาของฟังก์ชันและนอก handler ของ `mob_hit_frame` · แก้ด้วย
  `dict.fromkeys` + `wanted` เป็น sorted-unique (ซึ่งปิด D7 เรื่องลำดับ `absent=` ไปด้วย)
- **D5** ขีดฆ่าประโยค "REPLACES it with the current scene" ที่ไม่จริงอีกต่อไป
- **D6** `src/pirateforce_foundation/mob_hit_frame.py:73` เป็นเท็จแล้ว (**เขตของ LANE-B ไม่แตะ**)
  ส่งจดหมายแจ้งแทน
- **D8** ย่อหน้า "D1 ยังไม่ปิด" ข้างบนขีดฆ่าแล้ว
ทุกข้อมีการ์ดเทสของตัวเอง (`test_D1_...`/`test_D2_...`/`test_D3_...`/`test_D4_...`)

### สารบัญ GMUI (P-3) — `gm/gmui_catalog.py` + `gm/data/gm_tool_log_types.tsv`
...
3. `BUTTONS` — **ว่างโดยเจตนา** · `total_is_unknown()` คืน True ⇒ ห้ามรอบไหนเขียน "ปุ่ม x/y ทำงานแล้ว"
   🔴 `assert_backed()` ปฏิเสธแถวที่อ้างว่าเซิร์ฟเวอร์ตอบปุ่มได้โดยไม่ระบุ handler ที่มีอยู่จริง
   (เช็คทุกแถวตอน import) — นี่คือเหตุผลที่มันเป็นโค้ดไม่ใช่ตารางในจดหมาย
ใบ RE ที่จะเติมตารางนี้: `notes_to_chief/20260904_1328_LANE-GM-RE-TICKET-gmui-three-pages-button-to-opcode-map.md`

### ค้นก่อนถอด
`external/00_SEARCH_HERE_FIRST.md` **ค้นแล้ว: ไม่เจอ** สารบัญ widget ของ GMUI ·
`gamedata/00_SEARCH_HERE_FIRST.md` **ค้นแล้ว: เจอบางส่วน** (`TEXTDATA_TH__GMTOOL` 97 แถว ·
`TEXTDATA_TH__UI_MESSAGE` มี `GMUI` แถวเดียว ไม่มีรายชื่อปุ่ม)
...
### แก้ตามผล pf-adversary ในรอบเดียวกัน (`q3cde9` · PR `#745` commit ที่สอง)

pf-adversary คืนผลหลังเปิด PR พบสองข้อระดับ CRITICAL ที่วัดจริงบน store/lifecycle/router ตัวจริง
แก้ในรอบเดิมทั้งหมด ไม่เลื่อน:

- 🔴 **D1** `FoundationSession.checkpoint` เขียน **แถวในหน่วยความจำ** ด้วย
  (`self.selected = replace(self.selected, position=position)`) การเขียนที่จังหวะประกอบเฟรม
  จึงไปตั้ง `selected.position.scene_id` เป็นฉากปลายทาง **ก่อน** ที่ไคลเอนต์จะรายงาน
  ⇒ `_gm_warp_resync_selected_scene` คืนค่าเร็ว (มันเช็ค `target.scene_id == selected.position.scene_id`)
  ⇒ census ของฉากปลายทางไม่ถูกประกอบ · `last_target_pos` ไม่ถูกล้าง · membership ของมอนไม่รีเซ็ต
  · `scene_label_is_server_guess` ไม่ถูกตั้ง = KA1A-ROOTCAUSE กับ `GT-172` F-1 กลับมาพร้อมกัน
  **แก้:** โมดูลถ่ายรูป `foundation.selected` ก่อนเขียน แล้ว **คืนกลับ** หลังเขียน (อ่านกลับยืนยันด้วย)
  — แถวถาวรขยับ แถวในหน่วยความจำไม่ขยับ ตรงตามที่ `runtime.py` เขียนกำกับไว้เองว่า "SCENE_ID ONLY"
- 🔴 **D2** สาขา `/warp <n> <x> <y>` — วัดจริง: `/warp 126 <x> <y>` เขียน `scene_id=126`
  (พิน `login_entry_allowed=False`) แล้ว **ล็อกอินครั้งถัดไปถูกปฏิเสธ** `scene_not_allowed_at_login`
  และมีแต่ล็อกอินเท่านั้นที่เขียนแถวทับได้ = ตัวละครกลับเข้าเกมไม่ได้ถาวร · R306 ยังวัดว่าเฟรมรูปนี้
  ทำให้ไคลเอนต์ปิดตัวเอง จึงไม่มี TargetPos มาแก้แถวให้
  **แก้สองชั้น:** (1) **ถอนการต่อสาขานั้นทั้งหมด** — `COO 1452` ข้อ 4 สั่งห้ามแตะไว้แล้ว
  และเหตุผลเดิมที่อ้างว่า "สาขานั้นปิดอยู่" ผิด (`1744` ข้อ 3 ปิดเฉพาะเส้น ForcePos)
  (2) โมดูล **ปฏิเสธการเขียนลงฉากที่ล็อกอินครั้งถัดไปจะไม่รับ** (`login_would_accept`, fail-closed)
  — การเขียนใบนี้มีอยู่เพื่อล็อกอินครั้งถัดไป การเขียนแถวที่ล็อกอินจะปฏิเสธจึงแย่กว่าไม่เขียน
- **D3/D4** เป็นผลพวงของ D1 (`GM_WARP_POSITION_CONFIRMED` ถูกกลบ · เหตุการณ์
  `no_durable_position_write` โกหกบนเฟรมที่เขียนจริง · `client_confirmed_scene` รับค่าเดาของเซิร์ฟเวอร์)
  ปิดไปพร้อมกับตัวแก้ D1
- **D5** อ่านกลับเทียบแค่ `scene_id` ⇒ วาปฉากเดิมผ่านเกณฑ์ตั้งแต่ก่อนเขียน
  **แก้:** เทียบทั้งแถว `(scene_id, scene_seq, x, y, z)` และพิมพ์ token จาก **แถวที่อ่านกลับมา**
- **D6** ไฟล์เทสเดิมไม่มี DB และไม่มีเฟรมเลย · มิวแทนต์หลักของมันยืนยันค่าคงที่ของ fixture
  (แดงไม่ได้ไม่ว่าจะแก้โค้ดผลิตอย่างไร) **แก้:** เขียนใหม่บน `SQLiteStore`/`CharacterLifecycle`/
  `FoundationSession`/สาขาผลิตจริง · วัดมิวแทนต์สี่ตัวแล้วแดงทุกตัว (ไม่เรียกตัวเขียน · ไม่คืน `selected`
  · อ่านกลับแค่ `scene_id` · ถอดเกตล็อกอิน)
- **D7** ถอน import ที่ไม่ได้ใช้ · แก้ประโยคในดอกสตริงที่โค้ดไม่รองรับ (ขีดฆ่า ไม่ลบ)

**D8 ยังเปิดอยู่ (ไม่ได้ปิดในรอบนี้)**: (ก) วาปครั้งแรกของล็อกอินใน dispatch เดียวกัน
(ข) หน้าต่างระหว่างเขียนแถวกับที่ซ็อกเก็ตส่งไบต์จริง — บันทึกใน backlog ของไฟล์รอบ

## รอบ ydlvtt (2026-09-04T17:54+07:00) — บรรทัดคอนโซลของ "เขียนพลาด"

`COO-DECISION 20260904_1646` ข้อ 2 (ตอบใบ `1620`/`1652` ของรอบ `q3cde9` ด้านบน): `#745`
ขึ้น `main` แล้วมี `GM_WARP_SCENE_PERSISTED` เมื่อเขียนสำเร็จ แต่ไม่มีบรรทัดคอนโซลคู่กันเมื่อเขียนไม่สำเร็จ
— ผู้เทสที่นั่งจออ่าน stderr ไม่ได้อ่าน `session.events` แยก "เขียนแล้ว" กับ "เขียนพลาดเงียบ" ไม่ออก
และ `GT-172` F-3 จะปิดไม่ได้ถ้าแยกไม่ออก

**ใหม่:** `FAIL_CONSOLE_TOKEN = "GM_WARP_SCENE_PERSIST_FAILED"` ใน `gm/warp_scene_persist.py`
พิมพ์ทาง stderr เป็น `GM_WARP_SCENE_PERSIST_FAILED scene=<n> reason=<เหตุ>` ผ่านเฮลเปอร์ `_fail()`
ที่ทุก `return` ของ `persist_warp_scene` หลังจุดตรวจ `isinstance(target, WarpTarget)` เรียกก่อนคืนค่า
`<เหตุ>` คือคำเดียวกับค่าที่ฟังก์ชันคืนกลับอยู่แล้ว (`OUTCOME_*`) — แหล่งเดียว ไม่ใช่คำศัพท์ชุดที่สอง
ที่ต้องคอยให้ตรงกัน · ไม่พิมพ์เมื่อ `OUTCOME_NOT_A_TARGET` (ยังไม่รู้ scene ที่จะอ้าง เพราะไม่ใช่การวาปจริง)
และไม่พิมพ์เมื่อ `OUTCOME_PERSISTED` (มี `CONSOLE_TOKEN` ของตัวเองอยู่แล้ว)

pf-adversary quota ของรอบนี้ (`COO 1428`): **ไม่ได้เรียก** — งานคือเพิ่มจุดพิมพ์คอนโซลสองแห่งใน
ฟังก์ชันเดิมที่ผ่าน pf-adversary ไปแล้วในรอบ `q3cde9` (โครงสร้าง/ประตูเขียน/การอ่านกลับไม่เปลี่ยน)
ทำรีวิวมือแทนแบบมีเป้าเจาะจง: ไล่ทุก `return` ในฟังก์ชันเทียบกับตารางในโมดูล docstring เดิม
ยืนยันว่าทุก non-`OUTCOME_PERSISTED` non-`OUTCOME_NOT_A_TARGET` มี `_fail()` นำหน้าครบ (8/8 จุด)
และมิวแทนต์ที่ลบ `_fail()` ออกจากจุดใดจุดหนึ่งจะทำเทสใหม่ 6 ตัวในหัวข้อถัดไปแดง (วัดด้วยมือ ไม่ใช้เครื่องมือ)
**เซสชันนี้ไม่มี Agent/Task tool แยกสำหรับเรียก pf-adversary จริง** — บันทึกไว้ตรง ๆ ตามกติกา
`AGENTS.md` §7 บรรทัด 105 (ข้อยกเว้นรีวิวมือใช้ได้เมื่อไม่มีเครื่องมือจริงเท่านั้น)

เทสใหม่ 6 ตัวใน `tests/test_gm_warp_scene_persist.py`: บรรทัดคอนโซลของ `OUTCOME_LOGIN_WOULD_REFUSE`
(พร้อม `reason=`) · `OUTCOME_ROW_NOT_TOUCHED` (พร้อมยืนยันว่า `GM_WARP_SCENE_PERSISTED ` ไม่ปน) ·
`OUTCOME_WRITE_REFUSED_PermissionError` (ชื่อชนิด error เท่านั้น ไม่มีข้อความ) · stderr ที่ปิดแล้ว
ไม่ทำให้ outcome เปลี่ยน · `OUTCOME_NO_SESSION_DOOR` พิมพ์ token · `OUTCOME_NOT_A_TARGET` ไม่พิมพ์อะไรเลย

### nonclaim

1. **GM ข้ามขั้นไหน**: ไม่มี — รอบนี้ไม่ใช้ GM ข้ามการทดสอบใด ๆ เป็นการเพิ่มบรรทัด log เท่านั้น
2. **ผู้เทสจะทำอะไรได้ที่เมื่อวานทำไม่ได้**: `/warp` ที่เขียนแถวไม่สำเร็จตอนนี้พิมพ์
   `GM_WARP_SCENE_PERSIST_FAILED scene=<n> reason=<เหตุ>` ทาง stderr — เมื่อวานมีแต่ `session.events`
   ที่ผู้เทสนั่งจอไม่เห็น
3. ไม่อ้างว่า `GT-172` F-3 ปิดแล้ว (นั่นเป็นงานของ chief ตามใบ attended `1452` ข้อ 5) ·
   ไม่อ้างว่าผ่าน pf-adversary จริง (รีวิวมือแทน ตามที่ประกาศไว้ข้างบน) ·
...
## รอบ 741zlx (2026-09-04T19:17+07:00) — adversary ครั้งที่ 2 ของ `#745` และ D8 ข้อ 1 วัดจริง

`COO-DECISION 20260904_1744` ข้อ 3/4 และ `20260904_1848` ข้อ 2: `#745` (ตัวแก้ CRITICAL D1/D2)
ขึ้น `main` ไปแล้วตั้งแต่ 17:24 แต่ commit ที่สอง (ตัวแก้เอง) ยังไม่เคยถูก adversary ดู
รอบนี้ปิดสองอย่าง: (1) รอบตรวจครั้งที่สองของตัวแก้ (2) วัด D8 ข้อ 1 ด้วยมิวแทนต์จริง

### D8 ข้อ 1 — วัดแล้ว "ไม่เกิด" และรู้ว่าอะไรกันไว้

คำถามของ `1744` ข้อ 4: วาปครั้งแรกของล็อกอินในดิสแพตช์เดียวกัน — บล็อก census ที่รันทีหลัง
ในเฟรมเดียวกันจะประกอบฉากปลายทางโดยใช้ `last_target_pos` ที่ยังเป็นพิกัด **ต้นทาง** หรือไม่

รอยต่อที่ยังไม่มีใครวัด: การเขียนแถวเกิด **ก่อน** (ใน `chat_command_action._warp_teleport_action_no_coords`
ตอนประกอบ action) และ resync เกิด **ทีหลัง** (ใน `runtime.dispatch` จาก label ของ action นั้น) —
`tests/test_gm_warp_scene_persist.py` วิ่งเฉพาะครึ่งแรก (ไม่แตะ dispatch)
และ `GmWarpCensusLatchClearTests` วิ่งเฉพาะครึ่งหลัง (arm ด้วย `record_warp_target` ตรง ๆ
ไม่มีการเขียนแถวจริงเลย) ไฟล์ใหม่ `tests/test_gm_warp_persist_census_anchor.py` วิ่งทั้งสองครึ่ง
ต่อกันบนคอนเนกชันจริงหนึ่งอัน (store จริง · lifecycle จริง · `FoundationSession` จริง · dispatch จริง)

**ผล**: เส้นทางที่ shipped อยู่ตอนนี้ — แถว DB ย้ายไปฉากปลายทาง · แถวในหน่วยความจำถูกคืน
⇒ resync เห็นว่าเป็นการวาปข้ามฉากจริง relabel แล้ว `last_target_pos = None` ก่อนที่ census
ในดิสแพตช์เดียวกันจะอ่านได้ · `world_census_sent` ถูกปลดล็อก ⇒ **D8 ข้อ 1 ไม่เกิด**

**มิวแทนต์ที่พิสูจน์ว่าไม่ใช่เรื่องบังเอิญของ fixture**: แทน `_restore_selected` ด้วย no-op ที่
รายงานว่าสำเร็จ (รูปเดียวกับร่างแรกที่ pf-adversary จับได้ในรอบ `q3cde9`) ของอื่นจริงหมด
⇒ แถวในหน่วยความจำชี้ปลายทาง ⇒ resync early-return แบบ "same-scene" ⇒ `last_target_pos`
ยังเป็นพิกัดต้นทาง และ latch ไม่ถูกปลด = **D8 ข้อ 1 เกิดครบรูป** ⇒ ตัวคืนค่าของ D1 คือสิ่งที่กันไว้

วัดกลับอีกทาง: ใส่มิวแทนต์ที่ตัวโค้ดผลิตจริง (`if False and not _restore_selected(...)`)
แล้วรันไฟล์ใหม่ ⇒ เทสหัวข้อหลักสองตัว **แดง** (ไม่ใช่ไฟล์ที่เขียวได้ทุกสภาพ)

### D8 ข้อ 2 — ไม่ใช่เขตของสายนี้ ส่งเป็นใบ

`CORE-REQUEST-GM-055` (`notes_to_chief/20260904_1924_LANE-GM-CORE-REQUEST-GM-055-*.md`):
เขียนแถวที่ `chat_command_action.py:3228` แต่ไบต์ออกสายที่ `pf_login_game_server_v141.py:7755`
...
## รอบ vlk8rq (2026-09-04T22:15+07:00) — พิกัดที่พิมพ์เองปิดจริงที่จุดเดียว และกู้ `#764` กลับมา

### กู้งานรอบก่อน (Addendum A)

`#764` (รอบ `2bikkx`) **ปิดโดยไม่ merge** เวลา 21:18+07 — เกตแดง เวิร์กโฟลว์ปิดใบเองเพื่อไม่ให้
ล็อกค้าง (คอมเมนต์ของ `merge-claude-pr.yml` บอกตรง ๆ ว่ากิ่งไม่หาย ให้รอบถัดไปกู้เอง)
สาเหตุแดง = สี่เทสที่แดงอยู่บน `main` ก่อนหน้าอยู่แล้ว ไม่ใช่ของ `#764` (`COO 2153` วัดซ้ำหลัง
`#763`: เหลือหนึ่ง = เทส exemption ใน `runtime.py` ของ chief เอง) ⇒ รอบนี้ cherry-pick
สองคอมมิตของ `#764` (`12d2d72` งานจริง + `6d247ec` เอกสาร) ลงกิ่งใหม่จาก `main` ปัจจุบัน
ไม่มี conflict · เนื้อ `rollback_warp_scene_on_send_failure` ไม่เปลี่ยนสักบรรทัด

### งานแรก: `COO-DECISION 20260904_2045` ข้อ 1 — พิกัดที่พิมพ์เอง ปิดจริง

**สิ่งที่ผิดก่อนหน้านี้** (finding 10 ของรอบ `741zlx` วัดแล้ว รายงานใน `1930`): การปิดเดิมเป็น
**per-branch** — `WARP_SAME_SCENE_FORCE_POS_AUTHORIZED` ปิดครึ่ง ForcePos ฉากเดียวกันเท่านั้น
`/warp 2 100 200` ข้ามฉากถูกจัดเส้นทางก่อนหน้านั้นหนึ่งกิ่ง และ **ส่งเฟรม TeleportVital 73 ไบต์
จริงพร้อมพิกัดที่ GM พิมพ์** สิ่งที่ `#745` ถอนบนเส้นทางนั้นคือ "การเขียนแถว" ไม่ใช่เฟรม

**ตัวแก้ — การ์ดเดียว ที่หัวฟังก์ชัน**:
- `warp_executor.WARP_TYPED_COORDINATES_AUTHORIZED = False` (แฟล็กตัวที่สาม แยกจากสองตัวข้าง ๆ
  เพราะมันไม่ได้เป็นเรื่องของรูปเฟรมใดเฟรมหนึ่ง แต่เป็นเรื่องของ "พิกัดที่พิมพ์เอง" ทั้งชนิด ·
  ขยายแฟล็กเดิมไม่ได้ เพราะจะปิด `/warp <n>` เปล่า ๆ ที่ R306 ผ่านห้าครั้งและเจ้าของสั่งห้ามแตะ)
- `chat_command_action._warp_action`: `if has_coordinates and not ...AUTHORIZED:` **เหนือทุกกิ่ง** —
  เหนือ `_no_coords_live_target` · `same_scene_with_basis` · composer ทั้งสองตัว ·
  `_park_warp_target` · ประตูเขียนของ `warp_scene_persist` · `_stage_action`
  ⇒ ไม่มีไบต์ · ไม่มี target ค้าง · ไม่มีแถว staged · ไม่มีการเขียน DB
- คำใหม่: `EVENT_WARP_WITHHELD_TYPED_COORDS_CLOSED` /
  `OUTCOME_WARP_WITHHELD_TYPED_COORDS_CLOSED` (`withheld_typed_coordinates`) + ประโยค blocker
  ของตัวเองใน `NO_BYTES_BLOCKERS` — ชื่อของตัวเอง ไม่ใช้ซ้ำกับคำของการ์ด ForcePos เพราะ
  ผู้เทสอ่านบรรทัดคอนโซลเพื่อรู้ว่า "การ์ดตัวไหน" หยุดคำสั่ง แล้วเดินไปคนละใบ

**เทส** `tests/test_gm_warp_typed_coordinates_closed.py` (9 ตัว): สามรูปที่ adversary ใช้
(ฉากเดียวกัน · ข้ามฉากสด · ข้ามฉากตอนแฟล็ก live ปิด = ตกไป staging) แต่ละรูปตรวจ
**สี่อย่างที่ต้องไม่เหลือ** ไม่ใช่แค่ "ไม่มีเฟรม" (เพราะ "ไม่มีเฟรม" อย่างเดียวคือสิ่งที่ทำให้
finding 10 ซ่อนอยู่ได้หนึ่งรอบ) · มิวแทนต์เขียนเป็นเทส: พลิกแฟล็กเป็น True แล้วรูปที่สอง
compose เฟรมจริงกลับมาทันที ⇒ ถ้าลบการ์ดทิ้ง คลาสข้างบนแดง ไม่ใช่เขียวด้วยเหตุผลอื่น
· `/warp <n>` เปล่ายังคงส่งได้ ปักไว้เป็นเทสของตัวเอง

**เทสไฟล์เดิมที่ต้องแก้**: หกไฟล์ใช้ `/warp <n> <x> <y>` เป็นตัวอย่าง "คำสั่งที่ประกอบเฟรมได้"
⇒ เปิดการ์ดไว้ใน `setUp` พร้อมคอมเมนต์ว่า **คำตอบของบูตจริงไม่ได้ถูกยืนยันในไฟล์นั้น** และอยู่ที่ไหน
· `test_gm_chat_command_action.py`: `open_the_version_gate`/`close_the_version_gate` กลายเป็นสามเกต
· คลาส `SameSceneForcePosClosedTests` เดิมอ้างว่าเป็น "คำตอบของบูตจริง" — ขีดฆ่าและย้ายหน้าที่นั้น
ไปคลาสใหม่ `TypedCoordinatesClosedTests` ส่วนคลาสเดิมเหลือหน้าที่ยืนยันการ์ดชั้นที่สอง

### งานที่สอง: `ADVERSARY_PENDING #745-R2` ข้อ 5 (`COO 2045` ข้อ 4)

`warp_scene_persist.login_would_accept` เรียก `world_scene_travel.destination` โดยไม่ส่ง registry
⇒ **อ่านไฟล์ registry จากดิสก์ใหม่ทุกครั้ง ไม่มีแคช** โมดูลนี้มีหน้าที่ "ทำนายว่าล็อกอินครั้งหน้า
จะยอมรับแถวนี้ไหม" แต่ไปถามไฟล์ที่เปลี่ยนใต้โปรเซสได้ ⇒ คำทำนายกับล็อกอินที่ทำนายไม่ตรงกันได้
และทิศที่แย่คือทิศที่ "เขียนแถวที่ล็อกอินจริงปฏิเสธ" = อาการ brick ที่ docstring ของโมดูลเตือนไว้เอง

**ตัวแก้**: `_login_registry_snapshot()` อ่านครั้งเดียวแล้วถือไว้ (`reset_login_registry_snapshot_
for_tests()` สำหรับชุดเทส ทรงเดียวกับ `dispatch.reset_rate_limit_state_for_tests`)
· fail-closed และ **จำว่าล้ม**: registry อ่านไม่ได้ = ปฏิเสธทุกฉาก และไม่ลองใหม่ในการวาปถัดไป
(การลองใหม่คือการเอาการอ่านดิสก์กลับขึ้นเส้นทางร้อนที่ตัวแก้นี้เกิดมาเพื่อเอาออก)
· เทสใหม่ 3 ตัว: อ่านดิสก์ครั้งเดียวจากห้าคำถาม · registry ที่เปลี่ยนกลางรัน ไม่เปลี่ยนคำตอบ ·
registry ที่อ่านไม่ได้ ปฏิเสธทุกฉากและไม่ย้อนไปอ่านซ้ำ
🔴 **ยังไม่แก้**: ข้อ 6 (`REFUSED_NO_PINNED_SPAWN` ไม่ถูก mirror) และข้อ 7 (`compose_refused_*`
เป็นคำที่ไม่มี input ใดไปถึง) — ยกยอดไปรอบถัดไป

### nonclaim

- ไม่มีอะไรในรอบนี้พิสูจน์ว่าไคลเอนต์เห็นอะไร: ทั้งหมดเป็นข้อเท็จจริงระดับโมดูล/เทส
- `/warp <n> <x> <y>` **ไม่ได้ถูกซ่อม** มันถูก **ปิด** — สิ่งที่จะเปิดคืนคือผล RE ที่ diff
  เฟรมพร้อมพิกัดกับ capture จริง ไม่ใช่รอบที่พลิกแฟล็ก
- ไม่มีบัญชีใดได้หรือเสียสถานะ GM ในรอบนี้ · ไม่มีขั้นตอนไหนถูกข้ามด้วยสถานะ GM
- `#764` ที่กู้กลับมา ไม่ได้เปลี่ยนสถานะ `CORE-REQUEST-GM-055` — จุดเสียบยังเป็นของ chief

### ผล pf-adversary ของรอบ `vlk8rq` — 9 ข้อ แก้ 7 บันทึก 2

เรียกจริง (มี Agent tool ในเซสชันนี้) · เอเจนต์ทำงานใน worktree แยก ไม่แตะกิ่งของรอบ

**1 (MAJOR · วัดแล้ว)** ไฟล์ที่เจ็ดที่ใช้วาปมีพิกัดเป็น "ทางเดิน" ถูกมองข้าม:
`test_gm_standalone_map_is_not_chat_writable.py` — สี่เทสของ
`ChatCommandsCannotWriteTheStandaloneMapTests` เคยเดินถึง composer ข้ามฉากจริง หลังการ์ดใหม่
ไม่เดินถึงเลย ⇒ ไฟล์ที่มีไว้พิสูจน์ว่า "ไม่มีคำสั่ง GM ไหนเขียนแมพ standalone" กลายเป็นเขียว
เพราะไม่ได้เดินไปถึงประตู (ปลูก `open(...,'w')` หลัง composer แล้ว 27 เทสยังเขียว)
ตัวยาม anti-vacuity ของไฟล์เองมองไม่เห็น เพราะมัน grep เฉพาะ `EVENT_REFUSED_PREFIX`
ส่วนคำใหม่เป็น `withheld_` **แก้**: เปิดการ์ดใน `setUp` ของไฟล์นั้น ด้วยเหตุผลเดียวกับที่มัน
patch `FORCE_POS_VITAL_VERSION_CONFIRMED` อยู่แล้ว + ขีดฆ่าคอมเมนต์ตารางที่กลายเป็นเท็จ

**2 (MAJOR · วัดแล้ว)** เฮลเปอร์ "สี่อย่างที่ต้องไม่เหลือ" ของไฟล์เทสใหม่ **มองเห็นแค่สอง**:
มันอ่าน target ที่ park กับแถวถาวรจาก event list แต่ `EVENT_WARP_TARGET_NOT_RECORDED`
ยิงเฉพาะตอน park **ล้ม** (docstring ของ `_park_warp_target` เขียนไว้เองว่า "ไม่มี event ตอนสำเร็จ")
และ prefix ของ persist ยิงเฉพาะกิ่งไม่มีพิกัด ⇒ target ที่ถูก park จริงผ่านการตรวจ "ไม่ park อะไร"
**แก้**: อ่านจากตัวมันเอง `getattr(session, "gm_last_warp_target", None)` (ตัววัดเดียวกับที่
`SameSceneForcePosClosedTests` ใช้อยู่แล้ว)

**3 (MAJOR · วัดแล้ว)** snapshot ของข้อ 5 เป็น **first-use ไม่ใช่ boot** และสองประโยคที่รับน้ำหนัก
ใน docstring เป็นเท็จ: ล็อกอินจริงใช้ **ออบเจกต์** `scene_entry_registry` ที่ `runtime.py:706`
โหลดตอนบูต ไม่ใช่ไฟล์ที่โมดูลนี้อ่านเองรอบสอง ⇒ สองคำตอบต่างกัน **เฉพาะตอนไฟล์ถูกแก้**
ซึ่งคือกรณีเดียวที่ประโยคนั้นอ้างว่าไม่เกิด **แก้**: ขีดฆ่าประโยคทั้งสองในรอบที่เขียนมันเอง
+ เปิด `CORE-REQUEST-GM-056` ขอ `runtime.py` ส่ง registry ของบูตมาให้ (เขต chief)
🔴 **ข้อ 5 ของ `#745-R2` จึงยังไม่ปิด** — การอ่านถูก "ย้าย" ไม่ได้ "คืนดี" กับล็อกอิน

**4 (MEDIUM · วัดแล้ว)** แฟล็ก `TAKEN` ถูกตั้ง **ก่อน** โหลด และไม่มีล็อก ⇒ คอนเนกชันที่สอง
ที่วาประหว่างอ่านเห็น `TAKEN=True, SNAPSHOT=None` ได้ `login_would_refuse` และ **เสียแถวถาวร**
(จำลองด้วย loader หน่วง 0.3 วิ: `{'B': False, 'A': True}`) — การอ่านต่อครั้งแบบเดิมทำแบบนี้ไม่ได้
**แก้**: `threading.Lock` + ตั้ง `TAKEN` **หลัง** โหลดเสร็จ ในล็อก + เทสสองเธรดจริง

**5 (MEDIUM)** การอ่านล้มครั้งเดียวล็อกทั้งโปรเซส แล้วรายงานด้วยคำ `login_would_refuse`
ที่โทษ "นโยบายของฉาก" ทั้งที่เหตุคือ "อ่านไฟล์ไม่ได้" = รูปเดียวกับ finding 2 ของรอบก่อน
ในโมดูลนี้เอง **แก้**: คำใหม่ `OUTCOME_LOGIN_REGISTRY_UNREADABLE` + บรรทัดคอนโซลของตัวเอง + เทส

**6 (MEDIUM)** 31 เทสในหกไฟล์ผ่านใต้แฟล็กที่ชิป `False` (ถอด patch ออกแล้วแดง 31 ตัว:
`queued_confirm_arming` 9/18 · `audit_outcome` 7 · `no_bytes_line` 8 · `frame_tail` 3 ·
`dispatch_wiring` 2 · `typo_refused_notice` 2) — ทางเลือกที่ดีกว่าคือเปลี่ยน fixture เป็น
`/warp <n>` เปล่า ซึ่งยังเดินเส้นทางที่ชิปจริง 🔴 **ไม่แก้รอบนี้ = หนี้ ยกไปรอบถัดไป**
คอมเมนต์ในทั้งหกไฟล์บอกตรง ๆ แล้วว่าไฟล์นั้นไม่ได้ยืนยันคำตอบของบูตจริง และคำตอบจริงอยู่ที่ไหน

**7 (LOW-MEDIUM)** "พิมพ์บรรทัดคอนโซล" เป็นหนึ่งในสามสิ่งที่ `2045` ข้อ 1 สั่ง แต่ไม่มีเทสไหนปัก
(ทุกเทสจับคอนโซลแล้วทิ้ง) **แก้**: เฮลเปอร์รับ `console` แล้วยืนยันว่าคำ withheld อยู่ในบรรทัดจริง

**8 (LOW)** รูปที่ไม่มีตำแหน่งอ่านได้ ถูกปฏิเสธด้วยคำ `refused_warp_no_current_position`
เหนือการ์ดใหม่ ⇒ คำว่า "ANY" ใน docstring มีข้อยกเว้นหนึ่ง **แก้**: เขียนข้อยกเว้นลงไปตรง ๆ
(ศูนย์ไบต์เหมือนกัน แต่ผู้เทส grep คอนโซลเพื่อเลือกใบ)

**9 (LOW)** การ์ดอยู่ที่ router เท่านั้น composer ไม่มีการ์ดชั้นสอง 🔴 **ไม่แก้รอบนี้**:
วันนี้ `_warp_action` เป็นผู้เรียกเดียวใน `src/` (ตรวจแล้ว: `runtime.py:7825` และ
`lane_hooks/lane_gm_chat_command.py` เข้าทาง `make_gm_chat_command_action` ทั้งคู่)
= กันอนาคต ไม่ใช่รูที่เปิดอยู่

**สิ่งที่ adversary ลองแล้วไม่พัง** (บันทึกไว้เพราะมันคือส่วนที่ปักแน่นจริง): มิวแทนต์ถอดไส้การ์ด
...
### pf-adversary รอบ `hv8ets` — คืนผลหลังผมเปิด PR ไปแล้ว หกข้อ แก้สี่ ส่งต่อสอง

🔴 **บทเรียนของรอบนี้ ไม่ใช่เรื่องโค้ด**: agent เงียบ ~22 นาทีจนผมสรุปว่า "ไม่คืนผล" เขียนไฟล์รอบ
เป็น `ADVERSARY_MANUAL` และเปิด PR เซิร์ฟเวอร์ไปแล้ว **แล้วมันจึงคืนผล** และในผลนั้นมีข้อที่ทำให้
เซิร์ฟเวอร์บูตไม่ขึ้น ⇒ **รอบหลังห้ามสรุปว่า agent ตายจากความเงียบ** ให้ push แล้วรอผลก่อนปิดใบเสมอ
· ผลถูกเอามาแก้ครบก่อน push จริง และชุดเต็มถูกรันใหม่ (สองครั้งในรอบ พร้อมเหตุผลในไฟล์รอบ)

**D2 [HIGH, วัดแล้ว] ใบสั่ง "หนึ่งบรรทัด" ของผมถึง chief คอมไพล์ไม่ผ่าน** — `runtime.py` ไม่ได้
import `warp_scene_persist` (มีจาก `.gm` ห้าโมดูล ไม่มีตัวนี้) ⇒ `NameError` ที่ `make_state_class`
ครั้งแรก = `app.py:834` = เซิร์ฟเวอร์ไม่บูต · **ข้อที่แย่ที่สุดของรอบ** เพราะโมดูลนี้ประกาศ NEVER RAISES
ด้วยเหตุผลว่า "exception ที่นี่คิดกับทั้งเซิร์ฟเวอร์" แล้วใบสั่งของผมเองพาไปที่นั่น
**แก้**: ใบ `0121` แก้เป็นสองบรรทัด (import + call) เวลา 01:45 · docstring เขียน IT IS TWO LINES

**D4 [MEDIUM, วัดแล้ว] ตัวแก้ของผมเองแก้ผิดทาง** — probe home **กิน** iterator ⇒
`SceneRegistry(destinations=<generator>)` ผ่าน probe · รายงาน `INSTALLED` · แล้ว `login_would_accept`
ตอบ `False` ทุกฉากตลอดชีพโปรเซส โดยตัวบอกเดียวคือคำว่า `unknown` = **อาการที่ probe อ้างว่าปิด
รายงานเป็นความสำเร็จ** · การกัน `len` ไม่ให้ raise แก้แค่อาการ
**แก้จริง**: `isinstance(destinations, abc.Sized)` **ก่อน** probe แล้ว **ปฏิเสธ** ด้วยคำ
`boot_registry_refused_unusable_unsized_rows` · tuple/list/set/frozenset ยังผ่านครบ

**D1 [HIGH] ไม่มีสัญญาณไหนแยกการต่อสายที่ถูกออกจากที่ผิดได้** — `use_boot_scene_registry(load_scene_registry())`
คือการอ่านดิสก์ **ครั้งที่สาม** ทิ้งข้อบกพร่องไว้ทั้งดวง แต่พิมพ์บรรทัดคอนโซลเหมือนกันทุกตัวอักษร
และเทสใหม่ทั้งสองคลาสยังเขียว — ทุกสัญญาณที่รอบนี้ชิปเป็นฟังก์ชันของ **สถานะ** ไม่ใช่ **identity**
**แก้**: `login_registry_is(candidate) -> bool` (identity · `False` เสมอเมื่อยังไม่มีอะไรติดตั้ง
แม้ถาม `None`) **⇒ เกณฑ์รับของเทส wiring รอบหน้าคือ `login_registry_is(scene_entry_registry)`
ห้ามเกรดด้วยโทเคน/คำ source/จำนวนฉาก/`replaced=` เด็ดขาด**

**D5 [LOW-MEDIUM] docstring อวดอ้างเกินจริง** — "ONE registry object in the process" **เท็จ**
`lifecycle.py:121` เป็นการอ่านครั้งที่สาม และ**มันคือตัวที่เกตการเขียนถาวรจริง**
(`lifecycle.py:201`/`:252`) ⇒ ขีดฆ่าประโยคเดิม เขียนขอบเขตที่ถูกแทน ("การทำนายล็อกอิน" ไม่ใช่
"ทั้งโปรเซส") · **หนี้ที่ยังไม่มีเจ้าของใบ: การอ่านที่สามใน `lifecycle`**

**D3 [MEDIUM-HIGH, วัดแล้ว] ส่งต่อ chief** — `make_state_class` ถูกเรียก 227 ครั้ง แทบทั้งหมดในเทส
⇒ เมื่อเสียบแล้ว ทุกครั้งจะติดตั้ง process-global โดยไม่มี teardown · วัดจริง: registry ที่ปั้นใน
`mock.patch` ค้างอยู่หลัง patch ออก ⇒ เทสหลังจากนั้นได้คำตอบจากทะเบียนที่ไม่เคยมีในไฟล์ไหน
ชุดเต็มเขียวเพราะไม่มีใครถามฉาก 278 ทีหลัง = **"เขียวเพราะยังไปไม่ถึง"** ⇒ ขอ chief เคาะสามทาง

**D6 [PROPOSED] ติดตั้งซ้ำชนะเงียบ ๆ** — ตัวบอกใบเดียวคือ `replaced=boot` ⇒ ปักเทสไว้แล้ว

**ที่ adversary ทำให้พังไม่ได้**: deadlock/torn read (stress 6 เทรด 6 วิ · 0 exception) ·
...
## รอบ `ff30oi` (2026-09-05 04:11 +07:00) — R313 ปิดบั๊กแชท 2-vital · double-warp วัดแล้วและเจอหนี้

ที่มาของงาน: `COO-DECISION 20260905_0345` (ตอบ `20260905_0301`) ข้อ 4 = P-3 คือคิวหลัก ·
ข้อ 3 = เทส double-warp เป็นงานสำรอง 1 · และ `NOW.md` ข้อ 4 บรรทัด "บั๊ก GM แชท 2-vital
ในเฟรมเดียวถูกทิ้ง (R313 §3) = LANE-GM"

### 1 บั๊กแชท 2-vital (R313 §3) — ปิดแล้วฝั่งเซิร์ฟเวอร์

สิ่งที่วัดได้จริง (ไม่ใช่สมมุติ): R313 02:01:58 ไคลเอนต์ส่งเฟรมเดียว 171 ไบต์ = vital `0xAC52`
(ข้อความ `/warp 126`) **ต่อท้ายด้วย** vital `0x0F01` `UserSetting_UpdateServerSettingVital`
คอนโซลพิมพ์ `LANE_GM_CHAT_TAIL reason=tail_unknown_vital_id tail_vitals=0 ids=none
chat_bytes=none payload_bytes=151` แล้วคำสั่ง**หายไปเฉย ๆ** ไม่มี `GM_CHAT_STAGED` ไม่มีแถว
Panya พิมพ์แล้วเงียบ ต้องพิมพ์ซ้ำอีก 2 นาทีครึ่งจึงติด

เหตุ: `0x0F01` ไม่มีแถวใน `vital_walk.body_length_table` ⇒ `_walk_tail` คืน
`TAIL_UNKNOWN_VITAL_ID` แล้วทิ้ง body ทั้งก้อน

แก้: outcome ใหม่ `TAIL_UNDECLARED_BODY` — เมื่อ `decode_local_talk_payload` (ตัวถอดเข้ม
ตัวเดิม ไม่แก้) รับ prefix ที่เสนอไปแล้วครบทุกไบต์ **และ** tail ขึ้นต้นด้วยหัว nested vital ที่อ่านได้
ครบ (`u16 tag 0x12` + `u8 tag 0x0B`) แต่ id นั้นไม่มีความยาว body ประกาศไว้ ⇒ **เก็บ body
ไว้ คำสั่งเดิน** และไม่บริโภค tail เหมือนเดิม

เหตุผลที่ไม่ใช่การเดาขอบเขต: ขอบเขตมีผู้สมัครเดียวเสมอ — `_chat_body_boundary` คำนวณจากสอง
ฟิลด์ความยาวที่**หัว** payload เท่านั้น แล้วตัวถอดเข้มต้องรับไบต์ชุดนั้นพอดี การเดิน tail ไม่เคยเป็น
ตัว**หา**ขอบเขต มันแค่ยืนยันซ้ำ และบนเฟรมของ R313 การยืนยันซ้ำเป็นไปไม่ได้ในหลักการ
(body ของ `0x0F01` ไม่ใช่ความยาวคงที่ ไม่มีแถวในอนาคตที่ทำให้เดินจบ) การปฏิเสธตรงนั้นจึงไม่ได้
กันขอบเขตผิดออก มันทิ้งขอบเขตที่พิสูจน์แล้ว

ยังปิดอยู่เหมือนเดิม: tail ที่ห้าไบต์แรกไม่ใช่หัว nested vital ยังเป็น `TAIL_TRUNCATED` และผู้เรียก
เก็บพฤติกรรมของ main ไว้ทั้งดุ้น · ตัวตัดสินว่าใครสั่งได้ยังเป็น `handle_local_talk_chat` กับ
`gm_accounts` เหมือนเดิม อยู่ใต้จุดนี้ · `TAIL_UNKNOWN_VITAL_ID` เก็บชื่อไว้ ขีดฆ่าใน docstring
ไม่ลบ (`RetiredReasonTests` ปักว่าไม่มี code path ไหนคืนมันแล้ว)

เทส: `R313CapturedFrameTests` สร้าง payload จาก hex ของจดหมาย R313 §3 ตรง ๆ (ขยาย
`00×8` เต็ม ไม่ derive) · เช็คเลขความยาวกับ `payload_bytes=151` ที่คอนโซลจริงพิมพ์ไว้เป็นตัวตรวจ
การถอดความ · ปักว่า `0x0F01` ยังไม่มีความยาวประกาศ (ถ้าวันหนึ่ง LANE-E ประกาศ เทสจะแดง
แทนที่จะเงียบ ๆ ไปทดสอบเรื่องอื่น) · และปักว่าคนที่ไม่ใช่ GM ส่งเฟรมเดียวกันยังได้ศูนย์

### 2 double-warp (งานสำรอง 1 ตาม `0345` ข้อ 3) — วัดแล้ว และเจอหนี้จริง

`DoubleWarpTests` ยิงผ่าน router จริง store จริง เฟรมประกอบจริง สามข้อ:

- `/warp 2` สองครั้งติด: เฟรม**ไบต์เหมือนกันเป๊ะ** (composer deterministic ปลายทางเดิม) ⇒
  สองเฟรมแยกกันไม่ออกบนสาย ตัวแรกยืนยันแล้วปิดหน้าต่างของตัวที่สอง — **ไม่เสียหาย** เพราะค่าแถว
  ก็เท่ากัน undo ที่เสียไปเป็น no-op อยู่ดี · นี่คือคำตอบของ "คำถามที่ยังไม่มีคำตอบ" ที่รอบ `8u0j50`
  ทิ้งไว้
- `/warp 2` แล้ว `/warp 3`: เฟรมต่างกัน ⇒ เฟรมของตัวแรกที่มาถึงทีหลัง**ไม่**เคลียร์ park ของตัวที่สอง
  (`left_parked_other_frame`) การแทนที่ทำงานถูก
- 🔴 **หนี้ที่เจอ และแก้ในรอบเดียวกัน**: `/warp 2` (เฟรมยืนยันส่งแล้ว — ผู้เล่นอยู่ฉาก 2 บนจอจริง)
  แล้ว `/warp 3` ส่งไม่ออก ⇒ undo เดิม**ย้อนแถวไปฉาก 1** ไม่ใช่ฉาก 2 เพราะ
  `rollback_warp_scene_on_send_failure` อ่านแถวที่จะย้อนจาก `session.foundation.selected.
  position` ซึ่งคือ "ตำแหน่งล่าสุดที่**ไคลเอนต์**รายงาน" และ warp จงใจไม่อัปเดตมัน ⇒ หลัง warp
  ที่สอง มันเก่าไปสอง warp ผลคือตัวละครที่ถูกส่งไปเกาะคุกและยืนยันแล้ว จะไปโผล่ Port Royal ตอน
  ล็อกอินรอบหน้า

  แก้: `ParkedWarpSend` รับ `previous_position` · `park_warp_send` รับเป็นอาร์กิวเมนต์ที่สาม ·
  `_persist_warp_scene` คืน `previous` ออกมาเป็นตัวที่สามของ tuple (มันจับแถวนี้ไว้อยู่แล้วสำหรับ
  `verdict.undo`) · `on_game_frame_send_failed` เรียก `rollback_warp_scene` ด้วยแถวที่ park ไว้
  ถ้ามี ไม่มีก็ตกกลับไปใช้ delegate เดิมทุกประการ (ปักด้วยเทส)

  **แก้ก่อนติดสาย ไม่ใช่หลัง**: หน้าต่างนี้ยังไม่มีชีวิต เพราะยังไม่มีใครเรียก
  `on_game_frame_send_failed` จนกว่า `CORE-REQUEST-GM-057` จะลง main — ตาข่ายที่กำลังจะถูก
  กางต้องถูกก่อนตอนกาง

### nonclaim ของรอบนี้

- **ไม่มีอะไรผ่านจอในรอบนี้** · ไม่มีบัญชีใดได้/เสียสถานะ GM · ไม่มีขั้นตอนใดถูกข้ามด้วย GM ·
  ไม่ประกาศว่า M2/M3/P-2/P-3 ขยับ
- ข้อ 1 แก้ฝั่งเซิร์ฟเวอร์เท่านั้น — **ยังไม่มีใครพิมพ์ `/warp` ทับเฟรม UserSetting บนไคลเอนต์จริง
  แล้วเห็นผลบนจอ** หลักฐานชั้น client-observable ยังเป็นศูนย์ ต้องรอบ attended
- ข้อ 2 ทั้งก้อนเป็น headless: session attribute กับแถว SQLite ไม่มีซ็อกเก็ตจริง ไม่มีจอ
- ไม่ได้ตัดสินว่า body ของ `0x0F01` แปลว่าอะไร รู้แค่ว่าหัวห้าไบต์ของมันอ่านได้
- `vital_count` บนบรรทัดคอนโซลยังเป็น `unavailable` — `runtime.py` ไม่ส่ง `parsed.vital_count`
  ให้สายนี้ (เขตของ chief)
...
### `pf-adversary` รอบ `ff30oi` — เจอ 7 ข้อ แก้ทุกข้อในรอบเดียวกัน

สั่งต้นรอบพร้อมเริ่มงานตามกติกา (COO `0903_2345`) ผลกลับมาก่อน push · นี่คือครั้งที่ 1 จาก 2

- 🔴 **D-A MAJOR (MEASURED) — regression ที่ผมทำเอง** `/warp 2` และ `/warp 3` ประกอบเสร็จ
  ทั้งคู่**ก่อน**เฟรมไหนออกจากซ็อกเก็ต แล้วสายหลุด: park ตัวที่สองถือ `previous_position` = ฉาก 2
  ซึ่ง**เฟรมของมันเองก็ยังไม่ได้ยืนยันว่าส่งออก** ⇒ undo ดันแถวไป**ข้างหน้า** ค้างที่ฉาก 2 ทั้งที่
  ไคลเอนต์อยู่ฉาก 1 และไม่เคยได้รับอะไรเลย แล้ว park ถูกเคลียร์ ⇒ ความล้มเหลวของ warp ตัวแรก
  คืน `nothing_parked` แก้ไม่ได้อีก · **แย่กว่าโค้ดเดิมที่ผมมาแทน** (delegate เดิมได้ฉาก 1 ถูก)
  **แก้**: park ที่ยังอยู่ในเซลล์ = ยังไม่ยืนยัน ⇒ การแทนที่ต้อง**หอบแถวที่เก่าที่สุดต่อไป** ไม่ใช่เอา
  แถวของตัวเอง · เทสใหม่สามตัว (สอง warp · สาม warp · และ "ยืนยันแล้วต้องเริ่มรันใหม่" กันไม่ให้
  การหอบต่อไม่มีวันหมดอายุ) · มิวแทนต์ตัดการหอบต่อ = แดง 2 ตัว
- 🔴 **D-B MODERATE (MEASURED)** เกตเดิมเป็น `is not None` ⇒ แถวที่**ชนิดผิด** (ไม่ใช่
  `Position`) ลงทางใหม่แล้วได้ `nothing_to_roll_back` **และไม่ตกไป fallback** = ตาข่ายปลดตัวเอง
  เงียบ ๆ · และทางใหม่ไม่เช็ค `record.label` เลย ⇒ วินัย "หนึ่ง label" ตายไปครึ่ง
  **แก้**: เกตเป็น `label == SEND_FAILURE_WARP_ACTION_LABEL and isinstance(previous, Position)`
  ไม่ผ่าน = ใช้ delegate เดิม · เทสสองตัว · มิวแทนต์กลับไป `is not None` = แดง 2 ตัว
- 🔴 **D-C MODERATE (MEASURED)** `RetiredReasonTests` เดิม grep หา `return ChatTailSplit`
  กับชื่อคงที่ให้อยู่**บรรทัดฟิสิคัลเดียวกัน** ⇒ ผูกค่าใส่ตัวแปรแล้ว wrap call สามบรรทัด = คืนชื่อที่
  ปลดระวางได้จริงโดยเทสยังเขียว **แก้**: ปักที่ `_walk_tail.__code__.co_consts` (ทุกสะกดต้องผ่าน
  ตรงนี้ คอมเมนต์ผ่านไม่ได้) + เทสเชิงพฤติกรรมวนทุกรูป tail ที่ไฟล์นี้ยิง · มิวแทนต์เดิม = แดง
- **D-D (ปิดไปแล้วตอน commit แต่ไม่มีอะไรปัก)** การขยาย `_persist_warp_scene` เป็น 3-tuple
  ทำให้เทสสี่ตัวใน `test_gm_warp_scene_rollback.py` พังโดยที่ diff ไม่ได้แตะ **แก้เพิ่ม**:
  `PersistReturnArityTests` ปัก arity = 3 จาก AST ของฟังก์ชันเอง + ปักการ unpack ที่จุดเรียกจริง
- **D-E MINOR (MEASURED)** คำอ้าง "เฟรมต้องยังดูเหมือน chat body แล้วตามด้วย vital ทั้งสองครึ่ง"
  จริง ๆ กว้างแค่ **สองไบต์ tag** (`tail[0]==0x12`, `tail[3]==0x0B`) id และไบต์ที่เหลืออิสระหมด
  **แก้**: เขียนความกว้างจริงลง docstring และเพิ่มเทสปักว่าไบต์ขยะพันไบต์ก็ผ่าน — เพื่อไม่ให้รอบหลัง
  อ้าง tail เป็นหลักฐานเรื่องเฟรม · ตัวที่กันจริงไม่ใช่ tail แต่คือ "ขอบเขตมีผู้สมัครเดียว + ตัวถอดเข้ม"
- **D-F MINOR (MEASURED)** `0xAC52` ไม่มีความยาวประกาศเช่นกัน ⇒ เฟรมที่มีแชท**สองบรรทัด**
  ลงทาง undeclared: บรรทัดแรกเดิน บรรทัดที่สองหาย **เงียบกว่าเดิม** (ก่อนรอบนี้หายทั้งคู่แต่ GM ได้
  คำปฏิเสธ) **แก้**: outcome แยกชื่อ `TAIL_SECOND_CHAT_DROPPED` + event ของตัวเอง +
  บรรทัดคอนโซลบอก `ids=0xAC52` · **ไม่ได้อ้างว่าแก้การสูญหาย** — ตัวแก้จริงคือ "ให้ตัวอ่านแชทวน
  ทุก nested vital" ตามที่ R313 §3 ขอ ซึ่งไม่ใช่ของรอบนี้ (เข้า backlog)
- **D-G MINOR** docstring อ้างว่า "ทุก park ที่จุดเรียกของสายนี้ทำ มี `previous_position`" — **เท็จ**
  ตัวละครที่ยังไม่มีแถว `character_positions` (วาปครั้งแรกในชีวิต) `row_before_warp` คืน `None`
  แต่ยัง `PERSISTED` และยัง park **แก้**: เขียนใหม่ให้ตรง + เทสยิงรูปนั้นผ่าน route จริง

**ที่ adversary พยายามแล้วพังไม่ได้** (บันทึกไว้เพราะเป็นครึ่งที่มีค่าเท่ากัน): การถอดความ hex R313
ตรงไบต์ต่อไบต์ (เขาแกะเองจากจดหมายอีกรอบ) · ไม่มีการขยายสิทธิ์ว่าใครสั่งได้ · ขอบเขตมีผู้สมัคร
เดียวจริง เฟรมที่ตั้งใจสร้างขยับไม่ได้ · ไม่มีการเขียนคอนโซล/event แบบไร้ขอบเขต · `TAIL_TRUNCATED`
ยัง fail-closed จริง · `QUIET_REASONS` ยังถูก · `MAX_VITALS_PER_FRAME` ไม่ถูกข้าม

**คำถามที่ adversary ทิ้งไว้ และรอบนี้ยังไม่ตอบ**: เมื่อมี warp หลายใบค้างพร้อมกัน เซลล์เดียวเก็บ
...
- 🔴 **D2/D3 (ไม่มีอะไรในรีโปเปลี่ยนสถานะเมื่อ hookup ลง)** — adversary เอาบรรทัดของ chief ไปวาง
  ...
  > 🔴 **แก้ภายหลัง รอบ `j2jluj` (2026-09-05) — อย่าอ่านย่อหน้า D2/D3 ข้างบนเป็นสถานะปัจจุบัน**
  > ~~"`HookupWiringPinTests` อ่าน `runtime.py` เป็นข้อความ ... ปักคำตอบวันนี้ = ยังไม่ต่อสาย"~~
  > **ถอนทั้งประโยค** · pf-adversary รอบ `rs8uyz`/R350 พิสูจน์ว่าหมุดข้อความนั้นเป็น substring scan:
  > บรรทัดที่ถูกคอมเมนต์ก็ยังนับ ⇒ คอมเมนต์บรรทัดเรียกออกแล้วยัด lambda เปล่า = rollback ตายใน
  > production ขณะที่ชุดเทสเขียวเหมือนเดิม · ตาม `COO-DECISION 20260905_0947` ("WIRED = observed,
  > not named") และ `0948` ข้อ 2(ก) คลาสนี้ถูกเขียนใหม่ทั้งคลาสในรอบ `j2jluj` ให้ปักที่ **event**
  > `gm_warp_send_watch_install_installed` (ผู้เขียนที่เดียวคือ `_announce_install`) บน session ที่
  > ออกจากตัวสร้าง production จริง + เทส end-to-end ที่อ่านแถวในฐานข้อมูลกลับ
  > 🔴 อ่านให้ครบ: **ยามคือทั้งคลาส ไม่ใช่เทส event ใบเดียว** — event พิสูจน์ว่าตัวติดตั้ง *วิ่งจริงและ
  > ไม่ปฏิเสธ* บน object จริง แต่ไม่ได้พิสูจน์เองว่า observer ทั้งสองมีชีวิต (`refused_already_present`
  > เป็นคำตอบที่ถูกสำหรับคลาสที่ประกาศเมธอดเอง = รูป A) · ตัวที่ฆ่ามิวแทนต์ "ยัด lambda เปล่าก่อน
  > ตัวติดตั้ง" คือเทส end-to-end ที่อ่านแถว DB (pf-adversary รอบ 2 วัดแล้ว)
  > ~~"ตัวเลข 61 -> 80 ใบในไฟล์ · `HookupWiringPinTests` 2"~~ **ล้าสมัย** — วัดใหม่ทุกครั้ง อย่าอ้างต่อ
  > ~~"ที่ยังไม่ปิด: `runtime.py:1599` ยังไม่ลง · คำถาม liveness ของ `send_lock` ยังไม่มีใครตอบ"~~
  > **ปิดทั้งสองข้อแล้ว**: จุดเรียกลง main รอบ `rs8uyz`/R350 (`#806`) · คำถาม liveness วัดแล้วในรอบ
  > `j2jluj` (`SendLockLivenessTests` · ตารางตัวเลขอยู่ในดอกสตริงของ `gm/warp_send_watch.py`)
...
## รอบ `y1evqj` (2026-09-05T22:11+07:00) — P-3 ได้ตัวส่วนแล้ว: 17 แถว จากภาพหน้าจอ GT-207 ที่ commit ไว้ตั้งแต่ 2 ก.ย.

**สิ่งที่เปลี่ยน**: `gm/gmui_catalog.py` เคยประกาศว่า `BUTTONS` ต้องว่างจนกว่าจะมี RE runner ที่มี
client image มาเติม และ `total_is_unknown()` เป็น `True` ⇒ ไม่มีใครพูด "ปุ่ม x จาก y" ได้เลย
รอบนี้พบว่า**ครึ่งหนึ่งของคำถามไม่ต้องใช้ image เลย**: `pf_bridge/evidence_screens/GT207_R304_gmui_*`
คือภาพ 1656x1000 สี่ใบของ client จริงที่เปิด GMUI จริง หน้าละใบ (แท็บ 1 มีสองใบ) commit ไว้ตั้งแต่
`GT-207` ผ่านเมื่อ 2026-09-02 — ภาพถ่ายบอกไม่ได้ว่าวิดเจ็ตส่ง opcode อะไร แต่ไม่มีอะไรบอก
**จำนวน**ได้ดีกว่าภาพถ่าย

**ตัวเลข**: 7 + 5 + 5 = **17 แถวฟังก์ชัน** (แถวที่มี radio เลือกหน้าที่ ตามด้วยช่องกรอกของมัน และ
ทุกหน้ามีปุ่ม "ปฏิบัติ" หนึ่งปุ่มที่ลงมือกับแถวที่ถูกเลือก) ⇒ `progress()` = `(0, 17)`
`total_is_unknown()` = `False` แล้ว · แต่ `total_is_confirmed_on_screen()` = `False` ยังคงอยู่

**ทำไมยังไม่ confirmed**: หน้า 1 แถวห่างกัน ~40px สม่ำเสมอ **ยกเว้น**ระหว่างแถว 5 (~y=525) กับ
แถว 6 (~y=605) ที่ว่าง ~80px — กว้างพอดีหนึ่งแถว และว่างเหมือนกันทั้งสองใบของแท็บ 1 ⇒ 17 คือ
**พื้น** ที่ภาพสองใบเห็นตรงกัน ไม่ใช่**เพดาน**ที่พิสูจน์แล้ว เก็บไว้ในค่าคงที่ `PAGE_1_UNEXPLAINED_GAP`

~~**ไม่อ่านป้าย**: 15 จาก 17 แถวมี `label_status = UNREAD` · อีกสองแถวเป็น `LATIN_PARTIAL`~~ [STALE since round dl1etn -- see the dl1etn section at the end of this file: all 17 rows carry a table caption now] (ขึ้นต้นด้วย
โทเคนละติน `NPC` หน้า 1 แถว 3 และ `BUFF` หน้า 3 แถว 2 ส่วนภาษาไทยหลังจากนั้นอ่านไม่ออกแน่ชัดที่
ความละเอียดนี้) — ป้ายที่เดาคือแถวที่ guard ของโมดูลนี้มีไว้กันตั้งแต่แรก การอ่านป้ายทั้ง 17 คือ
**หนึ่ง**รอบ attended (บล็อก `ATTENDED:` อยู่ในจดหมายของรอบนี้) ไม่ใช่ 17 ใบ

**ชื่อหน้ายังไม่เปลี่ยน**: `PAGE_UNNAMED_2`/`PAGE_UNNAMED_3` คงเดิม — ภาพยืนยันว่ามีปุ่มแท็บสามปุ่มจริง
(โครงสร้าง) แต่ **ไม่ได้**ทำให้อ่านชื่อแท็บออก ⇒ เทส `test_only_one_page_name_is_committed_evidence`
ยังเขียวโดยไม่ต้องแก้ และไม่มีการอ้างชื่อที่ไม่มีหลักฐาน

**หลักฐานถูกหมุด**: sha256 ของภาพทั้งสี่ใบอยู่ใน `ROW_CENSUS_SCREENSHOTS` และ sha256 ของ census tsv
เองอยู่ใน `ROW_CENSUS_SHA256` — แก้แถวเงียบ ๆ ไม่ได้ ต้องขยับหมุดในคอมมิตเดียวกัน

**มิวแทนต์ที่รันจริงรอบนี้ (ไม่ใช่เขียนแล้วเชื่อ)**: (M2) ให้ทุกแถวอ้าง `client_frame=0x51E9` →
`test_no_censused_row_claims_an_opcode_or_a_handler` แดง · (M3) พลิก
`total_is_confirmed_on_screen()` เป็น `True` → `test_a_known_count_is_still_not_a_confirmed_one` แดง ·
(M4) ลบแถว `1/7` แล้วขยับหมุด sha ตาม → เทสนับสามใบแดง · คืนไฟล์แล้ว sha ตรงเดิมทุกไบต์
...
## รอบ `dl1etn` (2026-09-06T01:12+07:00) — ป้ายทั้ง 17 แถวของ GMUI ไม่ต้องรอจอ: มันเป็นข้อความในตารางที่บ้านนี้ commit ไว้แล้ว

รอบ `y1evqj` นับแถวได้ 17 แถวจากภาพ GT-207 แล้วเขียนว่า "ป้ายอ่านไม่ออก 15 แถว · latin บางส่วน 2 แถว"
และขอใบ attended (`GT-269`) ให้คนไปนั่งอ่านป้ายที่จอจริง **รอบนี้พบว่าไม่ต้อง** — ป้ายทั้งแผงเป็นข้อความใน
`pf_bridge/gamedata/tables/TEXTDATA_TH__UI_MESSAGE.tsv` ซึ่งอยู่ในรีโปมาตั้งแต่ต้น และเป็น `n_ID` ที่ต่อกัน
เป็นบล็อกเดียว

- `src/pirateforce_foundation/gm/data/gmui_label_block.tsv` (ใหม่) = สไลซ์ของตารางนั้น คัดลอกทีละไบต์
  (`n_ID` 1386-1413 · 1439 · 1440 · 1891-1896 = 36 แถว) หมุดด้วย `LABEL_BLOCK_SHA256` ตรวจตอน import
  แบบเดียวกับ `gm_tool_log_types.tsv` · sha ของ **ตารางต้นทางเต็มไฟล์** อยู่ที่ `LABEL_SOURCE_TABLE_SHA256`
- `GMUI_LABEL_BLOCK_ROLES` (ใหม่) = **คำกล่าวอ้างของสายนี้** ว่าแต่ละ `n_ID` อยู่ตรงไหนของแผง (ตัว tsv
  เป็นข้อความของไคลเอนต์ล้วน ไม่มีการตีความปนอยู่ในไฟล์ข้อมูล)
- `gmui_widget_census.tsv` เพิ่มคอลัมน์ `label_row_id` · `label_status` เปลี่ยนจาก `UNREAD`/`LATIN_PARTIAL`
  เป็น `TABLE_EXACT` (16 แถว) / `SCREENSHOT_ONLY` (1 แถว: หน้า 3 แถว 3)

**หลักฐานไม่ใช่ "id ติดกัน" — หลักฐานคือรอยต่อ** id ที่เรียงกันในตารางข้อความไม่ได้แปลว่ามันคือป้ายของ
หน้าต่างนี้ สิ่งที่ทำให้เชื่อได้คือ **รูปร่างของ run กับรูปร่างของแผงตรงกันทีละแถว** และตรงกันแบบที่จับคู่
อย่างอื่นไม่ได้: แถวที่ run มีสตริงตัวเลือกสองตัว = แถวที่ภาพมี radio สองปุ่ม · แถวที่ run มีคำบรรยายแกน
สามตัว (1390/1391/1392) = แถวที่ภาพมีช่องตัวเลขสามช่อง · หน่วย "นาที (0 คือถาวร)" (1411) อยู่บนแถวเดียว
ที่ภาพวาดหน่วยแบบมีวงเล็บ (หน้า 2 แถว 4) · เทส `LabelBlockTests.test_the_shapes_agree_where_the_run_says_they_must`
คือการตรวจข้อนี้ในโค้ด ไม่ใช่ในประโยค

**ช่องว่างหน้า 1 ยังไม่ปิด** `PAGE_1_GAP_CANDIDATE = 1396` เป็น **ผู้ต้องสงสัย ไม่ใช่คำตอบ** — เหตุผลที่
ยังไม่ใช่คำตอบเขียนไว้ในค่าคงที่เอง: run ของหน้า 2 ก็มีสตริงที่ไม่ถูกวาดเหมือนกัน (1403) และหน้า 2
**ไม่มีช่องว่างเลย** ⇒ สตริงที่ไม่ถูกวาดไม่จำเป็นต้องกินพื้นที่ layout ในไคลเอนต์ตัวนี้
`total_is_confirmed_on_screen()` จึงยัง `False` เหมือนเดิม

**สิ่งที่ยังต้องใช้คนที่จอ** (คือเนื้อที่เหลือของ `GT-269` หลังรอบนี้): แถวไหนถูกเลือกเป็นค่าตั้งต้น · แถวไหน
disabled · ปุ่ม "ปฏิบัติ" (1413) ลงมือกับแถวที่เลือกจริงหรือไม่ · ช่องว่างหน้า 1 คืออะไร · ป้ายหน้า 3 แถว 3
(อ่านออกบนภาพ แต่ไม่ตรงกับแถวใดในตารางที่ commit ไว้เลย)

🔴 **กับดักที่ต้องพูดครั้งเดียวให้ชัด**: ชื่อแท็บหน้า 2 (1440) **ไม่ตรงกับเนื้อหาของหน้า 2** — ชื่อพูดเรื่อง
"เลื่อน/เพิ่มระดับ" แต่วิดเจ็ตใต้มันคือ เกิดมอน · สังหารมอน · แบนสนทนาสองบล็อก · ช่องพิมพ์คำสั่งดิบ ⇒
ใช้ป้ายแถว (1399..1412) พูดว่าหน้า 2 ทำอะไร **ห้ามใช้ชื่อแท็บ** (`PAGE_2_TITLE_DOES_NOT_MATCH_ITS_CONTENT`)

`progress()` ยัง `(0, 17)` **ไม่ขยับ** — การอ่านป้ายไม่ใช่การตอบปุ่ม · ตัวเลขใหม่คือ
`labels_are_read() = (16, 17)` ซึ่งเป็นคนละตัวและเทสบังคับให้ทั้งสองอยู่ด้วยกัน

### รอบ `dl1etn` ต่อ — `pf-adversary` คืนผลก่อนปลดล็อก และหักล้างสามข้อของรอบนี้เอง (แก้ครบในคอมมิตเดียวกัน)

สั่งต้นรอบ คืนผลใน 16 นาที **ก่อน** push ⇒ ไม่ใช่ `ADVERSARY_PENDING` · ข้อที่เปลี่ยนโค้ดจริงรอบนี้:

- **D1 (หนักที่สุด — คำปฏิเสธที่เป็นเท็จ)** สายนี้เขียนลงไฟล์ข้อมูลและลงค่าคงที่ว่า ป้ายหน้า 3 แถว 3
  *"ไม่ตรงกับแถวใดในตารางที่ commit ไว้เลย"* — **เท็จ** มันอยู่ที่ `n_ID 1671` = `เปลี่ยนค่าย`
  adversary หาเจอด้วยการวัด**ความกว้างที่เรนเดอร์**ของกลุ่มกลิฟแล้วกวาดทั้งตาราง ส่วนสายนี้ค้นแค่
  "ใน run" เท่านั้น ⇒ แก้เป็น `TABLE_EXACT 1671` (ยืนยันเองซ้ำด้วยการ crop 16x: 8 กลุ่มกลิฟ
  เ-ป-ลี่-ย-น-ค่-า-ย ตรง และ exact-cell grep ทั้ง 188 ตารางเจอแถวเดียว) ⇒ `labels_are_read()` = **(17, 17)**
  · `FUNCTION_LABEL_SCREENSHOT_ONLY` เปลี่ยนถ้อยคำเป็น "ยังหาไม่เจอ" ไม่ใช่ "ไม่มี"
- **D2 (ข้ออ้างที่แรงเกินหลักฐาน)** ประโยค *"จับคู่อย่างอื่นไม่ได้"* — **เท็จ** adversary สลับ id ระหว่าง
  แถวที่รูปร่างเหมือนกันแล้ว **เทสทั้งชุดยังเขียว** · ความจริง: **8 แถว** มี shape triple เดียวกัน
  `(0 radio, 1 text, 0 numeric)` (adversary นับ 7 — สายนี้แก้ให้ครบเป็น 8: `p1r3` ก็ shape นี้ แต่ถูกยึด
  ด้วย latin token `NPC` ต่างหาก) ⇒ เหลือ **7 แถวที่ไม่มีอะไรเรียงลำดับให้เลยนอกจากสมมติฐาน**
  ⇒ ตั้งค่าคงที่ `ROW_ORDER_PREMISE` (`n_ID` เรียงขึ้น = ลำดับที่ไคลเอนต์วาด · **ไม่ได้วัด**) แล้วเขียน
  ข้อจำกัดลง docstring ตรง ๆ · เทสใหม่สองใบตรึงว่า 8 แถวนั้นแยกไม่ออกจริง และ premise ต้องมีคำว่า
  `NOT MEASURED`
- **D3 (เทสที่ตรวจ 4 จาก 51 ตัวเลข)** `test_the_shapes_agree...` วนจาก dict ของ role ⇒ แถวที่ไม่มี role
  (หน้า 3 แถว 4 ซึ่งมี radio 2 ปุ่มบนภาพ) **ไม่เคยถูกเยี่ยม** · adversary แก้ค่าเป็น 9/42/0 แล้วเทสยังเขียว
  ⇒ เพิ่ม `test_every_shape_number_is_pinned...` ตรึงครบ **17 × 3 = 51 ตัวเลข** เป็น literal
- **D4** *"เป็นบล็อกเดียวต่อกัน"* — **เท็จ** (ชื่อแท็บห่าง 26/451 id · `1671` ห่าง 258 id · sub-label ของ
  หน้า 3 แถว 5 อยู่นอก run) ⇒ เปลี่ยนชื่อและเนื้อเทสเป็น `test_the_block_is_two_runs_plus_four_strays_not_one_run`
- **D8b** เหตุผลเดิมที่ให้ไว้ว่าทำไมช่องว่างหน้า 1 ยังเป็น "ผู้ต้องสงสัย" (หน้า 2 มีสตริงไม่ถูกวาดแล้วไม่มี
  ช่องว่าง) **ข้ามแกน** — `1403` เป็นตัวเลือก radio = พื้นที่แนวนอน ส่วนช่องว่างหน้า 1 = สล็อตแถวแนวตั้ง
  (adversary วัดได้ 2 × 40.0 px เป๊ะทั้งสองภาพ) ⇒ **ถอนเหตุผลเดิม** เก็บข้อสรุปเดิมไว้ด้วยเหตุผลที่ซื่อกว่า
  ("ไม่มี artifact ไหนบอกว่าแถวที่ไม่ถูกวาดกินที่แนวตั้งหรือไม่")
- **D9** `label_is_unread` กลับด้านความหมาย (คืน `True` ให้แถวเดียวที่คนอ่านจริง) ⇒ เปลี่ยนชื่อเป็น
  `label_has_no_table_row`
- **D6/D7** บันทึกไว้: `ใส่จำนวน` มีทั้ง `n_ID 114` และ `1371` · `ปลดแบนสนทนา` มีทั้ง `1217` และ `1404`
  ⇒ ถอนการยืนยัน id ของ sub-label หน้า 3 แถว 5 · `1404`/`1405` ถูกใช้สองแถว = หลักฐานว่า run ไม่ใช่
  ลำดับการวาดล้วน ๆ (เขียนอยู่ใน `ROW_ORDER_PREMISE`) · `1403` อาจเป็นป้ายของช่อง text เต็มความกว้าง
  บนหน้า 2 แถว 3 แทนที่จะเป็น "ไม่ถูกวาด" — บันทึกเป็นการอ่านที่แข่งกันอยู่ ยังไม่ตัดออก

สิ่งที่ adversary **ทำลายไม่ได้**: การระบุว่า run นี้เป็นของหน้าต่าง**นี้** (`X:`/`Y:`/`Z:` มีอย่างละครั้งเดียว
ใน 188 ตาราง · `นาที (0 คือถาวร)` มีครั้งเดียว) · หมุด sha ทั้งสาม · `progress()` ที่ยัง `(0, 17)` ·
รั้วห้ามคำไทยรั่วเข้า `function` string · เกต cp874/ASCII

## รอบ `vz8mru` — จับคู่ป้าย GMUI กับตาราง log type ของ GMTOOL: ลองแล้ว ปฏิเสธ

`rounds/GM_20260906_0112_dl1etn_...md` ทิ้งงานต่อไว้ให้ P-3: 16 จาก 17 แถวมีป้ายจริงแล้ว
(`gmui_catalog.rows_with_a_read_label`) และตาราง `GMTOOL` (`log_types()` 97 แถว) เป็นสิ่งที่ใกล้เคียงที่สุด
กับ "รายชื่อการกระทำ GM ที่มีอยู่จริง" ในบรรดา artifact ที่ commit แล้ว — ถ้าป้ายของแถวไหนตรงกับ log type
ไหน นั่นคือ artifact ที่สองที่ชี้ไปแถวเดียวกัน ควรเดินสายก่อน (`LOG_TYPES_ARE_NOT_BUTTONS` เตือนไว้แล้วว่า
log type ไม่ใช่ปุ่ม การจับคู่นี้ตั้งแต่ต้นไม่ได้หวังจะ**ปิด**แถวไหน แค่จะช่วย**จัดลำดับ**)

**ผลลบสองชั้น**: `gm/gmui_log_type_join.py` (ใหม่) รันสองการค้นแยกกัน

1. **จับคู่ทั้งสตริง** (`CANDIDATES` — เช็คทุก substring สองทิศทาง บนทั้ง 37 แถวของบล็อกที่ copy มา ไม่ใช่
   แค่ 16 ป้ายแถว) ได้ **3 ผลตรง** และทั้งสามย้อนกลับไปที่จุดเดียว: แถว 1896 (หน้า 3 แถว 5, `กิจกรรมกอบกู้`)
   เป็นคำผสมสองคำ แต่ละครึ่งไปตรงกับ log type คนละแถว (`กอบกู้` = id 4 · `กิจกรรม` = id 12) และ `กิจกรรม`
   ตัวเดียวกันนั้นยังเป็นชื่อแท็บหน้า 3 (`ฟังก์ชั่นกิจกรรม`, บล็อก 1891) ⇒ ชื่อแท็บครอบทุกแถวใต้มัน จึงไม่ใช่
   หลักฐานของแถวใดแถวหนึ่ง และคำผสมที่แยกเป็นสองคำสามัญที่มีอยู่ทั่วตาราง 97 แถวก็ไม่ใช่หลักฐานเช่นกัน
   ⇒ `NO_JOIN_SURVIVES_BECAUSE`

2. **จับคู่บางส่วน (longest common substring, ≥6 ตัวอักษร)** — เพิ่มรอบนี้หลัง `pf-adversary` หักล้างร่างแรก
   ที่เขียนว่า *"ไม่มีแถวไหนใน 97 แถวมีคำว่าผู้เล่น มอนสเตอร์ แบน เตะ วาป หรือค่ายเลย"* — **เท็จ**: log id 1
   คือ `ดรอปมอนสเตอร์` (มีคำว่า `มอนสเตอร์`) และ log id 14/43/47 มีคำว่า `ผู้เล่น` — adversary เจอเพราะการ
   จับคู่ทั้งสตริงข้างบน**มองไม่เห็น**คำที่ฝังอยู่กลางคำผสมคนละคำ (`มอนสเตอร์ที่เกิด` ของบล็อก 1399 ไม่ใช่
   substring ทั้งก้อนของ `ดรอปมอนสเตอร์` และกลับกัน) ⇒ เพิ่มการค้นที่สองที่หา longest-common-substring จริง
   แล้วนับว่าคำที่เจอนั้นไปโผล่ใน log message กี่แถวจาก 97 (ตัววัดความหายาก แยกจากการจับคู่ทั้งสตริง)

   ผลบนข้อมูลปักหมุดวันนี้: เจอ 7 คำร่วมที่ยาว ≥6 ตัวอักษร `ผู้เล่น`(3 แถว) `เปลี่ยน`(8 แถว) `ตัวละคร`(2 แถว)
   `กิจกรรม`(1) `กอบกู้`(1) `มอนสเตอร์`(1) และเศษ `ื่อผู้เล่น` (1, เป็น artifact ของขอบเขต LCS ที่ชนกับ
   `เมื่อ`+`ผู้เล่น` — ไม่ใช่คำใหม่ ยังเป็น `ผู้เล่น` เดิม) — คัดเฉพาะที่ **หายาก** (≤1 แถว log) เหลือ 4:
   `กิจกรรม` · `กอบกู้` · `มอนสเตอร์` · เศษ `ื่อผู้เล่น` ตัวที่น่าสนใจที่สุดคือ `มอนสเตอร์`: บล็อก 1399
   "มอนสเตอร์ที่เกิด:" (แถวสั่งเกิดมอน หน้า 2 แถว 1) กับ log id 1 "ดรอปมอนสเตอร์" (ของหล่นจากมอนสเตอร์) —
   ใช้คำนามเดียวกัน (`มอนสเตอร์`) แต่คนละกริยา (เกิด vs ดรอป) ⇒ อ่านแล้ว **ปฏิเสธ** ไม่ใช่ระดับเดียวกับ
   หลักฐานที่ต้องใช้ (`rare_overlaps()` ยังไม่เพียงพอให้ auto-promote เข้า `backed_matches()` — ต้องมีคนยืนยัน
   ที่จอว่ากดแถวนี้แล้วเกิด log type 1 จริง ถึงจะเลื่อนสถานะได้)

**สิ่งที่แก้จริงตาม `pf-adversary` (คืนผลก่อน push ครั้งนี้ — ไม่ใช่ `ADVERSARY_PENDING`)**:

- **D1 (คำปฏิเสธเท็จ)** ข้อความ *"ไม่มีคำว่าผู้เล่น มอนสเตอร์ แบน เตะ วาป หรือค่าย"* — แก้เป็นคำกล่าวอ้างที่แคบ
  และตรวจได้จริง: `ACTION_ROWS_WITH_NO_LOG_MATCH` ระบุ 12 แถว GMUI ที่เป็น "การกระทำ" ของ GM (ซ่อนตัว ·
  ปรากฏตัว · วาปฉาก/NPC/ผู้เล่น · เรียกผู้เล่น · เตะผู้เล่น · เกิดมอน · สังหารมอน · แบนสนทนา (2 แบบ) ·
  เปลี่ยนค่าย) แล้วยืนยันว่าทุกแถวมีผลตรง **0** ใน `CANDIDATES` จริง (เทสอ่านจากลิสต์นี้ ไม่พิมพ์ซ้ำ) —
  ส่วน**คำนาม**ทั่วไป (ผู้เล่น/มอนสเตอร์/ไอเทม) มีอยู่จริงในตารางแต่ไม่ใช่หลักฐานของ**การกระทำ**ใดแถวหนึ่ง
- **D2** เทสเดิม `test_log_type_domain_note_names_the_absent_categories` เช็คแค่ว่าโน้ตของตัวเองมีคำ
  ภาษาอังกฤษที่อ้าง ไม่ได้เช็คว่าจริงหรือเปล่า (tautology) ⇒ เปลี่ยนเป็นเทสที่ชี้ไปยัง
  `ACTION_ROWS_WITH_NO_LOG_MATCH` ที่ตรวจได้จริงแทน
- **D3** docstring เขียนว่า "ทั้ง 39 แถว" ของบล็อก — นับผิด ของจริงคือ **37 แถว** (`GMUI_LABEL_BLOCK_ROLES`/
  `LABEL_BLOCK` มี 37 คีย์ · เลข 39 มาจากการนับ `1404`/`1405` เป็นสองบทบาทซ้อนแยกกัน ซึ่งโค้ดไม่ได้เดินแบบ
  นั้น) ⇒ แก้ตัวเลขในทุกที่ที่พูดถึง
- **D4** เทสคุมความครบของการค้น (`covers every row`) เดิมเช็คแค่ว่าผลตรงเป็นสับเซตของบล็อกทั้งหมด — ผ่านได้
  แม้การค้นข้ามบางแถวไปเงียบ ๆ (adversary ใส่ `if role == "undrawn": continue` กลางลูปแล้วเทสยังเขียว)
  ⇒ เขียนใหม่ให้ลูปเดียวกันบันทึกบล็อก id ที่ **เดินจนจบจริง** (`_searched_block_ids()`, เพิ่มเข้าเซตเป็น
  บรรทัดสุดท้ายของ loop body) แล้วเทสเทียบกับเซตนั้นแทน — ลองใส่ mutation เดิมซ้ำ: เทสแดงแล้ว (`test_the_
  search_visits_every_row_of_the_copied_block` ชี้ 1396/1403 หายไปตรง ๆ)
- **D5** ทิศทางที่สองของการเทียบ substring (`block_norm in log_norm`) เป็น dead code บนข้อมูลจริงวันนี้ —
  adversary ตัดทิ้งแล้วเทสทั้งชุดยังเขียว เพราะผลตรงทั้งสามหลุดผ่านอีกทิศทางเดียว ⇒ แยก predicate ออกเป็น
  `_is_mutual_substring` แล้วเทสด้วยสตริง ASCII สังเคราะห์ทั้งสองทิศทางตรง ๆ ไม่พึ่งข้อมูลไทยจริง

สิ่งที่ adversary ตรวจแล้วไม่ใช่ข้อบกพร่อง: จำนวนผลตรง 3 ราย (ยืนยันซ้ำใน interpreter ใหม่) · เกต ASCII
ของทั้งสองไฟล์ (`.py` เพิ่มมาไม่มีอักขระนอก ASCII เลย) · รั้ว `backed_matches()` (ลองยัด id ชื่อแท็บเข้า
`_ATTENDED_CONFIRMED_JOINS` แล้วเทสจับได้ตามที่ docstring บอกไว้) · ไม่มีความสัมพันธ์เชิงโครงสร้างระหว่าง
log id 4/12 ที่จะพลิกคำตัดสิน "บังเอิญ" (ตาราง log ไม่มีคอลัมน์หมวดหมู่แยกจาก `n_LogType` เอง)

**nonclaim ของรอบนี้**: ไม่มีปุ่มไหนถูกต่อสาย · `progress()` ยัง `(0, 17)` เท่าเดิม · `backed_matches()`
ยัง `()` · การจับคู่ทั้งสองแบบเป็นแค่การ**จัดลำดับ**ว่าแถวไหนน่าเดินสายก่อน ไม่ใช่การพิสูจน์ว่าแถวไหนต่อสาย
กับ log type ไหนจริง ต้องมีคนยืนยันที่จอก่อนเสมอ · ไม่แตะสถานะโลกต่อฉากเลย (ตารางอ่านอย่างเดียว ไม่มี state)

## รอบ `l86bt4` (2026-09-06T04:12+07:00) — `/lv <n>` เขียนเลเวลจริงลงแถว และล็อกอินครั้งถัดไปส่งเลขนั้น

PANYA-ORDER `20260906_0155` (เส้นตาย 14:00) สั่งให้ `/lv <n>` เป็นงานแรก เพราะ LANE-CS
เทสสกิลท่าโจมตีที่ต้องมีเลเวลไม่ได้ถ้าไม่มีคำสั่งนี้

**สิ่งที่ทำ** — `src/pirateforce_foundation/gm/level_command.py` (โมดูลใหม่) + `_lv_action`
ใน `gm/chat_command_action.py`:

- เขียน `characters.level` ผ่านประตูเดิมของ LANE-DB (`store.write_typed_attributes`)
  ไม่ขอ method ใหม่ ไม่แตะโค้ดของเขา
- **ไม่ส่งเฟรมแอตทริบิวต์ใด ๆ** — เฟรมที่จะเปลี่ยนเลเวลสดคือ sparse `0x309A`
  (BasicAttr bit `0x0002` · `attr_wire.py:424` · RE-117) ซึ่ง `GT-193`/`GT-218` วัดแล้วว่าฆ่าไคลเอนต์
  และกฎ (b'') ของ `attr_wire.py` ห้ามอยู่ · ครึ่ง "เห็นสดโดยไม่ relog" = ใบ RE ของรอบนี้
- ส่ง **หนึ่งประโยคแชท** กลับ socket เดิม (`LV SET RELOG` / `LV NO CHANGE` · codec 0xAC52 เดิม)
- ยาม canonical DB เหมือน `/speed` — `AGENTS.md` sec 7 ห้ามแตะ `state/pirateforce.sqlite3`
  ⇒ ใช้ได้เฉพาะบูต run-copy (`--db`) · ผลข้างเคียงที่ต้องเขียนไว้: เลเวลอยู่รอด relog
  **ภายในบูตเดียว** เท่านั้น บูตหน้าที่ copy DB ใหม่จะกลับเป็นค่าเดิม

**สี่ข้อที่ pf-adversary เปลี่ยนก่อน commit** (รายละเอียดในไฟล์รอบ `rounds/GM_20260906_0412_l86bt4_*`):

1. **D1** — ประตูล็อกอินเป็น "ครบสามหรือไม่ส่งเลย" ⇒ แถวที่ `hp_max IS NULL` /
   `hp_current > hp_max` / `hp_max = 0` จะถูก vitals gate ปฏิเสธทั้งแถว แล้วล็อกอินส่งค่าคงที่ `1`
   ⇒ เขียนสำเร็จแต่จอขึ้น 1 · แก้: `login_would_send()` ถามประตูของล็อกอินเองหลังเขียน
   ถ้าตอบไม่ ⇒ คืนค่าเดิมและปฏิเสธ (`login_would_not_send_it`)
2. **D5** — เพดานเดิม 65535 (ความกว้าง u16) ผิดอันตราย: ตารางไคลเอนต์
   `CONSTDATA_TH__STANDARD_STATUS` มีแค่แถว 1..255 และแถบ EXP ของไคลเอนต์ (`0x519299`)
   **หารด้วยแถว `level + 1`** ⇒ เพดานจริง = 254
3. **D6** — undo ที่ผูกกับ verdict ทำงานเฉพาะตอนเขียน audit ไม่สำเร็จ ⇒ ย้ายการซ่อมมาอยู่ใน
   `write_level` เอง และบอกในคำปฏิเสธว่าคืนค่าได้หรือไม่ (`_row_put_back` / `_row_still_carries_it`)
4. **D7/D11** — อ่านค่าเดิมจากคอลัมน์ (`read_typed_attributes`) ไม่ใช่จาก vitals gate ·
   `console_line` กรอง ASCII จริง ไม่ใช่แค่เขียนใน docstring
...
## Modules delivered (round `nqgmam`, `warp <scene name>`)

### สิ่งที่ผู้ปฏิบัติงานหน้าไคลเอนต์ได้เพิ่ม
พิมพ์ **ชื่อฉาก** แทนเลขได้แล้ว — `warp Prison Exile Island` ให้ผลเท่ากับ `warp 2` ทุกประการ
(บันไดไมล์สโตน M2 เขียนเกณฑ์ไว้ว่า "เกาะ 2 และ 3" และสองชื่อที่ตรงกับสองแถวนั้นคือ
`Prison Exile Island` (id 2) กับ `Spice Paradise Island` (id 3)) · ไม่ต้องเปิดตารางเทียบเลขอีก

### กติกาสามข้อ และเหตุผลที่วัดมา ไม่ใช่ที่ชอบ
1. **ชื่อไม่รับพิกัด** — 52 ชื่อจาก 330 แถวลงท้ายด้วยตัวเลข (`Navy Prison2` · `Atlantic Ocean1`)
   ⇒ กฎ "สองโทเคนท้ายคือ x/y" แยกจาก "ชื่อลงท้ายด้วยเลข" ไม่ได้ในทุกทิศทาง · `warp Navy Prison2 10 20`
   จึงถูก **ปฏิเสธ** ไม่ใช่วาปแบบเงียบ ๆ โดยทิ้งพิกัด · อยากได้พิกัดให้ใช้รูปเลข ซึ่งไม่กำกวมเลย
2. **ชื่อซ้ำไม่เดา** — `Hidden Island` อยู่บน 20 ฉาก · `Poseidon Island` 10 ฉาก ในตารางของไคลเอนต์เอง
   ⇒ คืน id ทั้งหมดแล้วให้ปฏิเสธพร้อมบอกทางออก ไม่มี "ตัวแรกชนะ"
   · สี่แถวที่ **ไม่มีชื่อ** (13 · 137 · 138 · 141) ไปถึงได้ด้วย id เท่านั้น ตลอดไป
3. **รูปเลขไม่เสียอะไรเลย** — id ที่ไม่มีในตารางยังวาปได้เหมือนเดิม (เลขคือค่าบนสาย ยืนได้ด้วยตัวเอง)
   ส่วนชื่อไม่มีความหมายเลยถ้าไม่มีตาราง ⇒ **ชื่อที่ไม่รู้จัก = parse error** · ความไม่สมมาตรนี้คือกฎ
   ไม่ใช่ของหลุด และเทสปักไว้ทั้งสองข้าง

### บรรทัดที่ยื่นให้ผู้ปฏิบัติงานไม่สะท้อนสิ่งที่พิมพ์เข้ามา
ข้อความปฏิเสธทั้งสองแบบเป็น ASCII ล้วน และ **ไม่ใส่ข้อความที่ผู้ใช้พิมพ์กลับไป** — บรรทัดพวกนี้ไปโผล่
คอนโซล cp874 · ผู้ปฏิบัติงานเห็นบรรทัดที่ตัวเองพิมพ์อยู่แล้ว · การสะท้อนข้อความจากไคลเอนต์คือทางที่
อักขระเดียวฆ่าคอนโซลได้ (สะท้อนน้อยกว่าเส้นทาง `_require_int` เดิมที่ใส่ token ดิบลงข้อความด้วยซ้ำ)
· เทส `test_every_name_in_the_table_survives_the_console_encoding` เป็นสายชนวน: ถ้าวันหนึ่ง
ตารางถูก re-derive แล้วมีชื่อนอก cp874 ใบนี้แดงทันที (วันนี้วัดแล้ว: 330/330 เข้ารหัส cp874 ได้)

### nonclaim
- **ไม่ใช่หลักฐานว่า M2 ผ่าน** — นี่คือเครื่องมือไปถึงสภาพที่จะเทส ไม่ใช่ผลของการเทส · การวาปด้วย GM
  ไปถึงเกาะแล้วเห็นเกาะ ไม่ใช่ M2 ผ่าน (`prompts/LANE-GM.md` ประโยคที่สาม)
- ไม่ได้แตะเส้นทางที่ **ส่ง** เฟรม เลยสักบรรทัด — การแก้ทั้งหมดจบที่ parse: ปลายทางได้ `GmCommand`
