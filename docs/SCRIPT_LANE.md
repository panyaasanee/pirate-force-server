...
### API status table (132/160 stub, 28/160 real, as of round `7v7yn2`)
...
"Round 7v7yn2" below) by `tests/test_script_lua_api_quest.py`, the 2
`Player.*` real rows (`GetLv`, `GetClass`) by
`tests/test_script_lua_api_player.py` (round `gqjas5`, see "Round gqjas5"
below), and the 2 additional `Trigger.*` real rows (`QuestActiveProgress`,
`QuestFinishProgress`, round `7v7yn2`, sharing `Quest.*`'s own state door)
by `tests/test_script_lua_api_trigger.py` / `proven` (real + a GT
ticket where a tester watched it work on screen -- none yet).  Next lane
priority, per charter: the remaining 10 `Trigger.*` rows (`GetContactMode`
needs an RE ticket; the rest need wire-frame encoders this lane does not
own), then the rest of `Quest.*` (15 names: `CountDownTime`/reward-and-
grant names still need a LANE-DB column or a `Player.*` grant seam this
lane does not own yet, `GetWeekDay`/`CheckWishQuest` on undocumented
enums/cross-lane guild state -- see "Round 7v7yn2" below), then the rest of
`Player.*` (71 names, grouped by blocker in `lua_api/player.py`'s own
`STILL_STUBBED` -- item/equipment state, a stat-grant write seam, other
per-character stat reads, skill/buff state, teleport/vehicle/camera
frames, UI/cutscene frames, the instance-entry frame, and `MobAppear`
itself).  `Instance.*` is now 9/9 real -- no rows of that namespace remain
in `STILL_STUBBED`.
