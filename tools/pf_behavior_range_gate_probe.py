...
EXACT_HOOKS = {
    "gate": {"va": 0x4758D0, "code": "6aff68c0a4b80064a100000000", "runtime_relocations": [{"offset": 3, "size": 4}]},
    ...
    "range_complete": {"va": 0x75562C, "code": "db4424105f5e5d5b59c20800", "runtime_relocations": []},
}
...
function install(){const m=Process.enumerateModules().find(x=>x.name.toLowerCase()===config.binary.filename.toLowerCase());if(!m||m.size!==config.binary.size_of_image)throw new Error('runtime module guard mismatch');const at={};for(const [name,h] of Object.entries(config.hooks)){at[name]=m.base.add(h.va-config.binary.image_base);if(hex(at[name],h.code.length/2)!==guardedHex(at[name],h,m))throw new Error('runtime code guard mismatch '+name);}const exactCaller=m.base.add(0x44eb22-config.binary.image_base);
Interceptor.attach(at.gate,{onEnter(args){this.valid=args[3].toUInt32()===0xea7d&&this.returnAddress.equals(exactCaller);if(!this.valid)return;const tid=this.threadId;if(gates.has(tid)||activeCount>=config.limits.max_active){fail('duplicate or bounded gate invocation');this.valid=false;return;}const s={id:++nextInvocation,started:Date.now(),range:null,ranges:0};gates.set(tid,s);activeCount++;this.id=s.id;this.address=this.context.pc.toString();emit('gate_enter',{thread_id:tid,sequence:++sequence,invocation:s.id,address:this.address,caller:this.returnAddress.toString(),action:0xea7d});const id=s.id;s.timer=setTimeout(()=>{const current=gates.get(tid);if(current&&current.id===id){fail('gate correlation timeout');gates.delete(tid);activeCount--; }},config.limits.timeout_ms);},onLeave(retval){if(!this.valid)return;const tid=this.threadId,s=gates.get(tid);if(!s||s.id!==this.id){fail('gate return correlation failure');return;}if(s.range!==null){fail('gate returned with active range');return;}const result=retval.toUInt32()&0xff;if(result>1){fail('gate returned non-boolean AL');return;}emit('gate_result',{thread_id:tid,sequence:++sequence,invocation:s.id,address:this.context.pc.toString(),result_bool:result,range_count:s.ranges});clearTimeout(s.timer);gates.delete(tid);activeCount--;}});
Interceptor.attach(at.range_function,{onEnter(args){const s=owner(this.threadId);if(!s)return;if(s.range!==null||s.ranges!==0){fail('range invocation order failure');return;}s.range={sourceKey:args[0].toUInt32(),mode:args[1].toUInt32(),selections:0};emit('range_enter',{thread_id:this.threadId,sequence:++sequence,invocation:s.id,address:this.context.pc.toString(),source_key:s.range.sourceKey,mode:s.range.mode});}});
...
Interceptor.attach(at.range_selected,{onEnter(){const s=owner(this.threadId);if(!s)return;if(s.range===null){fail('selection without range invocation');return;}if(++s.range.selections>config.limits.max_selections){fail('selection bound exceeded');return;}const entry=this.context.eax;if(!readable(entry,0x34)){fail('unreadable selected BEHAVIOR entry');return;}emit('range_selected',{thread_id:this.threadId,sequence:++sequence,invocation:s.id,address:this.context.pc.toString(),entry:entry.toString(),n_id:entry.add(4).readU32(),n_range:entry.add(0x30).readS32()});}});
Interceptor.attach(at.range_complete,{onEnter(){const s=owner(this.threadId);if(!s)return;if(s.range===null){fail('completion without range invocation');return;}const valueAt=this.context.esp.add(0x10);if(!readable(valueAt,4)){fail('unreadable selected range integer');return;}emit('range_complete',{thread_id:this.threadId,sequence:++sequence,invocation:s.id,address:this.context.pc.toString(),raw_selected:valueAt.readS32()});s.range=null;s.ranges=1;}});
