...
EVENTS = {"probe_ready", "behavior_entry_result", "behavior_entry_miss", "probe_error"}
...
MISS_FIELDS = {"thread_id", "sequence", "address", "caller", "manager", "key"}
...
class CaptureState:
    def __init__(self, require_entry: bool = False, require_lookup: bool = False):
    ...
    def accept(self, event: dict[str, Any]) -> None:
        if event["event"] == "probe_ready":
            ...
            self.lookups += 1
            if event["event"] == "behavior_entry_result":
                self.entries += 1
    ...
    def ensure_success(self) -> None:
        ...
        if self.require_lookup and not self.lookups:
            raise RuntimeError("BEHAVIOR lookup result was not observed")
...
def validate_event(value: Any) -> dict[str, Any]:
    ...
    fields = {
        ...
        "behavior_entry_miss": MISS_FIELDS,
    }[value["event"]]
    ...
    if value["event"] in {"behavior_entry_result", "behavior_entry_miss"}:
        integer_fields = fields - {"address", "caller", "manager", "entry", "hit_vector_object", "hit_vector_begin", "hit_vector_end"}
...
def main() -> int:
    ...
    state = CaptureState(args.require_entry, args.require_lookup)
