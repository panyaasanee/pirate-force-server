"""ARMING PROOF: the send-mail button is answered on a default boot.

Run it and read one line:

    UI_SEND_MAIL_ANSWER_ARMED answered=1 label=UI_SEND_MAIL_ANSWERED
    frame_bytes=<n> frame_matches=1 echo_is_the_players_bytes=1
    junk_refused=1 ceiling_is_enforced=1 head=<12hex> code=<12hex>
    RESULT=PASS

Same shape, same rules and the same reason as the proofs beside this
file, for ``Community_SendMailVital`` (``0x6E12``): a real boot with NO
flag and NO scenario, a real frame in, the frame that comes back
...
(``>``), not the EQUALITY the fixed-width classes use, because six
wstring fields make the payload grow -- so the meaningful measurement
is not "does the encoder emit N bytes" but "widen the reviewed row
inside this boot, press a payload that is now too big, get nothing;
restore, press the same payload, get an answer" -- the same shape round
m54yxh D3 forced onto the fixed-width proofs, adapted for a ceiling
instead of an equality.

WHAT IT SAYS.  On this commit, on a default boot, a send-mail frame is
ANSWERED with the player's own bytes, in the envelope this project
already ships, under a label the outbound frame-shape registry names
for this id -- so it passes the send-point gate rather than going
around it.

WHAT IT DOES NOT SAY.  What the real client DRAWS, and not that the
shipped client's "send mail" control emits this id: ``RE-312`` nonclaim
2 records ``observed_frames = 0`` for all eight classes in both
directions and nonclaim 4 declines to claim direction.  Its handler is
proven to exist (RESULT-1, ``0x00645BF0``) and the ordinary receive
path is proven to reach the slot (RESULT-2, ``0x005F38B2``);
...
WHY IT LIVES IN ``tools/`` AND NOT ``src/``.  Same reason as the trade
and friend-removal proofs beside it: a ``make_runtime_vitals`` call
inside ``src/`` moves the ``SRC_VITAL_STREAM_SITES`` census pin, a
shared instrument under repair by chief, and the honest way past
another lane's guard is not to ask for an allowlist entry in it (NOW
`2050`).
...
from pirateforce_foundation import ui_mail_wire as wire
...
TOKEN = "UI_SEND_MAIL_ANSWER_ARMED"
...
def _ceiling_is_enforced(press, payload_len: int) -> int:
    ...
    Shrink the row below this payload's own length (the permissive edit
    a wider ``>`` would hide), press, require nothing back; restore,
    press, require an answer.  The row is restored in a ``finally``: a
    proof that leaves the registry it borrowed in a different state than
    it found it would be reporting on a server nobody is going to run.
    """
...
def run() -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        state = state_type("ui-send-mail-armed")
        state.dispatch(legacy.parse_outer(
            legacy._synthetic_client_login_pc("ui-send-mail-armed")
        ))
        ...
        payload = wire.encode_send_mail_payload(
            wire.SendMailFields(
        ...
        # TWO NEGATIVE CONTROLS.  The first payload cannot decode at all;
        # the second is a whole valid payload plus one unexplained byte,
        # which ``require_exhausted`` refuses before the round trip and
        # before the ceiling check.  Neither is evidence about the
        # ceiling -- ``ceiling_is_enforced`` below measures that.
        ...
        expected_pc, expected_frame = legacy.make_runtime_vitals(
            [(vital_id, wire.COMMUNITY_SEND_MAIL_VITAL_VERSION, payload)]
        )
        frame_matches = int(
        ...
        probe_pc, _probe_frame = legacy.make_runtime_vitals(
            [(vital_id, wire.COMMUNITY_SEND_MAIL_VITAL_VERSION, marker)]
        )
