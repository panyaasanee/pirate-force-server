...
# ASCII only in this file: a non-ASCII separator here (a MIDDLE DOT, U+00B7)
...
# Round 6p22bu re-pin (LANE-A), 17 -> 18 and 26 -> 27 and 16 -> 17.
# world_population_bg0004.py, the Bg0004 (Slave Market Island) census
# (COO-DECISION 2026-08-30T14:41+07:00), builds one entry and sends one
# carrier, the exact same shape world_population_bg0015.py added at round
# w0pu2i.  Not wired to any player-reachable path this round (see that
# module's own "NOT WIRED" docstring paragraph) -- this census counts every
# module in src/, not only the ones a player can reach today.
# Round u3jo4g re-pin (LANE-A), 18 -> 19 and 27 -> 28 and
# 17 -> 18.  world_population_bg0010.py, the Bg0010 (Deep Sea Temple floor 1)
...
# Round p4wire re-pin (LANE-A), 21 -> 22 and 30 -> 31 and 20 -> 21.
# world_population_bg0008.py, the Bg0008 (Silver Harbour) census (fifth door
# of the same COO-DECISION 2026-08-30T14:41+07:00 sequence), builds one entry
# and sends one carrier, the same shape as bg0006 before it -- also wired and
# door-opened in the same round it was built.
# Re-pin (LANE-A, this round), 22 -> 23 and 31 -> 32 and 21 -> 22.
# world_population_bg0003.py, the Bg0003 (Spice Paradise Island) census
...
# Round 78zayw re-pin (LANE-A), 24 -> 25 and 33 -> 34 and 23 -> 24.
# world_population_bg0007.py, the Bg0007 (Voodoo Island) census (seventh
# door of the same COO-DECISION 2026-08-30T14:41+07:00 sequence), builds one
# entry and sends one carrier, the same shape as bg0003 before it -- also
# wired and door-opened in the same round it was built.
# Round ir0lpw re-pin (LANE-A), 25 -> 26 and 34 -> 35 and 24 -> 25.
# world_population_bg0009.py, the Bg0009 (Death City Sea) census (eighth
# door of the same COO-DECISION 2026-08-30T14:41+07:00 sequence), builds one
# entry and sends one carrier, the same shape as bg0007 before it -- also
# wired and door-opened in the same round it was built.
# Round 68mm02 re-pin (LANE-A), 26 -> 27 and 35 -> 36 and 25 -> 26.
# world_population_bg0011.py, the Bg0011 (Deep Sea Temple floor 2) census
# (ninth door of the same COO-DECISION 2026-08-30T14:41+07:00 sequence),
# builds one entry and sends one carrier, the same shape as bg0009 before
# it -- also wired and door-opened in the same round it was built.  This
# is the elevated-risk row (the_two_interiors, shared only with scene 10).
# Round yfbqmg re-pin (LANE-A), 27 -> 28 and 36 -> 37 and 26 -> 27.
# world_population_bg4001.py, the Bg4001 (Navy Training Camp) census
# (TENTH AND LAST door of the same COO-DECISION 2026-08-30T14:41+07:00
# sequence), builds one entry and sends one carrier, the same shape as
...
# ROUND yob0a2 (lane A): 30 -> 31.  world_population_bg3007.py, scene 304's
# ("Dark Fog Sea") census - the SECOND ocean panel, built so the sea a
# crossing at scene 126's map edge leads to is not empty when a GM stands
# in it.  Same shape as every sibling: one entry builder, one carrier.  Its
# registry LOGIN door stays shut; what admits it is the census hook's third
# arm (an owner-decreed arrival plus a live GM warp).
# ROUND 9zj630 (lane A): 31 -> 32.  world_population_bg3008.py, scene 305's
# ("Pale Silver Sea") census - the THIRD ocean panel and the other sea of
...
# chief for.
# ROUND dipufa (lane B): 33 -> 34.  name_colour_sweep.py's own
# build_sweep_population wraps the RE-155 dummy row's NPCAttr bytes into one
# ActorEntry call site, gated fail-closed on PF_NAME_COLOUR_SWEEP -- the
# same shape every other opt-in trial module in this list already carries.
guard(SRC_ACTOR_ENTRY_SITES == 34,
      "src/ builds actor entries at exactly 34 call sites (4 spawns + the "
      ...
      "remote-player-actor production module + the round-dipufa RE-155 "
      "dummy-row sweep)")
...
# ROUND 4uztfj (lane A): 38 -> 39, the Bg3001 (Atlantis) census's own
# carrier.  BOTH rounds moved this number in the same window and the merge
...
guard(SRC_MODULES_WITH_ACTOR_ENTRY == 33
      ...
      "33 named src/ modules build actor entries %s"
