#!/usr/bin/env python3
"""Checksum-guarded observe-only numeric action-data lookup probe."""
...
DEFAULT_CONFIG = Path(__file__).with_name("pf_action_data_binding_probe_local_config.json")
DEFAULT_CLIENT = ROOT.parent / "GameClient" / "GameClient.local.bin"
DEFAULT_CAPTURE_ROOT = ROOT.parent / "GameClient" / "capture_action_data_binding"
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if type(data) is not dict or set(data) != {"schema", "binary", "hooks", "limits"} or data["schema"] != 1:
        raise ValueError("invalid action-data binding probe config root")
    ...
    if data["hooks"] != EXACT_HOOKS or data["limits"] != EXACT_LIMITS:
        raise ValueError("action-data binding provenance differs from exact allowlist")
    return data
...
def guard_binary(path: Path, config: dict[str, Any]):
    ...
    if raw[off : off + len(sig)] != sig:
        raise ValueError("client code guard mismatch at numeric action lookup")
    return pe
...
def validate_output_path(output: Path, client: Path, config: Path, capture_root: Path = DEFAULT_CAPTURE_ROOT) -> Path:
    ...
    guarded = [Path(__file__).resolve(), Path(__file__).with_name("pf_action_data_binding_probe_config.json").resolve(), Path(__file__).with_name("pf_action_data_binding_probe_local_config.json").resolve()]
    for item in guarded:
...
class CaptureState:
    ...
    def ensure_success(self) -> None:
        ...
        if self.require_lookup and not self.lookups:
            raise RuntimeError("numeric action-data lookup was not observed")


def validate_event(value: Any) -> dict[str, Any]:
    if type(value) is not dict or value.get("schema") != 1 or value.get("event") not in EVENTS:
        raise ValueError("invalid action-data binding event")
