...
```json RUNTIMERES_COUNTS
{
  ...
  "src_actor_entry_call_sites": 34,
  "src_actor_stream_call_sites": 44,
  "src_modules_building_actor_entries": 33,
...
## NOTE — round fx0007 (2026-08-31, LANE-A): three live-mirror counts move for the Bg0006 census, built+wired+opened in one round

Fourth door of the same `COO-DECISION 2026-08-30T14:41+07:00` sequence: of the seven doors still shut
after scenes 4, 5 and 10 opened, scene 6 (Bg0006, "Ocean Walled City", 80 native placements) is the
highest by native placement count. `world_population_bg0006.py`, the census half of that pair
(`world_bg0006_identity.py` is the identity half and builds no actor entries itself), builds one entry
and sends one carrier — the same single-module move `world_population_bg0005.py` made at round
`l03cgh`.

`src_actor_entry_call_sites` moves **20 -> 21**, `src_actor_stream_call_sites` moves **29 -> 30**, and
`src_modules_building_actor_entries` moves **19 -> 20** (the new name sorts in alphabetically between
`world_population_bg0005.py` and `world_population_bg0010.py`). This is a NOTE rather than an erratum
because no published sentence was wrong — the three moved numbers live in the `RUNTIMERES_COUNTS`
block, a live mirror of a tool run that is expected to move when we write code; all three are re-pinned
in the tool (`tools/pf_runtimeres_actor_entry_static.py`) and in the bridge-only test module
(`tests/test_runtimeres_actor_entry_static.py`) in the same round, and the `guards` total stays **152**
...
**Wired AND opened, same round, the same compressed shape round `l03cgh` used for scene 5.**
Registering `world_scene_travel.CENSUS_SOURCES`, `world_population_handoff.ROSTER_COMPOSERS`, and
`lane_hooks/lane_a_scene_census.py`'s console reader, and flipping scene 6's registry row
`login_entry_allowed: true`, all land in this same round, for the identical reason round `l03cgh` gave:
the existing generic test (`tests/test_lane_a_scene_census.py::ComposerContractTests`) already assumed
every scene this lane composes a census for is also open at login, since scenes 4, 5, 10 and 14 all
were by the time this round started. See `scenarios/world_scene_registry_001.json`'s own
`login_entry_allowed_because` on the scene-6 row for the D1/D2/D3 safety check this round ran, and
this lane's own round file (`rounds/A_20260831_..._fx0007.md`) for the full account. Scene 6 does not
carry the elevated landing-geometry flag (`the_two_interiors`) scene 10's own row carries — checked,
not assumed. UNLIKE every sibling scene's crosswalk so far, this scene needed a THIRD unresolved-
identity reason: three resolved-in-MOBS leaders whose `MOBS_TIP.s_NAME` is non-ASCII (CJK) and cp874
cannot ship, dropped rather than mis-encoded — see `world_bg0006_identity.py`'s own docstring.

## NOTE — round p4wire (2026-08-31, LANE-A): three live-mirror counts move for the Bg0008 census, built+wired+opened in one round

Fifth door of the same `COO-DECISION 2026-08-30T14:41+07:00` sequence: of the six doors still shut
after scenes 4, 5, 10 and 6 opened, scene 8 (Bg0008, "Silver Harbour", 76 native placements) is the
highest by native placement count. `world_population_bg0008.py`, the census half of that pair
(`world_bg0008_identity.py` is the identity half and builds no actor entries itself), builds one entry
and sends one carrier — the same single-module move `world_population_bg0006.py` made at round
`fx0007`.

`src_actor_entry_call_sites` moves **21 -> 22**, `src_actor_stream_call_sites` moves **30 -> 31**, and
`src_modules_building_actor_entries` moves **20 -> 21** (the new name sorts in alphabetically between
`world_population_bg0006.py` and `world_population_bg0010.py`). This is a NOTE rather than an erratum
because no published sentence was wrong — the three moved numbers live in the `RUNTIMERES_COUNTS`
block, a live mirror of a tool run that is expected to move when we write code; all three are re-pinned
in the tool (`tools/pf_runtimeres_actor_entry_static.py`) and in the bridge-only test module
(`tests/test_runtimeres_actor_entry_static.py`) in the same round, and the `guards` total stays **152**
...
**Wired AND opened, same round, the same compressed shape rounds `l03cgh`/`fx0007` used for scenes
5 and 6.** Registering `world_scene_travel.CENSUS_SOURCES`, `world_population_handoff.ROSTER_COMPOSERS`,
and `lane_hooks/lane_a_scene_census.py`'s console reader, and flipping scene 8's registry row
`login_entry_allowed: true`, all land in this same round, for the identical reason rounds `l03cgh`/
`fx0007` gave: the existing generic test (`tests/test_lane_a_scene_census.py::ComposerContractTests`)
already assumed every scene this lane composes a census for is also open at login, since scenes 4, 5,
6, 10 and 14 all were by the time this round started. See
`scenarios/world_scene_registry_001.json`'s own `login_entry_allowed_because` on the scene-8 row for
the D1/D2/D3 safety check this round ran, and this lane's own round file
(`rounds/A_20260831_..._p4wire.md`) for the full account. Scene 8 does not carry the elevated
landing-geometry flag (`the_two_interiors`) scene 10's own row carries — checked, not assumed. UNLIKE
...
## NOTE -- round jqxe6v (2026-08-31, LANE-B): three live-mirror counts move for Bg0015's synthetic hostile-splice proof

`field_mob_hostile_bg0015.py` (COO-DECISION 2026-08-31T16:48+07:00 layer-1 unlock) builds one synthetic
civilian actor entry per Bg0015 placement for its own splice-proof function only -- not a runtime.py
call site, not a new encoder (same `legacy.make_npc_attr`/`make_remote_actor_entry`/
`make_runtime_remote_actors` this tool's own `SRC_ACTOR_ENTRY_SITES` already counts everywhere else).

`src_actor_entry_call_sites` moved **23 -> 24**, `src_actor_stream_call_sites` moved **32 -> 33**, and
`src_modules_building_actor_entries` moved **22 -> 23** (the new name, `field_mob_hostile_bg0015.py`,
sorts alphabetically first). Re-pinned in the tool, the bridge-only test module and the `RUNTIMERES_COUNTS`
block above in the same round; `guards` stayed **152**. This module does NOT register Bg0015 as a live
scene (`field_mobs._SCENE_TABLE_MODULES`/`live_scenes()` untouched) -- see that module's own docstring.

## NOTE -- this round (78zayw, 2026-08-31, LANE-A): three live-mirror counts move for the Bg0007 census, built+wired+opened in one round

Seventh door of the same `COO-DECISION 2026-08-30T14:41+07:00` sequence: of the four doors still shut
after scenes 4, 5, 10, 6, 8 and 3 opened, scene 7 (Bg0007, "Voodoo Island", 68 native placements) is
the highest by native placement count. `world_population_bg0007.py`, the census half of that pair
(`world_bg0007_identity.py` is the identity half and builds no actor entries itself), builds one entry
and sends one carrier -- the same single-module move `world_population_bg0003.py` made at round
`p7wm17`.

`src_actor_entry_call_sites` moves **24 -> 25**, `src_actor_stream_call_sites` moves **33 -> 34**, and
`src_modules_building_actor_entries` moves **23 -> 24** (the new name sorts in alphabetically between
`world_population_bg0006.py` and `world_population_bg0008.py`). This is a NOTE rather than an erratum
because no published sentence was wrong -- the three moved numbers live in the `RUNTIMERES_COUNTS`
block, a live mirror of a tool run that is expected to move when we write code; all three are re-pinned
in the tool (`tools/pf_runtimeres_actor_entry_static.py`) and in the bridge-only test module
(`tests/test_runtimeres_actor_entry_static.py`) in the same round, and the `guards` total stays **152**
...
used for scenes 5, 6, 8 and 3.** Registering `world_scene_travel.CENSUS_SOURCES`,
`world_population_handoff.ROSTER_COMPOSERS`, and `lane_hooks/lane_a_scene_census.py`'s console reader,
and flipping scene 7's registry row `login_entry_allowed: true`, all land in this same round, for the
identical reason rounds `l03cgh`/`fx0007`/`p4wire`/`p7wm17` gave: the existing generic test
(`tests/test_lane_a_scene_census.py::ComposerContractTests`) already assumed every scene this lane
composes a census for is also open at login, since scenes 3, 4, 5, 6, 8, 10 and 14 all were by the time
this round started. See `scenarios/world_scene_registry_001.json`'s own `login_entry_allowed_because`
on the scene-7 row for the D1/D2/D3 safety check this round ran, and this lane's own round file for the
full account. Scene 7 does not carry the elevated landing-geometry flag (`the_two_interiors`) scenes 10
and 11 carry -- checked, not assumed; its own marker-vs-placement gap (10.793 units) is the tightest
landing geometry any of this lane's own doors has opened on so far, and is INSIDE the placement extents
(unlike scene 3's 405.0-units-outside geometry). Eight of this scene's 44 resolved identities ship a
multi-variant outfit, all two-variant rows (unlike scene 3's nine-variant outlier) -- see
`world_bg0007_identity.py`'s own docstring.

## NOTE -- this round (ir0lpw, 2026-08-31, LANE-A): three live-mirror counts move for the Bg0009 census, built+wired+opened in one round

Eighth door of the same `COO-DECISION 2026-08-30T14:41+07:00` sequence: of the two doors still shut
after scenes 4, 5, 10, 6, 8, 3 and 7 opened, scene 9 (Bg0009, "Death City Sea", 63 native placements) is
the highest by native placement count. `world_population_bg0009.py`, the census half of that pair
(`world_bg0009_identity.py` is the identity half and builds no actor entries itself), builds one entry
and sends one carrier -- the same single-module move `world_population_bg0007.py` made at round
`78zayw`.

`src_actor_entry_call_sites` moves **25 -> 26**, `src_actor_stream_call_sites` moves **34 -> 35**, and
`src_modules_building_actor_entries` moves **24 -> 25** (the new name sorts in alphabetically between
`world_population_bg0008.py` and `world_population_bg0010.py`). This is a NOTE rather than an erratum
because no published sentence was wrong -- the three moved numbers live in the `RUNTIMERES_COUNTS`
block, a live mirror of a tool run that is expected to move when we write code; all three are re-pinned
in the tool (`tools/pf_runtimeres_actor_entry_static.py`) and in the bridge-only test module
(`tests/test_runtimeres_actor_entry_static.py`) in the same round, and the `guards` total stays **152**
...
**Wired AND opened, same round, the same compressed shape rounds `l03cgh`/`fx0007`/`p4wire`/`p7wm17`/
`78zayw` used for scenes 5, 6, 8, 3 and 7.** Registering `world_scene_travel.CENSUS_SOURCES`,
`world_population_handoff.ROSTER_COMPOSERS`, and `lane_hooks/lane_a_scene_census.py`'s console reader,
and flipping scene 9's registry row `login_entry_allowed: true`, all land in this same round, for the
identical reason rounds `l03cgh`/`fx0007`/`p4wire`/`p7wm17`/`78zayw` gave: the existing generic test
(`tests/test_lane_a_scene_census.py::ComposerContractTests`) already assumed every scene this lane
composes a census for is also open at login, since scenes 3, 4, 5, 6, 7, 8, 10 and 14 all were by the
time this round started. See `scenarios/world_scene_registry_001.json`'s own `login_entry_allowed_because`
on the scene-9 row for the D1/D2/D3 safety check this round ran, and this lane's own round file for the
full account. Scene 9 does not carry the elevated landing-geometry flag (`the_two_interiors`) scenes 10
and 11 carry -- checked, not assumed; its own marker-vs-placement gap (2198.81 units) is the WIDEST
landing geometry any of this lane's own doors has opened on so far, still INSIDE the placement extents
(unlike scene 3's 405.0-units-outside geometry, which is genuinely outside). Eleven of this scene's 38
resolved identities ship a multi-variant outfit, all two-variant rows (unlike scene 3's nine-variant
outlier, similar to scene 7's eight) -- see `world_bg0009_identity.py`'s own docstring. UNLIKE EVERY
EARLIER SIBLING, this scene's own registry `native_definition_count` (44) agrees with this round's own
placement-side count of distinct Mob-Set numbers (44) rather than disagreeing by one -- see
...
## NOTE -- this round (yfbqmg, 2026-09-01, LANE-A): three live-mirror counts move for the Bg4001 census, built+wired+opened in one round -- the TENTH AND LAST door

Tenth and final door of the same `COO-DECISION 2026-08-30T14:41+07:00` sequence: of the ten surveyed in
round `ga91m5`, only scene 130 (Bg4001, "Navy Training Camp", 42 native placements) remained shut after
scenes 4, 5, 10, 6, 8, 3, 7, 9 and 11 opened. `world_population_bg4001.py`, the census half of that pair
(`world_bg4001_identity.py` is the identity half and builds no actor entries itself), builds one entry
and sends one carrier -- the same single-module move `world_population_bg0011.py` made at round
`68mm02`.

`src_actor_entry_call_sites` moves **27 -> 28**, `src_actor_stream_call_sites` moves **36 -> 37**, and
`src_modules_building_actor_entries` moves **26 -> 27** (the new name sorts in alphabetically AFTER
`world_population_bg0015.py`, the last entry, rather than between two existing ones -- the only module
name so far whose numeric suffix does not start with `0`). This is a NOTE rather than an erratum
because no published sentence was wrong -- the three moved numbers live in the `RUNTIMERES_COUNTS`
block, a live mirror of a tool run that is expected to move when we write code; all three are re-pinned
in the tool (`tools/pf_runtimeres_actor_entry_static.py`) and in the bridge-only test module
(`tests/test_runtimeres_actor_entry_static.py`) in the same round, and the `guards` total stays **152**
...
**Wired AND opened, same round, the same compressed shape rounds `l03cgh`/`fx0007`/`p4wire`/`p7wm17`/
`78zayw`/`ir0lpw`/`68mm02` used for scenes 5, 6, 8, 3, 7, 9 and 11.** Registering
`world_scene_travel.CENSUS_SOURCES`, `world_population_handoff.ROSTER_COMPOSERS`, and
`lane_hooks/lane_a_scene_census.py`'s console reader, and flipping scene 130's registry row
`login_entry_allowed: true`, all land in this same round, for the identical reason rounds
`l03cgh`/`fx0007`/`p4wire`/`p7wm17`/`78zayw`/`ir0lpw`/`68mm02` gave: the existing generic test
(`tests/test_lane_a_scene_census.py::ComposerContractTests`) already assumed every scene this lane
composes a census for is also open at login, since scenes 3, 4, 5, 6, 7, 8, 9, 10, 11 and 14 all were by
the time this round started. See `scenarios/world_scene_registry_001.json`'s own
`login_entry_allowed_because` on the scene-130 row for the D1/D2/D3 safety check this round ran, and
this lane's own round file for the full account. UNLIKE SCENE 11, this scene does NOT carry the elevated
landing-geometry flag (`the_two_interiors`) -- checked, not assumed (n_CANGLIDE 1, n_LIMIT_HEIGHT 0, not
the (0, 0) pair that flag names). Its own marker-vs-placement gap (1018.201 units, OUTSIDE the placement
extents, unlike scene 11's own inside-extents marker) puts it among the six of the ten doors whose
marker landed outside rather than inside. Two of this scene's 17 resolved identities ship a
multi-variant outfit, both THREE-variant rows (a wider fan-out than every other sibling's own
two-variant rows, narrower than scene 3's nine-variant outlier) -- see `world_bg4001_identity.py`'s own
docstring. UNLIKE every earlier sibling, this scene's own registry `native_definition_count` (20)
disagrees with this round's own placement-side count of distinct Mob-Set numbers (18) by TWO rather than
by zero or one -- see `world_bg4001_identity.py`'s own DISCREPANCY paragraph for the full account,
including all four CLINE keys (two carrying real, non-zero leaders that no placement in this scene ever
points at, one more real-and-unreached, and one carrying a literal-zero leader) this scene's placements
