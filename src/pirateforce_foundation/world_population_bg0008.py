"""Bg0008 (Silver Harbour) census composer - LANE-A.
...
approved for scenes 4, 5, 10, 14 and 6.  This is the census half; the
identity half is ``world_bg0008_identity``.
...
``world_population_bg0005`` and ``world_population_bg0006`` are the same
shape for scenes 2, 14, 4, 5 and 6.  This is the seventh: it refuses
anywhere but scene 8, over ``world_bg0008_identity``'s 69 shippable
placements of 76.  Every wire call below is the exact frozen serializer the
other six already use (``legacy.make_npc_attr`` / ``make_remote_movement_attr``
...
WHAT IS DIFFERENT FROM THE FIVE SIBLING SCENES, NAMED RATHER THAN LEFT
IMPLICIT.
...
* No CJK/non-cp874 name family this scene needed - unlike bg0006's three
  ...
  plain ASCII (checked in ``world_bg0008_identity._self_check``).
...
* There is no faction bit on any entry here, same as every sibling and for
  ...
  (the rank-1, ai-combat-nonzero sets - 22 through 35 of the 41 resolved)
...
* LANDING GEOMETRY.  ``scenarios/world_scene_registry_001.json``'s own
  ...
  scene's marker point 8.8 units from the nearest native placement, INSIDE
  the placement extents - the tightest gap of any door opened so far) is
...
* WIRED AND OPENED IN THE SAME ROUND AS THIS BUILD, following rounds
  ``l03cgh``/``fx0007``'s compressed precedent for scenes 5 and 6 rather
  than the earlier three-round split (build, then wire, then open) scenes
  4's and 10's own first rounds used - the generic contract test
  ...
  scenes 4/5/6/10/14 all were by the time this round started.
"""
...
from . import world_bg0008_identity as identity
...
# Convention marker: matching the six sibling composer modules' own
# convention.
...
COUNT_SOURCE_FULL_ROSTER = "bg0008_full_roster"
COUNT_SOURCE_CALLER = "caller_requested"
COUNT_SOURCES = (COUNT_SOURCE_FULL_ROSTER, COUNT_SOURCE_CALLER)


class Bg0008CensusError(ValueError):
    """A refusal from this module, always with a reason in the message."""


@dataclass(frozen=True)
class Bg0008PopulationGeneration:
    """One built Bg0008 collection: its membership, its bytes, nothing sent."""
...
def _require_anchor(player_xyz: Any) -> tuple[float, float, float]:
    if type(player_xyz) is not tuple or len(player_xyz) != 3:
        raise Bg0008CensusError("player XYZ must be an exact three-value tuple")
    import math
    checked = []
    for axis, value in zip("xyz", player_xyz):
        if type(value) not in (int, float) or type(value) is bool:
            raise Bg0008CensusError(
...
def census_order(
...
) -> tuple[identity.Bg0008Placement, ...]:
    """The 69 shippable placements, nearest the anchor first.
    ...
    slice of the port.  ``shippable_placements`` has already dropped the 7
...
def _entry(legacy: Any, placement: identity.Bg0008Placement) -> bytes:
...
def build_bg0008_population(
    ...
    *,
    scene_id: int,
    count_source: str = COUNT_SOURCE_CALLER,
) -> Bg0008PopulationGeneration:
    """Build the Bg0008 roster as ONE RuntimeRes collection.  Sends nothing.

    ``scene_id`` has no default on purpose, the same reason every other
    builder gives: every actor in this table is encoded with scene 8, and a
    caller that forgets which scene it is in would deliver Silver Harbour
    NPCs into a different map.  Refuses anywhere but scene 8.
    """
    if type(scene_id) is not int or scene_id != SCENE_N_ID:
        raise Bg0008CensusError(
            "the Bg0008 roster is only valid in scene %d, not scene %r"
            % (SCENE_N_ID, scene_id))
    if count_source not in COUNT_SOURCES:
        raise Bg0008CensusError("unknown count source %r" % count_source)
    ...
    return Bg0008PopulationGeneration(
        count,
...
def wire_actor_count(generation: Bg0008PopulationGeneration) -> int:
    """Read the collection count back out of the bytes that will be sent."""
    if type(generation) is not Bg0008PopulationGeneration:
        raise Bg0008CensusError(
            "wire actor count needs a Bg0008PopulationGeneration")
    pc = generation.pc
...
def dispatch_report(generation: Bg0008PopulationGeneration) -> dict:
    ...
    if type(generation) is not Bg0008PopulationGeneration:
        raise Bg0008CensusError(
            "dispatch report needs a Bg0008PopulationGeneration")
...
def census_console_line(generation: Bg0008PopulationGeneration) -> str:
    """The one grep-able ``WORLD_CENSUS_BG0008`` line a boot in this scene
    prints.  ``assembled=N/76`` against the scene's REAL placement count, not
    against the 69 that happen to resolve.  ASCII only (bridge console is
    cp874).
    """
    report = dispatch_report(generation)
    return (
        "WORLD_CENSUS_BG0008 assembled={0}/{1} shippable={2} wire={3} "
...
def actor_lines(generation: Bg0008PopulationGeneration) -> tuple[str, ...]:
    ...
    shipped table is ASCII by ``world_bg0008_identity._self_check``.
    """
    if type(generation) is not Bg0008PopulationGeneration:
        raise Bg0008CensusError("actor lines need a Bg0008PopulationGeneration")
...
def unresolved_lines() -> tuple[str, ...]:
    ...
    """
    return tuple(
        "BG0008_UNSHIPPED placement=%d set=%d cline_row=%d leader_n_id=%d "
        "reason=%s"
...
def census_console_lines(
    ...
    """``WORLD_CENSUS_BG0008`` + one line per actor + one per dropped one.

    Everything a headless proof for this scene needs, in one call, built from
    ONE generation.
    """
    generation = build_bg0008_population(
