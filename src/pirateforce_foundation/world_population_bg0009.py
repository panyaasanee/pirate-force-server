"""Bg0009 (Death City Sea) census composer - LANE-A.

Eighth door of the ten surveyed in round ``12lyda``, following the same
placement-count-ranked sequence ``COO-DECISION 2026-08-30T14:41+07:00``
approved for scenes 4, 5, 10, 6, 8, 3 and 7.  This is the census half; the
identity half is ``world_bg0009_identity``.
...
``world_population_bg0008``, ``world_population_bg0003`` and
``world_population_bg0007`` are the same shape for scenes 2, 14, 4, 5, 6, 8,
3 and 7.  This is the tenth: it refuses anywhere but scene 9, over
``world_bg0009_identity``'s 57 shippable placements of 63.  Every wire call
below is the exact frozen serializer the other nine already use
...
WHAT IS DIFFERENT FROM THE EIGHT SIBLING SCENES, NAMED RATHER THAN LEFT
IMPLICIT.

* No CJK/non-cp874 name family this scene needed - unlike bg0006's three
  teleporter drops, every one of this scene's 38 resolved identities is
  plain ASCII (checked in ``world_bg0009_identity._self_check``).
...
* ELEVEN sets ship a multi-variant outfit, all two-variant rows (unlike
  bg0003's nine-variant outlier) - see ``world_bg0009_identity``'s own
  docstring.
...
* LANDING GEOMETRY.  ``scenarios/world_scene_registry_001.json``'s own
  ...
  scene's marker point 2198.81 units from the nearest native placement,
  INSIDE the placement extents, and not carrying ``the_two_interiors``) is
...
* WIRED AND OPENED IN THE SAME ROUND AS THIS BUILD, following rounds
  ``l03cgh``/``fx0007``/``p4wire``/``p7wm17``/``78zayw``'s compressed
  precedent for scenes 5, 6, 8, 3 and 7 rather than the earlier three-round
  split (build, then wire, then open) scenes 4's and 10's own first rounds
  used - the generic contract test
  ...
  scenes 3/4/5/6/7/8/10/14 all were by the time this round started.
"""
...
# Convention marker: matching the nine sibling composer modules' own
# convention.
...
COUNT_SOURCE_FULL_ROSTER = "bg0009_full_roster"
COUNT_SOURCE_CALLER = "caller_requested"
COUNT_SOURCES = (COUNT_SOURCE_FULL_ROSTER, COUNT_SOURCE_CALLER)


class Bg0009CensusError(ValueError):
    """A refusal from this module, always with a reason in the message."""


@dataclass(frozen=True)
class Bg0009PopulationGeneration:
    """One built Bg0009 collection: its membership, its bytes, nothing sent."""
...
def _require_anchor(player_xyz: Any) -> tuple[float, float, float]:
    if type(player_xyz) is not tuple or len(player_xyz) != 3:
        raise Bg0009CensusError("player XYZ must be an exact three-value tuple")
    import math
    checked = []
    for axis, value in zip("xyz", player_xyz):
        if type(value) not in (int, float) or type(value) is bool:
            raise Bg0009CensusError(
...
def census_order(
...
) -> tuple[identity.Bg0009Placement, ...]:
    """The 57 shippable placements, nearest the anchor first.
    ...
    slice of the sea.  ``shippable_placements`` has already dropped the 6
...
def _entry(legacy: Any, placement: identity.Bg0009Placement) -> bytes:
...
def build_bg0009_population(
    ...
    *,
    scene_id: int,
    count_source: str = COUNT_SOURCE_CALLER,
) -> Bg0009PopulationGeneration:
    """Build the Bg0009 roster as ONE RuntimeRes collection.  Sends nothing.

    ``scene_id`` has no default on purpose, the same reason every other
    builder gives: every actor in this table is encoded with scene 9, and a
    caller that forgets which scene it is in would deliver Death City Sea
    NPCs into a different map.  Refuses anywhere but scene 9.
    """
    if type(scene_id) is not int or scene_id != SCENE_N_ID:
        raise Bg0009CensusError(
            "the Bg0009 roster is only valid in scene %d, not scene %r"
            % (SCENE_N_ID, scene_id))
    if count_source not in COUNT_SOURCES:
        raise Bg0009CensusError("unknown count source %r" % count_source)
    ...
    return Bg0009PopulationGeneration(
        count,
...
def wire_actor_count(generation: Bg0009PopulationGeneration) -> int:
    """Read the collection count back out of the bytes that will be sent."""
    if type(generation) is not Bg0009PopulationGeneration:
        raise Bg0009CensusError(
            "wire actor count needs a Bg0009PopulationGeneration")
    pc = generation.pc
...
def dispatch_report(generation: Bg0009PopulationGeneration) -> dict:
    ...
    if type(generation) is not Bg0009PopulationGeneration:
        raise Bg0009CensusError(
            "dispatch report needs a Bg0009PopulationGeneration")
...
def census_console_line(generation: Bg0009PopulationGeneration) -> str:
    """The one grep-able ``WORLD_CENSUS_BG0009`` line a boot in this scene
    prints.  ``assembled=N/63`` against the scene's REAL placement count, not
    against the 57 that happen to resolve.  ASCII only (bridge console is
    cp874).
    """
    report = dispatch_report(generation)
    return (
        "WORLD_CENSUS_BG0009 assembled={0}/{1} shippable={2} wire={3} "
...
def actor_lines(generation: Bg0009PopulationGeneration) -> tuple[str, ...]:
    ...
    shipped table is ASCII by ``world_bg0009_identity._self_check``.
    """
    if type(generation) is not Bg0009PopulationGeneration:
        raise Bg0009CensusError("actor lines need a Bg0009PopulationGeneration")
...
def unresolved_lines() -> tuple[str, ...]:
    ...
    """
    return tuple(
        "BG0009_UNSHIPPED placement=%d set=%d cline_row=%d leader_n_id=%d "
        "reason=%s"
...
def census_console_lines(
    ...
    """``WORLD_CENSUS_BG0009`` + one line per actor + one per dropped one.

    Everything a headless proof for this scene needs, in one call, built from
    ONE generation.
    """
    generation = build_bg0009_population(
