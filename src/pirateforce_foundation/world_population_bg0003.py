"""Bg0003 (Spice Paradise Island) census composer - LANE-A.

Sixth door of the ten surveyed in round ``12lyda``, following the same
placement-count-ranked sequence ``COO-DECISION 2026-08-30T14:41+07:00``
approved for scenes 4, 5, 10, 14, 6 and 8.  This is the census half; the
identity half is ``world_bg0003_identity``.
...
``world_population_bg0005``, ``world_population_bg0006`` and
``world_population_bg0008`` are the same shape for scenes 2, 14, 4, 5, 6 and
8.  This is the eighth: it refuses anywhere but scene 3, over
``world_bg0003_identity``'s 62 shippable placements of 72.  Every wire call
below is the exact frozen serializer the other seven already use
...
WHAT IS DIFFERENT FROM THE SIX SIBLING SCENES, NAMED RATHER THAN LEFT
IMPLICIT.

* No CJK/non-cp874 name family this scene needed - unlike bg0006's three
  teleporter drops, every one of this scene's 41 resolved identities is
  plain ASCII (checked in ``world_bg0003_identity._self_check``).
...
* NINE sets ship a multi-variant outfit (widest fan-out any crosswalk in
  this lane has recorded, one leader listing NINE variants) - see
  ``world_bg0003_identity``'s own docstring.
...
* LANDING GEOMETRY.  ``scenarios/world_scene_registry_001.json``'s own
  ...
  scene's marker point 405.0 units from the nearest native placement,
  OUTSIDE the placement extents, and not carrying ``the_two_interiors``) is
...
* WIRED AND OPENED IN THE SAME ROUND AS THIS BUILD, following rounds
  ``l03cgh``/``fx0007``/``p4wire``'s compressed precedent for scenes 5, 6
  and 8 rather than the earlier three-round split (build, then wire, then
  open) scenes 4's and 10's own first rounds used - the generic contract
  ...
  login, since scenes 4/5/6/8/10/14 all were by the time this round
  started.
"""
...
from . import world_bg0003_identity as identity
...
# Convention marker: matching the seven sibling composer modules' own
# convention.
...
COUNT_SOURCE_FULL_ROSTER = "bg0003_full_roster"
COUNT_SOURCE_CALLER = "caller_requested"
COUNT_SOURCES = (COUNT_SOURCE_FULL_ROSTER, COUNT_SOURCE_CALLER)


class Bg0003CensusError(ValueError):
    """A refusal from this module, always with a reason in the message."""


@dataclass(frozen=True)
class Bg0003PopulationGeneration:
    """One built Bg0003 collection: its membership, its bytes, nothing sent."""
...
def _require_anchor(player_xyz: Any) -> tuple[float, float, float]:
    if type(player_xyz) is not tuple or len(player_xyz) != 3:
        raise Bg0003CensusError("player XYZ must be an exact three-value tuple")
    import math
    checked = []
    for axis, value in zip("xyz", player_xyz):
        if type(value) not in (int, float) or type(value) is bool:
            raise Bg0003CensusError(
...
def census_order(
...
) -> tuple[identity.Bg0003Placement, ...]:
    """The 62 shippable placements, nearest the anchor first.
    ...
    10 with no identity; this only orders what it returned.
    """
...
def _entry(legacy: Any, placement: identity.Bg0003Placement) -> bytes:
...
def build_bg0003_population(
    ...
    *,
    scene_id: int,
    count_source: str = COUNT_SOURCE_CALLER,
) -> Bg0003PopulationGeneration:
    """Build the Bg0003 roster as ONE RuntimeRes collection.  Sends nothing.

    ``scene_id`` has no default on purpose, the same reason every other
    builder gives: every actor in this table is encoded with scene 3, and a
    caller that forgets which scene it is in would deliver Spice Paradise
    Island NPCs into a different map.  Refuses anywhere but scene 3.
    """
    if type(scene_id) is not int or scene_id != SCENE_N_ID:
        raise Bg0003CensusError(
            "the Bg0003 roster is only valid in scene %d, not scene %r"
            % (SCENE_N_ID, scene_id))
    if count_source not in COUNT_SOURCES:
        raise Bg0003CensusError("unknown count source %r" % count_source)
    ...
    return Bg0003PopulationGeneration(
        count,
...
def wire_actor_count(generation: Bg0003PopulationGeneration) -> int:
    """Read the collection count back out of the bytes that will be sent."""
    if type(generation) is not Bg0003PopulationGeneration:
        raise Bg0003CensusError(
            "wire actor count needs a Bg0003PopulationGeneration")
    pc = generation.pc
...
def dispatch_report(generation: Bg0003PopulationGeneration) -> dict:
    ...
    if type(generation) is not Bg0003PopulationGeneration:
        raise Bg0003CensusError(
            "dispatch report needs a Bg0003PopulationGeneration")
...
def census_console_line(generation: Bg0003PopulationGeneration) -> str:
    """The one grep-able ``WORLD_CENSUS_BG0003`` line a boot in this scene
    prints.  ``assembled=N/72`` against the scene's REAL placement count, not
    against the 62 that happen to resolve.  ASCII only (bridge console is
    cp874).
    """
    report = dispatch_report(generation)
    return (
        "WORLD_CENSUS_BG0003 assembled={0}/{1} shippable={2} wire={3} "
...
def actor_lines(generation: Bg0003PopulationGeneration) -> tuple[str, ...]:
    ...
    shipped table is ASCII by ``world_bg0003_identity._self_check``.
    """
    if type(generation) is not Bg0003PopulationGeneration:
        raise Bg0003CensusError("actor lines need a Bg0003PopulationGeneration")
...
def unresolved_lines() -> tuple[str, ...]:
    ...
    """
    return tuple(
        "BG0003_UNSHIPPED placement=%d set=%d cline_row=%d leader_n_id=%d "
        "reason=%s"
...
def census_console_lines(
    ...
    """``WORLD_CENSUS_BG0003`` + one line per actor + one per dropped one.

    Everything a headless proof for this scene needs, in one call, built from
    ONE generation.
    """
    generation = build_bg0003_population(
