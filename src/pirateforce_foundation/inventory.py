...
def _core_is_carried_unchanged(
    value: BackpackState, core: BackpackState,
) -> bool:
    """Whether ``value`` is ``core`` with nothing but ACQUIRED rows added.

    Three conditions, and every one of them is a refusal that matters:

    * the three header fields are ``core``'s own.  A drifted header is a
      different bag, not a bag with an extra row in it.
    * every identity ``core`` holds is present in ``value`` with a
      byte-identical row -- same template, quantity, slot and both raw
      bytes.  A golden row that MOVED, was spent, or was retargeted fails
      here, which is what keeps this from becoming "any bag that happens to
      contain identity 1 and identity 3".
    * every OTHER row carries an identity strictly greater than every
      identity in ``core``.  That is not a stylistic bound: it is the only
      identity a pickup can ever have.  ``store.commit_acquired_backpack_item``
      refuses any identity that is not the character's
      ``character_backpacks.next_item_identity``, and that column is seeded
      by ``store._insert_initial_backpack`` at ``max(core identities) + 1``
      and only ever advances.  So a row below the bound was not issued by the
      pickup path, and this function has no business calling it acquired.

    A ``core`` of zero rows would make this answer True for every bag with
    matching headers; no starting bag is empty, and ``starting_core_of``
    below is the only caller, but the guard is here rather than in a comment.
    """
    if type(value) is not BackpackState or type(core) is not BackpackState:
        return False
    if not core.items:
        return False
    if (value.base_mask, value.base_identity, value.range_mask) != (
            core.base_mask, core.base_identity, core.range_mask):
        return False
    rows = {item.identity: item for item in value.items}
    if any(rows.get(item.identity) != item for item in core.items):
        return False
    highest_core_identity = max(item.identity for item in core.items)
    core_identities = {item.identity for item in core.items}
    return all(
        item.identity > highest_core_identity
        for item in value.items
        if item.identity not in core_identities
    )


def starting_core_of(value: BackpackState) -> BackpackState | None:
    """The starting bag ``value`` still carries, or ``None``.

    ``value in STARTING_BACKPACKS`` answers "is this bag untouched"; this
    answers "was this bag BORN as one of the starting bags and has since only
    been added to".  The difference is a player who picked one item up off a
    mob: their bag is not a starting bag any more and never will be again,
    and every gate that asks the first question has been telling them no.

    Returns the core itself rather than a bool so a caller can report WHICH
    bag, and so the answer cannot be mistaken for "yes it is a starting bag".
    """
    for core in STARTING_BACKPACKS:
        if _core_is_carried_unchanged(value, core):
            return core
    return None


def settled_core_of(value: BackpackState) -> BackpackState | None:
    """The starting bag whose MERGED state ``value`` still carries, or ``None``.

    The V111 replay answer.  Asked AFTER :func:`starting_core_of` by every
    caller, and the order is load-bearing: a bag that still holds the
    unmerged pre-state cannot be read as settled, because the pre-state's
    identity 1 carries the unsummed quantity and the merged core carries the
    sum.  The two cores are therefore mutually exclusive for the same
    starting bag, and asking the pre-state first means a bag that somehow
    matched both would be MERGED rather than silently called done.
    """
    for core in STARTING_BACKPACKS:
        if not can_merge_v111(core):
            continue
        if _core_is_carried_unchanged(value, merged_v111_state(core)):
            return core
    return None
