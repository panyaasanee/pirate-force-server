...
``environ`` is injectable so tests never touch the process environment.
...
(evidence gathered this round, pf_bridge/rounds/R342_*):
  ...
  MEASURED  The v141 protocol_name_id hash
            ...
            docs/PF_VITAL_NAMES.json -- an independent snapshot -- on all 17
            names the two share.  That hash over the name above is 0xC4AF.
...
the client's id assignment is the same hash it demonstrably is for 344 other
cases, then 0xC4AF follows.  That is a strong argument and it is still an
...
# The sea scene the attended round actually boots into.  Measured, not
# derived: GT-228/R308 (pf_bridge/notes_to_chief/20260904_1331_KA1A-R308-*)
# recorded `WORLD_SCENE scene_id=126` and an `S126-SPAWN` line on the very
# boot where island contact fired TriggerVital id 2 and id 3.  This is NOT
# the same claim as "126 is the Columbus destination" -- it is not, and
# `world_m2_sea_destination.py` explains at length why the ocean PANEL id
# and the ship scene are different things.  The trial fires where the
# tester and the islands are, which is here.
M2_SEA_SCENE_ID = 126
...
def console_line(state: str, scene_id: int, record_count: int = 0) -> str:
    ...
    without anyone having to read the build:
        ...
        M2_SURVEY_TRIAL_NOT_THIS_BOOT scene=126 reason=unset
        M2_SURVEY_TRIAL_NOT_THIS_BOOT scene=126 reason=malformed
    ...
    return (
        ...
        f" scene={int(scene_id)} reason={state}"
    )
