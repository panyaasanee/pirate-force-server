"""The Dark Fog Sea's cast as ONE census collection - LANE-A, scene 304.

The census half of round ``yob0a2``'s pair; ``world_bg3007_identity`` is the
identity half and holds every table row this file sends.  50 of the scene's
66 native placements, COUNTED BY PLACEMENT AND NOT BY NAME:
    ...
    19 hulls (``SP_*``) with a name plate: 9 Merchant Ship, 3 of the mined
     ...
     9 level-120 Pirate Ships, the only rows here with a real HP number
    20 invisible weather markers: 14 named ``Tornado`` and 6 placements of
       one nameless INVISIBLE set (the ``53|54`` pair, first leg shipped)
     2 islands-as-actors (``MAP_ISLAND_01``): Mad Sand Island, Pirate Lair

the cast of an ``n_SCENE_TYPE 8`` OCEAN PANEL, the same scene shape scene
126 has, rather than of a town.  The 16 that do not ship, and the single
reason all 16 share, are in ``world_bg3007_identity.UNRESOLVED``.

THE SIBLING PATTERN, NOT A FORK.  ``world_population`` builds bg0001's
census and refuses anywhere but scene 1; ``world_population_bg3001`` is the
same shape for scene 126 and is the module this one is copied from, line for
line, with this scene's own numbers.  It refuses anywhere but scene 304.
...
TRUE.  A GM who types ``/warp 304`` today lands in an empty ocean: the scene
has an arrival point (``#843``) and no cast at all.  With this file and its
registration in ``world_scene_travel.CENSUS_SOURCES`` /
``lane_hooks/lane_a_scene_census.py``, the arrival path composes these 50
actors for that session on its first TargetPosVital.  It is still GM-only -
``accounts.is_gm_account`` gates ``/warp``, ``login_entry_allowed`` for this
scene stays ``false``, and this file opens no door of its own.  NOBODY HAS
SEEN IT: no client has ever received a byte of this scene, and the ticket
this round files is what turns that sentence into a measurement.
...
* THE DOOR IS SHUT AND STAYS SHUT.  ``login_entry_allowed`` for scene 304 is
...
* 15 SETS SHIP ``INVISIBLE`` bodies and one of them ships with no name at
  all, both under the precedent ``world_bg0004_identity`` set 107 and scene
  126's own set 53 already ship under.
* 656 EXTRA SPAWN POINTS in the placement file are NOT shipped; this
...
from . import world_bg3007_identity as identity
...
COUNT_SOURCE_FULL_ROSTER = "bg3007_full_roster"
COUNT_SOURCE_CALLER = "caller_requested"
COUNT_SOURCES = (COUNT_SOURCE_FULL_ROSTER, COUNT_SOURCE_CALLER)


class Bg3007CensusError(ValueError):
    """A refusal from this module, always with a reason in the message."""


@dataclass(frozen=True)
class Bg3007PopulationGeneration:
    """One built Bg3007 collection: its membership, its bytes, nothing sent."""
...
def _require_anchor(player_xyz: Any) -> tuple[float, float, float]:
    if type(player_xyz) is not tuple or len(player_xyz) != 3:
        raise Bg3007CensusError("player XYZ must be an exact three-value tuple")
    import math
    checked = []
    for axis, value in zip("xyz", player_xyz):
        if type(value) not in (int, float) or type(value) is bool:
            raise Bg3007CensusError(
...
def census_order(
...
) -> tuple[identity.Bg3007Placement, ...]:
    """The 50 shippable placements, nearest the anchor first.
    ...
    slice of the ocean.  ``shippable_placements`` has already dropped the 16
...
def _entry(legacy: Any, placement: identity.Bg3007Placement) -> bytes:
    ...
    ``world_bg3007_identity``'s own HEADING paragraph -- 66 rows, f32_3 flat
    ...
    the BYTES by ``tests/test_world_population_bg3007.py`` -- shifting the
...
def build_bg3007_population(
    ...
    *,
    scene_id: int,
    count_source: str = COUNT_SOURCE_CALLER,
) -> Bg3007PopulationGeneration:
    """Build the Bg3007 roster as ONE RuntimeRes collection.  Sends nothing.

    ``scene_id`` has no default on purpose, the same reason every other
    builder gives: every actor in this table is encoded with scene 304, and
    a caller that forgets which scene it is in would deliver the Dark Fog
    Sea's ships and tornadoes into a different map.  Refuses anywhere but
    scene 304.
    """
    if type(scene_id) is not int or scene_id != SCENE_N_ID:
        raise Bg3007CensusError(
            "the Bg3007 roster is only valid in scene %d, not scene %r"
            % (SCENE_N_ID, scene_id))
    if count_source not in COUNT_SOURCES:
        raise Bg3007CensusError("unknown count source %r" % count_source)
    ...
    return Bg3007PopulationGeneration(
        count,
...
def wire_actor_count(generation: Bg3007PopulationGeneration) -> int:
    """Read the collection count back out of the bytes that will be sent."""
    if type(generation) is not Bg3007PopulationGeneration:
        raise Bg3007CensusError(
            "wire actor count needs a Bg3007PopulationGeneration")
    pc = generation.pc
...
def dispatch_report(generation: Bg3007PopulationGeneration) -> dict:
    ...
    if type(generation) is not Bg3007PopulationGeneration:
        raise Bg3007CensusError(
            "dispatch report needs a Bg3007PopulationGeneration")
...
def census_console_line(generation: Bg3007PopulationGeneration) -> str:
    """The one grep-able ``WORLD_CENSUS_BG3007`` line a boot in this scene
    prints.  ``assembled=N/66`` against the scene's REAL placement count, not
    against the 50 that happen to resolve.  ASCII only (the bridge console is
    cp874); no name appears on THIS line at all, see ``actor_lines`` for the
    layer that does print one.
    """
    report = dispatch_report(generation)
    return (
        "WORLD_CENSUS_BG3007 assembled={0}/{1} shippable={2} wire={3} "
...
def actor_lines(generation: Bg3007PopulationGeneration) -> tuple[str, ...]:
    ...
    the scene's 66 it means.
    """
    if type(generation) is not Bg3007PopulationGeneration:
        raise Bg3007CensusError("actor lines need a Bg3007PopulationGeneration")
...
def unresolved_lines() -> tuple[str, ...]:
    ...
    """
    return tuple(
        "BG3007_UNSHIPPED placement=%d set=%d cline_row=%d leader_n_id=%d "
        "reason=%s"
...
def census_console_lines(
    ...
    """``WORLD_CENSUS_BG3007`` + one line per actor + one per dropped one.

    Everything a headless proof for this scene needs, in one call, built from
    ONE generation.
    """
    generation = build_bg3007_population(
