"""Bg0006 (Ocean Walled City) census composer - LANE-A.

Fifth door of the ten surveyed in round ``12lyda``, following the same
placement-count-ranked sequence ``COO-DECISION 2026-08-30T14:41+07:00``
approved for scenes 4, 5, 10 and 14.  This is the census half; the identity
half is ``world_bg0006_identity``.
...
``world_population_bg0005`` and ``world_population_bg0010`` are the same
shape for scenes 2, 14, 4, 5 and 10.  This is the sixth: it refuses anywhere
but scene 6, over ``world_bg0006_identity``'s 66 shippable placements of 80.
Every wire call below is the exact frozen serializer the other five already
...
WHAT IS DIFFERENT FROM THE FOUR SIBLING SCENES, NAMED RATHER THAN LEFT
IMPLICIT.

* THREE resolved sets this scene's own crosswalk had to drop for a reason no
  sibling needed: a non-ASCII (CJK) ``MOBS_TIP.s_NAME`` that ``cp874`` cannot
  encode (sets 111-113, the scene's own teleporter NPCs going by their
  outfit).  See ``world_bg0006_identity``'s module docstring for the full
  account; this composer only sees the effect, three fewer shippable
  placements than a same-outfit sibling scene might have.
* No empty-name / INVISIBLE-marker placement family here (bg0004's set 107
  does not recur - every one of this scene's 38 resolved identities that
  ships has a real, ASCII ``MOBS_TIP.s_NAME``, checked in
  ``world_bg0006_identity._self_check``).
...
* There is no faction bit on any entry here, same as every sibling and for
  ...
  (the rank-1, ai-combat-nonzero sets - 25 through 35 of the 38 resolved)
...
* LANDING GEOMETRY.  ``scenarios/world_scene_registry_001.json``'s own
  ...
  scene's marker point 772.0 units from the nearest native placement,
...
* WIRED AND OPENED IN THE SAME ROUND AS THIS BUILD, following round
  ``l03cgh``'s compressed precedent for scene 5 rather than the earlier
  three-round split (build, then wire, then open) scenes 4's and 10's own
  first rounds used - the generic contract test
...
from . import world_bg0006_identity as identity
...
# Convention marker: matching the five sibling composer modules' own
# convention.
...
COUNT_SOURCE_FULL_ROSTER = "bg0006_full_roster"
COUNT_SOURCE_CALLER = "caller_requested"
COUNT_SOURCES = (COUNT_SOURCE_FULL_ROSTER, COUNT_SOURCE_CALLER)


class Bg0006CensusError(ValueError):
    """A refusal from this module, always with a reason in the message."""


@dataclass(frozen=True)
class Bg0006PopulationGeneration:
    """One built Bg0006 collection: its membership, its bytes, nothing sent."""
...
def _require_anchor(player_xyz: Any) -> tuple[float, float, float]:
    if type(player_xyz) is not tuple or len(player_xyz) != 3:
        raise Bg0006CensusError("player XYZ must be an exact three-value tuple")
    import math
    checked = []
    for axis, value in zip("xyz", player_xyz):
        if type(value) not in (int, float) or type(value) is bool:
            raise Bg0006CensusError(
...
def census_order(
...
) -> tuple[identity.Bg0006Placement, ...]:
    """The 66 shippable placements, nearest the anchor first.
    ...
    slice of the port.  ``shippable_placements`` has already dropped the 14
...
def _entry(legacy: Any, placement: identity.Bg0006Placement) -> bytes:
...
def build_bg0006_population(
    ...
    *,
    scene_id: int,
    count_source: str = COUNT_SOURCE_CALLER,
) -> Bg0006PopulationGeneration:
    """Build the Bg0006 roster as ONE RuntimeRes collection.  Sends nothing.

    ``scene_id`` has no default on purpose, the same reason every other
    builder gives: every actor in this table is encoded with scene 6, and a
    caller that forgets which scene it is in would deliver Ocean Walled
    City NPCs into a different map.  Refuses anywhere but scene 6.
    """
    if type(scene_id) is not int or scene_id != SCENE_N_ID:
        raise Bg0006CensusError(
            "the Bg0006 roster is only valid in scene %d, not scene %r"
            % (SCENE_N_ID, scene_id))
    if count_source not in COUNT_SOURCES:
        raise Bg0006CensusError("unknown count source %r" % count_source)
    ...
    return Bg0006PopulationGeneration(
        count,
...
def wire_actor_count(generation: Bg0006PopulationGeneration) -> int:
    """Read the collection count back out of the bytes that will be sent."""
    if type(generation) is not Bg0006PopulationGeneration:
        raise Bg0006CensusError(
            "wire actor count needs a Bg0006PopulationGeneration")
    pc = generation.pc
...
def dispatch_report(generation: Bg0006PopulationGeneration) -> dict:
    ...
    if type(generation) is not Bg0006PopulationGeneration:
        raise Bg0006CensusError(
            "dispatch report needs a Bg0006PopulationGeneration")
...
def census_console_line(generation: Bg0006PopulationGeneration) -> str:
    """The one grep-able ``WORLD_CENSUS_BG0006`` line a boot in this scene
    prints.  ``assembled=N/80`` against the scene's REAL placement count, not
    against the 66 that happen to resolve.  ASCII only (bridge console is
    cp874).
    """
    report = dispatch_report(generation)
    return (
        "WORLD_CENSUS_BG0006 assembled={0}/{1} shippable={2} wire={3} "
...
def actor_lines(generation: Bg0006PopulationGeneration) -> tuple[str, ...]:
    ...
    shipped table is ASCII by ``world_bg0006_identity._self_check``.
    """
    if type(generation) is not Bg0006PopulationGeneration:
        raise Bg0006CensusError("actor lines need a Bg0006PopulationGeneration")
...
def unresolved_lines() -> tuple[str, ...]:
    ...
    """
    return tuple(
        "BG0006_UNSHIPPED placement=%d set=%d cline_row=%d leader_n_id=%d "
        "reason=%s"
...
def census_console_lines(
    ...
    """``WORLD_CENSUS_BG0006`` + one line per actor + one per dropped one.

    Everything a headless proof for this scene needs, in one call, built from
    ONE generation.
    """
    generation = build_bg0006_population(
