...
MOB_COMBAT_NONCLAIMS = (
    ...
    "A HIT THAT DOES NOT KILL LEAVES NO GROUND GENERATION BEHIND IT, AND "
    "THE FRAME THAT PUTS A FLOOR BACK IS THE KILL'S DROP GENERATION, NOT "
    "EITHER DEATH FRAME.  Round j8qsxp, driven in Bg0002 - the scene the "
    "owner plays in - with a row genuinely standing on the floor "
    "(tests/test_mob_combat_dispatch_bg0002_kill.py::test_a_hit_that_does_"
    "not_kill_leaves_the_floor_cleared_behind_it), after he watched a "
    "dropped item blink out while he hit the next monster in R306/GT-216.  "
    "A post-arrival hit composes [announce, bar]: the announce carries the "
    "pool, the ~18 KB bar recompose does not, and it is the LAST action of "
    "that burst while the row is still alive in the server's own ledger.  "
    "A kill composes [announce, dying, dead, drop]: both death frames carry "
    "NO ground section, and the drop generation - the whole live ledger of "
    "the scene (mob_drop_presence.PRESENCE_SHAPE), ground bit set - is last "
    "and carries the older rows too.  SO THE READING IN pf_bridge "
    "notes_to_chief/20260903_1657 (R306 cross-lane item 1, offered there as "
    "a hypothesis) NAMES THE WRONG FRAME IN THE RIGHT BURST: ~~the death "
    "frame's ground section adds it back~~ - the drop generation beside it "
    "does.  THREE LIMITS, NAMED RATHER THAN IMPLIED: (1) LIST ORDER is what "
    "is measured; wire order and client-apply order are two further layers "
    "and only the first has ever been watched, so 'the client's pool ends "
    "up cleared' stays this lane's reading of these bytes; (2) the drop "
    "generation is not the only publisher of that shape - mob_loot's "
    "scene-boundary enter_scene_frames composes it with no kill involved; "
    "(3) the same letter says the vanish happened 'sometimes on the next "
    "first hit, sometimes not', and this model is deterministic, so "
    "something it has not found accounts for the 'sometimes not'.  The ~2 s "
    "ground heartbeat is NOT a restorer either way: its body is "
    "present-with-count-0, a no-op under RE-082's STATIC-ON-BRIDGE reading "
    "of the list consumer, so it can preserve a floor and never repopulate "
    "one - a reading this lane already flags as awaiting the COO where the "
    "body is composed, not a client observation",
)
