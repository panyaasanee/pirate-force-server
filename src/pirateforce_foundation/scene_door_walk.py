...
door: ``mob_combat.strike``, ``mob_death.ruling_for`` + ``mob_death.kill``,
``mob_loot.roll_drops`` + ``DropLedgerCell.loot_a_kill``.  A door is open
because this module made it open, once, on the shipped roster.
...
NONCLAIMS
---------
* NO claim about a client's screen.  Every "open" here is server-side: the
  bytes were composed and the refusal did not happen.  What a player sees
  when they land in the scene is an attended round's answer, and NOW.md
  still forbids opening a monster-hit ticket for scenes 3/5/14 until P-2
  (monster name colour) closes.
...
* NO claim about scene 4.  This module reports the CONDITION in item 3; the
  decision that follows from it is the COO's.
...
#: calls.  `lane_hooks`'s own gate reads this name.
...
SCENE_DOORS_REFUSED_TOKEN = "SCENE_DOORS_REFUSED"
...
#: sweep is what separates "this row's sets are not in the shipped tables at
#: all" (0 of N, always, for every N) from "this row drops sometimes".
...
class SceneDoors:
    ...
    @property
    def every_door_open(self) -> bool:
        """Every SHIPPED row of this scene went through all three doors.
        ...
        and neither is an empty roster.
        """
        return bool(self.rows) and not self.reason and all(
            row.every_door_open for row in self.rows)
...
def _walk_row(legacy: Any, mob: Any) -> RowDoors:
    ...
    try:
        ...
        target = bool(step.announce_frame or step.bar_frame)
...
def walk_scene(legacy: Any, scene: Any) -> SceneDoors:
    """Walk every shipped row of ``scene`` through all three doors.
    ...
    is a diagnostic nobody may wire.
    """
    ...
    try:
        ...
        return SceneDoors(label, reason=REFUSE_ROSTER_UNREADABLE)
    return SceneDoors(label, tuple(_walk_row(legacy, mob) for mob in roster))
...
def describe_scene_doors(walked: SceneDoors) -> str:
    ...
    if walked.reason:
        return "%s scene=%r reason=%s" % (
            SCENE_DOORS_REFUSED_TOKEN, walked.scene, walked.reason)
    return (
        "%s scene=%r rows=%d target=%d kill=%d drop=%d every_door=%s short=%s"
        % (
            SCENE_DOORS_TOKEN, walked.scene, walked.rows_walked,
...
def describe_live_scene_doors(legacy: Any) -> tuple:
    ...
    finished = tuple(
        one.scene for one in walked if one.every_door_open)
    lines.append(
        "%s summary live_scenes=%d every_door=%s"
        % (SCENE_DOORS_TOKEN, len(walked), ",".join(finished) or "none"))
    return tuple(lines)
