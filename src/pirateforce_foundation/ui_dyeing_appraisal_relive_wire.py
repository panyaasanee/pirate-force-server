...
TSV, which has no id column):
    ...
    DyeingVitalRes       0x2A00  1 field  (Dyeing group, 3 of 4)
    DyeingRemoveVital    0x3E1C  2 fields (Dyeing group, 3 of 4)
    DyeingShipVitalReq   0x441A  2 fields (Dyeing group, 3 of 4)
    ReliveVital          0x1AD4  2 fields (Relive group, 1 of 2)

All six are combined in one module (instead of one file per group like this
lane's earlier rounds) to stay inside ``AGENTS.md`` section 7's "วิธีเปิด PR"
~6-files-per-PR cap: three separate module+test pairs plus the
``docs/UI_LANE.md`` update would be 7 files; one module + one test file +
the doc update is 3.

Excluded from this module, with reasons:

- ``DyeingVitalReq`` (0x29E4, the Dyeing group's 4th class) -- its field 2
  is tagged ``UNTAGGED_STRING8_LEN32LE`` in ``PF_SERIALIZER_FIELDS.tsv``,
  which looks safe to treat as "no tag byte, just len32+payload". It is
  NOT safe: ``src/pirateforce_foundation/logout_hypothesis.py``'s
  ``ReturnSelectServerVital`` erratum (RE-196/GT-055) already proved this
  exact label only describes the scope of the string-writing HELPER CALL
  span -- a tag-push instruction can sit immediately before that call,
  outside the labelled span. ``DeleteActorVital``'s string field carries
  the IDENTICAL ``UNTAGGED_STRING8_LEN32LE`` label yet GT-018 independently
  confirmed a real 0x44 tag byte precedes it. No round has checked whether
  a tag-push instruction sits immediately before ``DyeingVitalReq``'s own
  ``string_wire_call@0x006C65A4`` (file_off 0x002C59A4). Shipping this
  field as untagged here would repeat the exact overclaim RE-196 already
  caught once. This class needs that specific static-RE question answered
  (tag-push found immediately before the call, or none found) before it
  can be added to this module or ``docs/UI_LANE.md``'s table --
  NEEDS-RE-STATIC, not HEADLESS-DONE.
...
``^DyeingShipVitalReq``, ``^ReliveVital``); W and R rows are identical shape
for every class here, each with its own distinct span (no shared serializer
case in this batch).
...
"helper-call-span" ambiguity excluding ``DyeingVitalReq`` above: every row
this module implements has ``gate_condition=ALWAYS`` and a ``span_start``/
``span_end`` matching the enclosing FUNCTION's own span (not a narrower
``string_wire_call@...`` sub-span) -- these are direct
...
``0x0089A740``), which none of these six classes' implemented fields use.
...
``proven_semantics`` column is ``UNKNOWN`` for every row of all six classes,
