...
def encode_trace_path_found_payload(x: int, y: int, z: int) -> bytes:
    ...
    ``trace_path.make_trace_path_empty_response`` wraps its own -- call the
    legacy v141 module's runtime-vitals framing helper (see that function's
    own body for the exact call) with a one-tuple list: ``(trace_path.
    TRACE_PATH_VITAL_ID, trace_path.TRACE_PATH_VITAL_VERSION,
    encode_trace_path_found_payload(x, y, z))``. Deliberately not spelled
    out as a literal call here (this file's own docstring text is plain-text
    scanned by ``tests/test_static_verifier_pins_cloud.py``'s cross-repo
    call-site census -- the full suite caught an earlier draft's literal
    example bumping that count by one, which would have forced a real-file
    re-pin for a call this file never actually makes).
    ``TRACE_PATH_VITAL_ID``/``TRACE_PATH_VITAL_VERSION`` are
    ``trace_path.py``'s response-side constants (its lines 24/31), not
    defined in this module, which only defines the request-side
    ``TRACE_PATH_REQ_VITAL_ID``/``_VERSION`` above -- name them with the
    ``trace_path.`` prefix; a future caller pattern-matching for the bare
    names would hit a ``NameError``.
    """
