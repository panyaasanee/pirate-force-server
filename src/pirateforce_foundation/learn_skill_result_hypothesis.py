...
# ---------------------------------------------------- the proven 0x673C frame
# The wire shape of this vital -- its static pins, its record triple and the
# encode/decode/compose trio -- lives in a plain frame module with no opt-in
# gate of its own, so the login lane can use the SHAPE without importing this
# sweep lane (COO-DECISION 20260907_2241).  Re-exported here under the names
# every existing caller and test already uses: the move changes no bytes and
# no signature.
from .learn_skill_result_frame import (  # noqa: F401  (re-exported)
    ...
    LEARN_SKILL_RESULT_READ_LOOP_LEN,
    LEARN_SKILL_RESULT_READ_LOOP_SHA256,
    LEARN_SKILL_RESULT_READ_LOOP_VA,
    LEARN_SKILL_RESULT_RECORD_OBJECT_STRIDE,
    ...
    LEARN_SKILL_RESULT_SERIALIZER_LEN,
