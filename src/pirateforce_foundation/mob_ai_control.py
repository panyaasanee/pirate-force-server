...
MOB_AI_CONTROL_NONCLAIMS = (
    ...
    "a key no module owns, and the fail-closed lookup answered False on "
    "every frame. [REPAIRED, round gjyxt5 (R324), 2026-09-03T18:xx+07:00, ticket 1648: the call site passes lane_b_mob_ai_tick.MODULE_NAME now, module_production_allowed answers True to it, and MOB_AGGRO_TICK_REACHABLE is True -- derived, not declared. So tick_step and commit_step DO run for a player who walks, and had not before that commit. ~~Still not observable: every shipped roster is non-offensive, so an undamaged mob ticks to the state it already had~~ [THAT CLAUSE STRUCK, round nfrrqa (LANE-B), 2026-09-03T20:1x+07:00, MEASURED FALSE and found by pf-adversary D4 still shipping here after mob_aggro.py's copy was struck: driven on a real shipped roster through mob_ai_scheduler.tick_session. ~~Bg0002, 17 rows, FIVE acquire the player by proximity (ai_wander 11: n_OFFESIVE=1, n_AGGRO=1200) and placement 92 (Orc Chief, template 103) returns INTENT_ATTACK_UNDELIVERABLE on every tick inside MELEE_ATTACK_RANGE~~ RE-MEASURED round najn72 (pf-adversary D8: that measurement described a roster that no longer exists): Bg0002 was re-mined through the crosswalk with the owner's outfit rule (NOW.md 1313, tick 20260908_0025), it ships 52 rows, ALL of them ai_wander 16, ZERO acquire, and placements 92-96 are not in its table at all -- so on scene 2 the struck clause is true again. It is still STRUCK, because it claimed EVERY shipped roster, and that is false: re-driven on Bg0003, 12 rows, TWO offensive, and with a player standing on placement 33 (Ward Apes, template 65) ONE row acquires and returns INTENT_ATTACK_UNDELIVERABLE on every tick inside MELEE_ATTACK_RANGE, because ATTACK_CADENCE_TICKS is 1. It is true of bg0001's four dummies and of Bg0002, and false of Bg0003 -- which is why the claim about EVERY roster stays struck. WHAT SURVIVES THE STRIKE, and is the half that carried the meaning: still not observable. No frame is composed for any of it -- nonclaim 4 below is untouched and ATTACK_INTENT_DELIVERABLE is still False.] The line "
    "reference above (runtime.py:5196-5210) is stale too -- the block "
    "moved again in that round. WHAT IS NOT STRUCK: the damage FOLD is "
    ...
    "MOB_AGGRO_TICK_REACHABLE, True since round gjyxt5). "
    "The one-string repair was asked for in pf_bridge CORE-REQUEST "
    "20260903_1639 and landed in round gjyxt5; the guard that went red on "
    "it is test_the_tick_gate_is_reported_not_assumed, which reads the "
    "call site out of the AST -- NOT "
    "test_the_gate_answers_what_it_answered_at_every_hand_spelled_site, "
    "which lost that row instead, because the site stopped typing a "
    "literal at all.",
