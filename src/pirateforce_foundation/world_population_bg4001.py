"""Bg4001 (Navy Training Camp) census composer - LANE-A.

Tenth and LAST door of the ten surveyed in round ``12lyda``, following the
same placement-count-ranked sequence ``COO-DECISION 2026-08-30T14:41+07:00``
approved for scenes 4, 5, 10, 6, 8, 3, 7, 9 and 11.  This is the census
half; the identity half is ``world_bg4001_identity``.
...
``world_population_bg0007``, ``world_population_bg0009`` and
``world_population_bg0011`` are the same shape for scenes 2, 14, 4, 5, 6,
8, 3, 7, 9 and 11.  This is the twelfth: it refuses anywhere but scene
130, over ``world_bg4001_identity``'s 41 shippable placements of 42.
Every wire call below is the exact frozen serializer the other eleven
already use (``legacy.make_npc_attr`` / ``make_remote_movement_attr`` /
``make_remote_actor_entry`` / ``make_runtime_remote_actors``), and the
wire header constants and the accepted initial+reapply schedule are
IMPORTED from ``world_population`` rather than redefined - they describe
the wire format, which is not scene-specific.

WHAT IS DIFFERENT FROM THE TEN SIBLING SCENES, NAMED RATHER THAN LEFT
IMPLICIT.

* NOT AN ELEVATED-RISK ROW.  Unlike scenes 10 and 11, this scene does NOT
  carry the registry's own ``table_row_differences.the_two_interiors``
  flag - measured, not assumed (``n_CANGLIDE`` 1, ``n_LIMIT_HEIGHT`` 0,
  not the (0, 0) pair that flag names).  Its marker point sits 1018.201
  units from the nearest native placement, OUTSIDE the placement extents
  (one of six of the ten doors landing outside rather than inside).
* No CJK/non-cp874 name family this scene needed among the 17 SHIPPED
  identities - every one of this scene's resolved identities is plain
  ASCII (checked in ``world_bg4001_identity._self_check``).
...
* ONE failure shape only among the 1 unresolved set ("MOBS row carries no
  s_OUTFIT") - the narrowest unresolved count of the eleven scenes this
  lane has crosswalked (1 of 42 placements, versus 5-11 for the earlier
  ten).
* TWO sets ship a multi-variant outfit, both THREE-variant rows (a wider
  fan-out than every sibling's own two-variant rows, narrower than
  bg0003's nine-variant outlier) - see ``world_bg4001_identity``'s own
  docstring.
* HP is real and comes from the table chain, not composed:
  ``STANDARD_MOB[MOBS.n_LEVEL_MIN].n_HPMAX``, sent as current == max
  (alive), matching every other composer's "no half-dead spawn"
  convention.  Unlike every sibling, this scene's own HP spread is wide
  within itself (421 for fifteen sets, 616267 for two) - see the identity
  module's own "LEVEL SPREAD" paragraph.
...
* LANDING GEOMETRY.  ``scenarios/world_scene_registry_001.json``'s own
  ``table_row_differences`` block for this scene (marker point 1018.201
  units from the nearest native placement, OUTSIDE the placement extents,
  and NOT carrying ``the_two_interiors``) is about the ARRIVAL POINT, not
  this composer - this file assembles the roster around whatever point a
  caller gives it (``player_xyz``), the same as every other composer, and
  carries no opinion on where a player should stand.
* WIRED AND OPENED IN THE SAME ROUND AS THIS BUILD, following rounds
  ``l03cgh``/``fx0007``/``p4wire``/``p7wm17``/``78zayw``/``ir0lpw``/
  ``68mm02``'s compressed precedent for scenes 5, 6, 8, 3, 7, 9 and 11
  rather than the earlier three-round split (build, then wire, then open)
  scenes 4's and 10's own first rounds used - the generic contract test
  ...
  scenes 3/4/5/6/7/8/9/10/11/14 all were by the time this round started.
* LAST OF THE ORIGINAL TEN.  With this scene wired and opened, every one
  of the ten doors round ``12lyda`` surveyed (COO-DECISION
  20260830_1441's own placement-count-ranked sequence) is open at login.
  This module does not claim anything about M2 (the sea/ship map family)
  or M3 (scene 14's own out-of-sequence crosswalk) - both are separate
  scopes this round does not touch.
"""
...
from . import world_bg4001_identity as identity
...
# Convention marker: matching the eleven sibling composer modules' own
# convention.
...
COUNT_SOURCE_FULL_ROSTER = "bg4001_full_roster"
COUNT_SOURCE_CALLER = "caller_requested"
COUNT_SOURCES = (COUNT_SOURCE_FULL_ROSTER, COUNT_SOURCE_CALLER)


class Bg4001CensusError(ValueError):
    """A refusal from this module, always with a reason in the message."""


@dataclass(frozen=True)
class Bg4001PopulationGeneration:
    """One built Bg4001 collection: its membership, its bytes, nothing sent."""
...
def _require_anchor(player_xyz: Any) -> tuple[float, float, float]:
    if type(player_xyz) is not tuple or len(player_xyz) != 3:
        raise Bg4001CensusError("player XYZ must be an exact three-value tuple")
    import math
    checked = []
    for axis, value in zip("xyz", player_xyz):
        if type(value) not in (int, float) or type(value) is bool:
            raise Bg4001CensusError(
...
def census_order(
...
) -> tuple[identity.Bg4001Placement, ...]:
    """The 41 shippable placements, nearest the anchor first.
    ...
    slice of the camp.  ``shippable_placements`` has already dropped the 1
    with no identity; this only orders what it returned.
    """
...
def _entry(legacy: Any, placement: identity.Bg4001Placement) -> bytes:
...
def build_bg4001_population(
    ...
    *,
    scene_id: int,
    count_source: str = COUNT_SOURCE_CALLER,
) -> Bg4001PopulationGeneration:
    """Build the Bg4001 roster as ONE RuntimeRes collection.  Sends nothing.

    ``scene_id`` has no default on purpose, the same reason every other
    builder gives: every actor in this table is encoded with scene 130, and
    a caller that forgets which scene it is in would deliver Navy Training
    Camp NPCs into a different map.  Refuses anywhere but scene 130.
    """
    if type(scene_id) is not int or scene_id != SCENE_N_ID:
        raise Bg4001CensusError(
            "the Bg4001 roster is only valid in scene %d, not scene %r"
            % (SCENE_N_ID, scene_id))
    if count_source not in COUNT_SOURCES:
        raise Bg4001CensusError("unknown count source %r" % count_source)
    ...
    return Bg4001PopulationGeneration(
        count,
...
def wire_actor_count(generation: Bg4001PopulationGeneration) -> int:
    """Read the collection count back out of the bytes that will be sent."""
    if type(generation) is not Bg4001PopulationGeneration:
        raise Bg4001CensusError(
            "wire actor count needs a Bg4001PopulationGeneration")
    pc = generation.pc
...
def dispatch_report(generation: Bg4001PopulationGeneration) -> dict:
    ...
    if type(generation) is not Bg4001PopulationGeneration:
        raise Bg4001CensusError(
            "dispatch report needs a Bg4001PopulationGeneration")
...
def census_console_line(generation: Bg4001PopulationGeneration) -> str:
    """The one grep-able ``WORLD_CENSUS_BG4001`` line a boot in this scene
    prints.  ``assembled=N/42`` against the scene's REAL placement count, not
    against the 41 that happen to resolve.  ASCII only (bridge console is
    cp874).
    """
    report = dispatch_report(generation)
    return (
        "WORLD_CENSUS_BG4001 assembled={0}/{1} shippable={2} wire={3} "
...
def actor_lines(generation: Bg4001PopulationGeneration) -> tuple[str, ...]:
    ...
    shipped table is ASCII by ``world_bg4001_identity._self_check``.
    """
    if type(generation) is not Bg4001PopulationGeneration:
        raise Bg4001CensusError("actor lines need a Bg4001PopulationGeneration")
...
def unresolved_lines() -> tuple[str, ...]:
    ...
    """
    return tuple(
        "BG4001_UNSHIPPED placement=%d set=%d cline_row=%d leader_n_id=%d "
        "reason=%s"
...
def census_console_lines(
    ...
    """``WORLD_CENSUS_BG4001`` + one line per actor + one per dropped one.

    Everything a headless proof for this scene needs, in one call, built from
    ONE generation.
    """
    generation = build_bg4001_population(
